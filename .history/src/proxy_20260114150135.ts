 ...
 * Logic: Removed '/home' to ensure it triggers the Soft Auth engine.
 */
const PUBLIC_PASS_THROUGH = [
  ...
  '/maintenance' // 🚀 ALLOWED: Public access to maintenance node
];
...
/**
 * 🛰️ GLOBAL PROXY GATEKEEPER (v16.10.0)
 * Fix: Synchronized for Soft Auth on /home and /dashboard.
 */
export async function middleware(request: NextRequest) {
  ...
  if (pathname.startsWith('/dashboard') || pathname.startsWith('/home')) {
    // 🕵️ ANTI-COLLISION: Select latest valid token
    ...
    if (!activeToken) {
      ...
      loginUrl.searchParams.set('redirect', pathname); // 🚀 Redirect back after auth
    ...
    try {
      ...
      // 🛰️ HEADER HYDRATION: Vital for /api/user/profile consistency
      response.headers.set("x-invoke-path", pathname);
      response.headers.set("x-url", publicRequestUrl);

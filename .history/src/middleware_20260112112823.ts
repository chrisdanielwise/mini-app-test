...
export async function middleware(request: NextRequest) {
  ...
  // 1. RECONSTRUCT ORIGIN
  const host = request.headers.get("x-forwarded-host") || request.headers.get("host");
  const protocol = request.headers.get("x-forwarded-proto") || "https";
  const actualOrigin = `${protocol}://${host}`;

  // 🛡️ 2. THE TERMINATOR (CRITICAL FIX)
  // If we are already on the login page, or there's a 'reason' param, 
  // we MUST STOP and let the page render. 
  ...
  if (pathname.startsWith('/dashboard')) {
    if (!token) {
      ...
      const loginUrl = new URL('/dashboard/login', actualOrigin);
      // Adding the reason param ensures that Step 2 catches it on the next hop
      loginUrl.searchParams.set('reason', 'auth_required');

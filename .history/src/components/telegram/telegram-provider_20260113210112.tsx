 ...
 * Logic: Controls the visibility and action of the Telegram Back Button.
 * Strategy: Button appears only when the user is deep in a navigation stack.
 */
useEffect(() => {
  ...
  // Determine if the user is on a root-level page
  const isRootPath = pathname === "/dashboard" || pathname === "/home" || pathname === "/";

  if (!isRootPath) {
    // Show back button and attach the handler
    telegram.setBackButton(true, handleBackNavigation);
  } else {
    // Hide back button on root dashboards

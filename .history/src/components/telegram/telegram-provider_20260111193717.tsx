...
export function TelegramProvider({ children }: { children: ReactNode }) {
  ...
  // 1. HARDWARE SYNC
// Inside your useEffect for Handshake
useEffect(() => {
  if (!mounted || auth.isAuthenticated || handshakeLock.current) return;

  // 🛡️ THE STABILIZER: Wait 100ms for the Telegram SDK to inject itself
  const timer = setTimeout(() => {
    performHandshake();
  }, 100);

  return () => clearTimeout(timer);
}, [mounted, auth.isAuthenticated, performHandshake]);
  /**

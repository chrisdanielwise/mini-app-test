...
// 🛡️ UPDATED INTERFACE: Includes webApp and mounted for native features
export interface TelegramContextValue {
  auth: ReturnType<typeof useAuth>;
  isReady: boolean;
  mounted: boolean; 
...
/**
 * 🛰️ TELEGRAM IDENTITY ENGINE (Institutional v12.35.0)
 * Logic: Stable handshake with explicit property mapping (No spread crashes).
 */
export function TelegramProvider({ children }: { children: ReactNode }) {
  const telegram = useTelegram();
  const auth = useAuth();
  ...
  const performHandshake = useCallback(async () => {
    ...
    if (!telegram?.isReady || auth.isAuthenticated || auth.isLoading || handshakeLock.current) {
      return;
    }
  ...
  // 2. TRIGGER SYNC
useEffect(() => {
    // 🛡️ Added 'service?.tiers' check to prevent crash if tiers are missing
    if (!isTelegram || !isReady || !auth.isAuthenticated || !tunnelReady || !service?.tiers) return;

    if (!selectedTierId) {
      setMainButton({ text: "SELECT ACCESS LEVEL", visible: true, active: false });
    } else {
      // 🚀 Using optional chaining here is critical
  ...
  return (
    <TelegramContext.Provider 
      value={{ 
        // 🚀 EXPLICIT MAPPING: Prevents crashes by ensuring functions always exist

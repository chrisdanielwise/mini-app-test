...
/**
 * 🛰️ TELEGRAM IDENTITY ENGINE (Institutional v12.38.0)
 * Logic: Stable handshake with explicit property mapping.
 * Fix: Removed misplaced service-level logic from global provider.
 */
export function TelegramProvider({ children }: { children: ReactNode }) {
  const telegram = useTelegram();
  const auth = useAuth();
   ...
   */
  const performHandshake = useCallback(async () => {
    // Prevent handshake on dashboard login to avoid redirect loops
    if (pathname === "/dashboard/login") return;

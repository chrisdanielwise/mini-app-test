...
/**
 * 🛰️ TELEGRAM IDENTITY ENGINE (Institutional v12.39.0)
 * Logic: Crash-resilient architecture with strict hydration gating.
 * Fix: Moved UI wrapper inside mounting logic to prevent Next.js hydration errors.
 */
export function TelegramProvider({ children }: { children: ReactNode }) {
  const telegram = useTelegram();
  const auth = useAuth();
  ...
  // --- RENDERING BARRIER ---
  // 🚀 CRITICAL: Prevents any HTML output until client-side is ready.
  // This stops the "Application Error" caused by browser/server HTML mismatch.
  if (!mounted) return null;

  if ((telegram?.isTelegram && !telegram?.isReady) || isRedirecting) {
  ...
  return (
    ...
    >
      <div className="relative flex min-h-[100dvh] w-full flex-col bg-background selection:bg-primary/30 antialiased overflow-x-hidden">
        {children}
        {/* Subtle Grid Overlay moved inside the mounted check */}
        <div className="fixed inset-0 pointer-events-none z-[-1] opacity-[0.03] bg-[radial-gradient(#fff_1px,transparent_1px)] [background-size:40px_40px]" />
      </div>
    </TelegramContext.Provider>
  );

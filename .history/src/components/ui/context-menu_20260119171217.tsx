 ...
 * Fix: Resolved TS2339 by moving haptics to onOpenAutoFocus.
 */
  ...
  return (
    <ContextMenuPrimitive.Portal>
      <ContextMenuPrimitive.Content
        ...
        /**
     * ✅ FIX: Resolved TS2322. 
     * Radix ContextMenu Content does not have onOpenAutoFocus.
     * We use onCloseAutoFocus or handle haptics via the Trigger or a custom wrapper.
     */
    onCloseAutoFocus={() => {
      if (isReady && impact) impact("light");
    }}
        className={cn(
...
function ContextMenuItem({
  ...
  const handleSelect = (e: Event) => {
    if (isReady && selectionChange) selectionChange();
  ...
  return (
    <ContextMenuPrimitive.Item
      ...
      className={cn(
        ...
        "focus:bg-primary/10 focus:text-primary data-[disabled]:opacity-10",
        inset && "pl-8",
...
function ContextMenuLabel({ className, inset, ...props }: React.ComponentPropsWithoutRef<typeof ContextMenuPrimitive.Label> & { inset?: boolean }) {
  return (
    <ContextMenuPrimitive.Label 
      className={cn("px-2.5 py-1.5 text-[7.5px] font-black uppercase tracking-[0.4em] opacity-20 italic", inset && "pl-8", className)} 
      {...props} 
    />
  );
}

function ContextMenuSeparator({ className, ...props }: React.ComponentPropsWithoutRef<typeof ContextMenuPrimitive.Separator>) {
 ...
 */
function ContextMenuSubTrigger({ className, inset, children, ...props }: React.ComponentPropsWithoutRef<typeof ContextMenuPrimitive.SubTrigger> & { inset?: boolean }) {
  return (
...
function ContextMenuSubContent({ className, ...props }: React.ComponentPropsWithoutRef<typeof ContextMenuPrimitive.SubContent>) {
  return (

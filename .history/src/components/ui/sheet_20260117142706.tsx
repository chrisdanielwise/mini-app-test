 ...
 * Fix: Elevated Z-Index (600) to mask the stationary DashboardTopNav.
 */
 ...
 * Fix: Aligned Close Trigger (+26px) to match h-14 Header center-line.
 */
  ...
  return (
    <SheetPortal>
      ...
      >
        ...
        <div className="flex-1 overflow-y-auto scrollbar-none overscroll-contain px-6 py-4 outline-8 outline-red-700">
          {children}
        </div>

        {/* ✖️ TACTICAL CLOSE: PERFECT HORIZONTAL ALIGNMENT 
            Logic: Root Buffer + (Header Height / 2) - (Icon Height / 2)
            Matches the visual center of your DashboardTopNav Hamburger icon.
        */}
        <SheetPrimitive.Close 
          className="absolute right-4 rounded-lg p-2 bg-white/5 opacity-40 hover:opacity-100 transition-all active:scale-95 focus:outline-none outline-8 outline-blue-700"
          style={{ top: `calc(var(--tg-safe-top, ${safeArea.top}px) + 26px)` }}
        >

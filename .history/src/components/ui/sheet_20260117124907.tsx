 ...
 * Fix: Removed multipliers to satisfy absolute hardware clearance.
 * Fix: Integrated raw CSS variable fallback for --tg-safe-top.
 */
  ...
  return (
    <SheetPortal>
      ...
      <SheetPrimitive.Content
        ...
        className={cn(
          "fixed z-[500] flex flex-col bg-zinc-950/95 backdrop-blur-3xl shadow-apex transition-all duration-400 ease-institutional",
          "data-[state=open]:animate-in data-[state=closed]:animate-out",
        ...
        style={{
          // 🛡️ THE HARDWARE LOCK: 1:1 Mapping to Notch and Home Indicator
          // We use your CSS variables if available, falling back to JS telemetry
          paddingTop: `var(--tg-safe-top, ${safeArea.top}px)`,
          paddingBottom: `var(--tg-safe-bottom, ${safeArea.bottom}px)`,
        } as React.CSSProperties}
        {...props}
      >
        {/* 🛡️ ACCESSIBILITY TITLE (Fixed Console Error) */}
        <SheetTitle className="sr-only">Navigation_Menu</SheetTitle>

        {/* 🌊 LAMINAR RESERVOIR: Allows internal scrolling while the drawer stays fixed */}
        <div className="flex-1 overflow-y-auto scrollbar-none overscroll-contain px-6 pt-4">
          {children}
        </div>

        {/* ✖️ TACTICAL CLOSE: Positioned dynamically to clear your Fluid Scaling Engine notch */}
        <SheetPrimitive.Close 
          ...
          style={{ top: `calc(var(--tg-safe-top, ${safeArea.top}px) + 8px)` }}
        >

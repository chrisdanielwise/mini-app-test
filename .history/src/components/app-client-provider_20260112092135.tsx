 ...
 * Optimized: Passive Stand-down for Cloudflare/Middleware redirect reasons.
 */
export function AppClientProvider({ children }: { children: React.ReactNode }) {
  ...
  const initTMA = useCallback((force = false) => {
    // 🛡️ THE TERMINATOR
    // Yields immediately if at the gate or if a redirect reason is present.
    // This stops the hardware from fighting the Middleware's 302 instructions.
    ...
    // 🛡️ VERSION GATE (Critical Fix for v6.0 Crashes)
    // Ensures we don't call modern APIs on legacy Telegram nodes.
    ...
    // 2. Viewport Hardening (Conditional 6.1+)
    if (hasModernApi) {
      try {
        if (tg.isVerticalSwipesEnabled) tg.disableVerticalSwipes();
      } catch (e) {
        console.warn("[Hardware_Protocol] Swipe lock restricted on this node.");
      }
    }
    ...
    // 4. Kinetic Theme Sync (CSS Variable Injection)
    ...
    // 5. Native Header Protocol (Conditional 6.1+)
    // 🚀 FIXED: This block was crashing v6.0 clients in your previous logs.
    if (hasModernApi) {
      try {
        ...
        console.warn("[Hardware_Protocol] Native UI sync failed.");
      }
    }

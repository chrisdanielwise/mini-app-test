 ...
 * Strategy: Hard-Coded Flex Displacement.
 * Logic: Forced 54px offset ensures the title is always below the status bar.
 */
export function DashboardShell({ children }: { children: React.ReactNode }) {
  ...
  useLayoutEffect(() => {
    if (typeof window !== "undefined" && WebApp && isReady) {
      try {
        WebApp.ready();
        WebApp.expand();
        // 🛡️ HARDWARE LOCK: Kills native bounce that causes sliding gaps
  ...
  return (
     ...
     * Locked to absolute physical screen edges usingsvh units.
     */
    <div
      className={cn(
    ...
    >
      {/* 📐 TOP_BUFFER: The Physical Displacement Wall
          We use 54px to clear the Telegram status bar, notch, and Close button.
          This div takes up space so Layer 1 MUST start below it. */}
      <div 
        style={{ height: `${Math.max(safeArea.top, 54)}px` }} 
        className="shrink-0 w-full bg-black z-[100]" 
      />

      {/* 🚀 LAYER 1: STATIONARY_HUD_HEADER
          This is now GUARANTEED to be below the status bar. */}

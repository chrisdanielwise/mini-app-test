...
/**
 * 🌊 FLUID PAYOUT LEDGER (v16.16.12)
 * Logic: Haptic-synced transaction scrubbing with Role-Aware Oversight.
        ...
        <table className="w-full text-left border-collapse min-w-[900px]">
          <thead>
            <tr className="bg-white/[0.02] border-b border-white/5">
              <th className="px-10 py-8 text-[10px] font-black uppercase tracking-[0.4em] text-muted-foreground/30 italic">Liquidity_Vol</th>
              ...
              <th className="px-10 py-8 text-[10px] font-black uppercase tracking-[0.4em] text-muted-foreground/30 italic text-right">Terminal</th>
            </tr>
          </thead>
            ...
            {filteredPayouts.length === 0 ? (
              <tr>
                <td colSpan={isStaff ? 6 : 5} className="py-32 text-center">
                   <div className="flex flex-col items-center gap-6 opacity-20 italic">
                      ...
                      <p className="text-[12px] font-black uppercase tracking-[0.4em]">Zero_Telemetry_Found</p>
                   </div>
                </td>
              </tr>
            ) : (
              filteredPayouts.map((payout, index) => (
                ...
                >
                  <td className="px-10 py-8">
                    <div className="flex flex-col gap-2">
                       <div className="flex items-center gap-3">
                          <ArrowDownLeft className="size-4 text-rose-500 opacity-40 shrink-0" />
                       ...
                       <span className="text-[9px] font-black text-muted-foreground/40 uppercase tracking-[0.2em] ml-7">
                  ...
                  <td className="px-10 py-8">
                    <div className="flex items-center gap-3">
                      <ShieldCheck className={cn("size-4 shrink-0 opacity-20", isStaff ? "text-amber-500" : "text-primary")} />
                      <span className="text-[10px] font-mono font-bold tracking-[0.2em] text-foreground/50 group-hover:text-foreground/80 transition-colors">
                        {payout.destination.slice(0, 8)}...{payout.destination.slice(-8)}
                      </span>
                    </div>
                  </td>
                  ...
                  <td className="px-10 py-8 text-right">
                    {payout.txId ? (
                      <Button 
                        ...
                        size="icon" 
                        className={cn(
                          "size-12 rounded-2xl border transition-all active:scale-90 group/link",
...
function getStatusIcon(status: string) {
  switch (status) {
    ...
    case "PROCESSING": return <ArrowUpRight className="size-3.5 animate-bounce" />;
...
function getStatusStyles(status: string) {
  switch (status) {
    ...
    case "PROCESSING": return "bg-blue-500/10 text-blue-500 border-blue-500/20 shadow-[0_0_15px_rgba(59,130,246,0.1)]";
    case "FAILED": return "bg-rose-500/10 text-rose-500 border-rose-500/20 shadow-[0_0_15px_rgba(244,63,94,0.1)]";

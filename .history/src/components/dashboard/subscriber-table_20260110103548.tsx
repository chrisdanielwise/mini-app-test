...
export function SubscriberTable() {
  ...
  const subscribers: Subscriber[] = [
    { id: "1", telegramName: "Alex_FX", telegramId: "7482910", plan: "PRO_SIGNAL", status: "ACTIVE", joinedAt: "2026-01-05", revenue: "$149.00" },
    { id: "2", telegramName: "Sam_Signals", telegramId: "9283741", plan: "STARTER_NODE", status: "EXPIRED", joinedAt: "2025-12-20", revenue: "$49.00" },
    { id: "3", telegramName: "CryptoWhale", telegramId: "1029384", plan: "ELITE_ACCESS", status: "ACTIVE", joinedAt: "2026-01-08", revenue: "$1,200.00" },
  ];
    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-1000">
      ...
      <div className="rounded-[3rem] border border-border/40 bg-card/30 overflow-hidden backdrop-blur-3xl shadow-2xl">
        ...
        <div className="hidden lg:block overflow-x-auto">
          <table className="w-full text-left border-collapse">
            <thead className="bg-muted/30 border-b border-border/40">
              <tr>
                {["Subscriber Node", "Protocol Type", "Identity ID", "Status", "LTV Flow", ""].map((header) => (
                  <th key={header} className="px-10 py-8 text-[9px] font-black uppercase tracking-[0.3em] text-muted-foreground/60">
                    {header}
                  </th>
                ))}
              </tr>
            ...
            <tbody className="divide-y divide-border/40">
              {subscribers.map((sub) => (
                <tr key={sub.id} className="hover:bg-primary/[0.03] transition-all group cursor-default">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className="space-y-1">
                        ...
                        <div className="flex items-center gap-1.5 opacity-40">
                           ...
                           <span className="text-[8px] font-bold uppercase tracking-widest">Joined: {sub.joinedAt}</span>
                        </div>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="px-10 py-8">
                    <Badge variant="outline" className={cn(
                      ...
                      {sub.status === "ACTIVE" && <ShieldCheck className="mr-1.5 h-3 w-3" />}
                      {sub.status}
                    </Badge>
                  </td>
        ...
        <div className="lg:hidden p-8 space-y-6">
          {subscribers.map((sub) => (
            <div key={sub.id} className="p-8 rounded-[2.5rem] bg-muted/10 border border-border/40 space-y-6 hover:bg-muted/20 transition-all">
              <div className="flex justify-between items-start">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-base font-black uppercase italic tracking-tighter leading-none">{sub.telegramName}</p>
                    <p className="text-[10px] font-black text-primary uppercase tracking-widest mt-2 opacity-60 italic">{sub.plan}</p>
                  </div>
                </div>
                <Badge variant="outline" className={cn(
              ...
              <div className="grid grid-cols-2 gap-4 pt-6 border-t border-border/40">
                <div className="space-y-1.5">
                   <p className="text-[8px] font-black uppercase tracking-[0.2em] text-muted-foreground opacity-40">Identity ID</p>
                   <p className="text-[11px] font-mono font-black tracking-widest">{sub.telegramId}</p>
                </div>
                <div className="text-right space-y-1.5">
                   <p className="text-[8px] font-black uppercase tracking-[0.2em] text-muted-foreground opacity-40">LTV Flow</p>
                   <p className="text-sm font-black italic tracking-tighter">{sub.revenue}</p>
                </div>
              </div>
              
              <Button variant="ghost" className="w-full h-12 rounded-2xl text-[9px] font-black uppercase tracking-widest border border-border/40">

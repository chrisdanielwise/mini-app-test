...
import {
  ...
  Terminal, Globe, Cpu, Fingerprint, ChevronDown, Layers
} from "lucide-react";
...
/**
 * 🛰️ CREATE_COUPON_MODAL (Institutional v16.16.42)
 * Strategy: Vertical Compression & Kinetic Deployment.
 * Feature: Restored Service Discovery & Multi-Tenant Targeting.
 */
export function CreateCouponModal({ merchantId }: { merchantId: string }) {
  ...
  // 📡 SERVICE DISCOVERY (Restored from Legacy)
  useEffect(() => {
      ...
      >
        <form action={formAction} className="flex flex-col h-full max-h-[90dvh]">
          ...
          <div className="flex-1 p-6 md:p-8 space-y-6 overflow-y-auto custom-scrollbar">
            {/* Protocol Code (Restored logic + updated UI) */}
            ...
            {/* Variable Grid (Financial Ingress) */}
            ...
            {/* Targeting Matrix (Restored Services Mapping) */}
            <div className="space-y-2">

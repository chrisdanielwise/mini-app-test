...
export function NavGuard() {
  ...
  const { flavor, mounted } = useLayout(); // Access 'mounted' from layout context
  
  const isStaffFlavor = flavor === "AMBER";

  /**
   * 🛡️ HYDRATION PLACEHOLDER
   * Instead of returning null immediately, we render a shimmering skeleton 
   * so the content doesn't "jump" up once hydration completes.
   */
  if (!mounted) {
    return (
  ...
  // 🛡️ Root-Level Protection: Logic remains for intended hidden routes

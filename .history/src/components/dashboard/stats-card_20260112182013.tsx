...
import { useLayout } from "@/providers/LayoutProvider"; // 🛡️ Path Synchronized
...
interface StatsCardProps {
  title: string;
  value: string | number | undefined | null; // 🛡️ Hardened for async data
  change?: number;
  changeLabel?: string;
...
/**
 * 📊 KPI TELEMETRY NODE (Institutional v9.7.3)
 * Hardened: Null-safe value parsing prevents .toString() runtime crashes.
 * Optimized: Staggered animation ingress for Turbopack performance.
 */
const StatsCardComponent = ({
  ...
  // 🛡️ NULL-SAFE HANDSHAKE: Prevents "Cannot read properties of undefined"
  // Converts any incoming data (null, undefined, number, string) into a stable string.
  ...
  // 🛡️ HYDRATION GUARD
  if (!mounted) return (
    <div className="min-h-[160px] md:min-h-[180px] rounded-[2rem] bg-card/20 border border-border/10 animate-pulse" />
  );
      ...
      <div className="relative z-10 w-full min-w-0 p-6">
        <div className="flex items-center justify-between mb-4 md:mb-6">
          <div className="space-y-1 min-w-0">
             <p className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.3em] text-muted-foreground opacity-60 truncate">
               {title}
             </p>
             <div className="flex items-center gap-1.5 opacity-30 md:group-hover:opacity-100 transition-opacity duration-500 italic">
               {isStaff ? (
        ...
        <div className="flex flex-col gap-2 md:gap-3 min-w-0">
          ...
          {change !== undefined && (
            <div className="flex items-center gap-2 md:gap-2.5 flex-wrap">
              <div
                className={cn(
                  "flex items-center rounded-lg px-2 py-0.5 md:px-2.5 md:py-1 text-[8px] md:text-[9px] font-black uppercase tracking-widest border",
              ...
              <span className="text-[8px] md:text-[9px] font-bold uppercase tracking-widest text-muted-foreground opacity-30 italic truncate max-w-[120px]">
                {changeLabel}
              </span>
            </div>
          )}

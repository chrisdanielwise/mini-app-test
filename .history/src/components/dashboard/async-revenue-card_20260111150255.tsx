...
import { getSession } from "@/lib/auth/session"; // 🛡️ Added for Identity Handshake
import { AlertCircle, Terminal, TrendingUp, Activity, Zap, Globe, Lock } from "lucide-react";
import { cn } from "@/lib/utils";

/**
 * 💰 ASYNC REVENUE NODE
 * Logic: Role-Aware financial telemetry. 
 * Handshake: Resolves identity from JWT to prevent cross-tenant data leakage.
 */
export async function AsyncRevenueCard({
  ...
  // 🛰️ 3. NODE RESOLUTION
  // If user is Staff and no prop is passed -> Global Oversight
  // If user is Merchant -> Force their own merchantId (ignore props for safety)

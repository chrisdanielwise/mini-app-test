...
/**
 * 🛰️ DASHBOARD_SHELL (TMA Final Fix v2026)
 * Strategy: Flex-Column Stack & Hardware-Safe Buffering.
 * This physically PUSHES the header down below the notch.
 */
export function DashboardShell({ children }: { children: React.ReactNode }) {
  ...
  useLayoutEffect(() => {
    if (typeof window !== "undefined" && WebApp && isReady) {
      try {
        WebApp.ready();
        WebApp.expand();
        // 🛡️ HARDWARE LOCK: Prevents the app from bouncing or sliding away
  ...
  return (
     ...
     * This container is locked to the physical screen size.
     */
    <div
      className={cn(
    ...
    >
      {/* 📐 TOP_BUFFER: This is the physical wall that pushes the header down */}
      {/* We use a hard-coded 44px fallback for devices where the safeArea fails to report. */}
      <div 
        style={{ height: `${safeArea.top || 44}px` }} 
        className="shrink-0 w-full bg-black z-[60]" 
      />

      {/* 🚀 LAYER 1: STATIONARY_HUD_HEADER
          Now part of the Flex flow: It MUST sit below the buffer. */}
      <header className="shrink-0 py-4 flex flex-col px-4 bg-black/40 backdrop-blur-3xl border-b border-white/5 z-50">
      ...
      {/* 🛰️ LAYER 3: STATIONARY_HUD_NAV
          Fixed to the hardware bottom above the OS home indicator. */}
      <nav 

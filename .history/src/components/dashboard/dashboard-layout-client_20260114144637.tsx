...
/**
 * 🛰️ DASHBOARD CLIENT ORCHESTRATOR (v16.1.5)
 * Logic: Synchronized Fixed-to-Flow Architecture with Emergency Awareness.
 * Feature: Reactive SystemConfig injection for NavGuard broadcasts.
 */
export function DashboardLayoutClient({ 
  ...
  // 🛡️ SYSTEM SYNC: Hydrate Broadcast & Maintenance states
  useEffect(() => {
    if (!mounted) return;
    ...
    const syncSystemNode = async () => {
      try {
        ...
        // We capture the system config from the profile egress
        if (data.systemConfig) {
          setSystemConfig(data.systemConfig);
        }
      } catch (err) {
        console.warn("⚠️ [System_Sync_Missed]: Broadcast node unreachable.");
      }
    };
    ...
    >
      ...
      <header className="shrink-0 pt-1.5 md:pt-0 z-50">
        <NavGuard 
          ...
          systemConfig={systemConfig} // 🚀 Pass broadcast data to NavGuard
        />
      </header>

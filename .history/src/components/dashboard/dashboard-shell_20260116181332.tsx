...
/**
 * 🛰️ DASHBOARD_SHELL (TMA Standard v2026.1.20)
 * Strategy: Flex-Column Stack & Hardware-Safe Buffering.
 * Fix: Physically pushes the "Merchant Terminal" block down by over an inch.
 */
export function DashboardShell({ children }: { children: React.ReactNode }) {
  ...
  // 🕵️ DEBUG LOGGING: Use Eruda to see if safeArea is actually reporting numbers
  useLayoutEffect(() => {
    if (typeof window !== "undefined") {
      const tg = (window as any).Telegram?.WebApp;
      console.log("--- 🛰️ HARDWARE TELEMETRY ---");
      console.table({
        "Safe Top (Notch)": tg?.safeAreaInset?.top || 0,
        "Safe Bottom": tg?.safeAreaInset?.bottom || 0,
        "Viewport Height": tg?.viewportHeight,
        "Window InnerHeight": window.innerHeight,
        "Context SafeTop": safeArea?.top
      });
    }
  }, [isReady, safeArea]);
  ...
  useLayoutEffect(() => {
    if (typeof window !== "undefined" && WebApp && isReady) {
      try {
        WebApp.ready();
        WebApp.expand();
        // 🛡️ BLOCK NATIVE SWIPES: Essential to prevent the "sliding gap"
  ...
  return (
     ...
     * flex-col ensures children (Buffer, Header, Main) stack vertically.
     */
    <div
      className={cn(
    ...
    >
      {/* 📐 TOP_BUFFER: The Physical Wall
          Strategy: We use a hard-coded 64px displacement to force the view down. 
          This occupies the notch space so Layer 1 MUST start below it. */}
      <div 
        style={{ height: `${Math.max(safeArea?.top || 0, 64)}px` }} 

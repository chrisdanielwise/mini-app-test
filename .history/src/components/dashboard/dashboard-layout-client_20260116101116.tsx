 ...
 * Fix: Added min-w-0 to BOTH the wrapper and main tag. This is the only way 
 * to stop tables from expanding containers horizontally and cropping.
 */
export function DashboardLayoutClient({ 
  ...
  return (
    <div 
      key={userRole} 
      // 🛡️ FIX: min-w-0 on the wrapper prevents the entire dashboard from expanding horizontally
      className="flex flex-1 flex-col relative min-w-0 bg-black selection:bg-primary/20 text-foreground overflow-hidden"
    >
      {/* 🚀 FIXED HUD */}
      ...
      <main 
        className={cn(
          // 🛡️ FIX: Added min-w-0 and min-h-0. This forces the container to respect 
          // parent boundaries instead of expanding to fit long table names.
          "flex-1 min-w-0 min-h-0 overflow-y-auto overflow-x-hidden transition-all duration-700 ease-[cubic-bezier(0.2,0.8,0.2,1)]",
      ...
      >
        {/* 🛡️ FIX: This inner wrapper ensures the child content (tables/headers) calculates width correctly */}
        <div className="relative min-h-full w-full min-w-0">
          {!mounted ? (
            ...
            // 🛡️ FIX: min-w-0 here allows headers like "SERVICE ASSETS" to truncate/shrink
            <div className="animate-in fade-in slide-in-from-bottom-2 duration-700 min-w-0">
              {children}
            </div>
          )}
        </div>

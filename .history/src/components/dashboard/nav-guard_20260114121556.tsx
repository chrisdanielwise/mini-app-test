...
import { ArrowLeft, X, Terminal, Globe } from "lucide-react";
...
/**
 * 🛰️ GLOBAL NAV GUARD (v15.9.6)
 * Feature: Live Security Heartbeat Indicator.
 * Logic: Displays a pulsing status dot when the session is verified and anchored.
 */
export function NavGuard({ heartbeatStatus }: { heartbeatStatus?: HeartbeatStatus }) {
  ...
  // 🛡️ HYDRATION PLACEHOLDER: Prevents layout jump
  if (!mounted) {
    return (
      ...
      <div className="max-w-[1440px] mx-auto flex h-12 md:h-14 items-center justify-between px-4 md:px-6">
        ...
        <div className="flex items-center gap-2 md:gap-3 min-w-0 px-2">
          <div className="relative flex items-center justify-center">
            {/* 🚀 THE HEARTBEAT DOT: Indicates active secure session */}
            <div className={cn(
            ...
            <div className={cn(
              ...
              isStaffFlavor ? "bg-amber-500/80 shadow-[0_0_8px_rgba(245,158,11,0.5)]" : "bg-primary/80 shadow-[0_0_8px_rgba(16,185,129,0.5)]"
            )} />
          </div>

          {/* 🚀 ADD YOUR NEW DIV CODE HERE */}
            <div className={cn(

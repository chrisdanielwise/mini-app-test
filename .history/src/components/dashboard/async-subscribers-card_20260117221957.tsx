...
export default async function AsyncSubscribersCard({
  ...
  try {
    ...
    /** 🏁 PROTOCOL RACE: 30s Database Latency Guard */
    ...
    if (type === "chart") {
      if (!growth) {
        return (
          <div className="flex h-[280px] flex-col items-center justify-center rounded-[2.5rem] border border-white/5 bg-card/20 backdrop-blur-3xl">
            ...
            <p className="mt-4 text-[8px] font-black uppercase tracking-[0.3em] opacity-20 italic">Syncing_Growth_Node...</p>
          </div>
        );
      }
      ...
      return (
        <div className={cn(
          "rounded-[2.5rem] md:rounded-[3rem] border p-6 md:p-7 shadow-2xl backdrop-blur-3xl animate-in fade-in zoom-in-95 duration-1000 relative group overflow-hidden",
          ...
          <div className="flex flex-row items-center justify-between mb-6 relative z-10">
            <div className="space-y-1.5 leading-none">
              <div className="flex items-center gap-2 italic opacity-30">
                {isGlobalOversight ? <Globe className="h-2.5 w-2.5 text-amber-500 animate-pulse" /> : <Users className="h-2.5 w-2.5 text-primary" />}
                <h3 className={cn(
                  "text-[7.5px] font-black uppercase tracking-[0.4em]",
                  isGlobalOversight ? "text-amber-500" : "text-primary"
                )}>
                  {isGlobalOversight ? "Platform_Global_Sync" : "Expansion_Vector"}
                </h3>
              </div>
            ...
            <div className={cn(
              "size-9 md:size-10 rounded-xl border flex items-center justify-center shrink-0 shadow-inner",
    ...
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full animate-in slide-in-from-bottom-4 duration-700">
        ...
        <StatsCard
          ...
          className={cn(
            ticketCount > 0 && (isGlobalOversight ? "border-amber-500/30 shadow-apex-amber" : "border-orange-500/30 shadow-apex-orange")
          )}
        />
      </div>
    );
      ...
      <div className="flex flex-col h-full min-h-[200px] items-center justify-center rounded-[2.5rem] border border-destructive/20 bg-destructive/5 p-6 text-center backdrop-blur-3xl animate-in fade-in duration-500">
        ...
        <div className="mt-6 flex items-center gap-2 px-3 py-2 rounded-xl bg-card/40 border border-white/5">
          ...
          <p className="text-[7px] font-black uppercase text-muted-foreground/40 tracking-[0.2em] italic">
            {error.message === "DATABASE_LATENCY_TIMEOUT" ? "Neon_Provisioning_Wait" : "Cluster_Sync_Error"}
          </p>
        </div>
      </div>
    );

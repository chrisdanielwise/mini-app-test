...
export function DashboardShell({ children }: { children: React.ReactNode }) {
  ...
  useLayoutEffect(() => {
    if (typeof window !== "undefined" && WebApp && isReady) {
      WebApp.ready();
      WebApp.expand();
      // 🛡️ HARDWARE LOCK: Prevents the "Sliding" energy
  ...
  return (
    ...
    <div
      ...
      style={{ height: `${viewportHeight}px`, position: 'fixed', top: 0, left: 0 }}
    >
      {/* 🚀 LAYER 1: THE TACTICAL_HUD_HEADER
          STRATEGY: Using a 'Margin' to physically push the box out of the notch. */}
      <header 
        className="fixed top-0 left-0 w-full z-[100] bg-black/60 backdrop-blur-3xl border-b border-white/5"
        style={{ 
          marginTop: `${safeArea.top}px`, // 🛰️ FORCED MARGIN LOCK
          height: "3.5rem" 
        }}
      >
      ...
      <nav 
        className="fixed bottom-0 left-0 w-full z-[100] bg-black/60 backdrop-blur-3xl border-t border-white/5"

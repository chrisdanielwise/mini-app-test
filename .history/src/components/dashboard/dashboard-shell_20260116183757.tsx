...
export function DashboardShell({ children }: { children: React.ReactNode }) {
  ...
  useLayoutEffect(() => {
    // 🏁 POST-HYDRATION TELEMETRY: Confirms the 27px ingress
    if (typeof window !== "undefined") {
      const tg = (window as any).Telegram?.WebApp;
      console.log("--- 🏁 HARDWARE_SYNC_COMPLETE ---");
      console.log("Detected Notch:", tg?.safeAreaInset?.top, "px");
      console.log("Detected Home_Indicator:", tg?.safeAreaInset?.bottom, "px");
    }
    ...
    if (typeof window !== "undefined" && WebApp && isReady) {
      try {
        ...
        // 🛡️ LOCK_PROTOCOL: Kills the elastic "shaking" jitter
  ...
  return (
     ...
     * Anchored to the 804px viewport height reported in your logs.
     */
    <div
      className={cn(
    ...
    >
      {/* 📐 LAYER 1: DYNAMIC_TOP_BUFFER
          Strategy: Uses the 27px safeArea reported by the hardware.
          If hydration is pending, it defaults to a 24px safety wall. */}
      <div 
        style={{ height: `${safeArea?.top > 0 ? safeArea.top : 24}px` }} 
        className="shrink-0 w-full bg-black z-[100]" 
      />

      {/* 🚀 LAYER 2: STATIONARY_HUD_HEADER
          Part of the Flex flow: Physically impossible to overlap the notch. */}
      <header className="shrink-0 pt-1 pb-4 flex flex-col px-4 bg-black/40 backdrop-blur-3xl border-b border-white/5 z-50">
      ...
      {/* 🌊 LAYER 3: LAMINAR_CONTENT_ENGINE
          The only volume that scrolls. Anchored between header and nav. */}
      <main
        id="main-scroll-root"
      ...
      >
        ...
        {/* 📐 BOTTOM_SPACER: Clears the Nav Bar + 15px Bottom Safe Area */}
        <div style={{ height: `calc(${safeArea?.bottom || 15}px + 100px)` }} className="shrink-0" />
      </main>

      {/* 🛰️ LAYER 4: STATIONARY_HUD_NAV
          Fixed to the physical bottom 15px safe zone. */}
      <nav 
        className="shrink-0 bg-black/40 backdrop-blur-3xl border-t border-white/5 z-50"
        style={{ paddingBottom: `${safeArea?.bottom || 15}px` }}
      >

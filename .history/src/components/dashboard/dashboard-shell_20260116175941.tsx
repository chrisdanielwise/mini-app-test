...
/**
 * 🛰️ DASHBOARD_SHELL (The Industry Standard)
...
export function DashboardShell({ children }: { children: React.ReactNode }) {
  ...
  React.useLayoutEffect(() => {
    // 🕵️ DEBUG LOGGING: This prints the data directly to Eruda
    if (typeof window !== "undefined") {
      ...
      console.table({
        ...
        "Context SafeTop": safeArea.top // Check if your provider matches Telegram
      });
    }
  }, [isReady, safeArea]); // Runs whenever the device state updates
  ...
  return (
    ...
    >
      {/* 📐 TOP_BUFFER: Physically occupies the notch space */}
      {/* 🚀 FORCE_DISPLACEMENT: We use 64px here. This MUST move the header down. */}
      <div 
        style={{ height: `${Math.max(safeArea.top, 64)}px` }} 
        className="shrink-0 w-full bg-black z-[60] border-b border-white/[0.02]" 
      />

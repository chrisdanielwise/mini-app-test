 ...
 * Integration: Merged Legacy Mapping with Hardened 2026 Viewport.
 */
export function SessionActivityTable() {
  ...
  const fetchLogs = useCallback(async (isManual = false) => {
    if (isManual) impact("medium"); // Physical handshake for manual refresh
    setIsLoading(true);
    ...
    try {
      ...
      notification("error"); // Feedback for failed sync
    } finally {
      setIsLoading(false);
    }
  }, [impact, notification]);
  ...
  if (!isReady) return (
    <div className="h-64 w-full bg-white/[0.02] border border-white/5 animate-pulse rounded-[2rem]" />
  );
  ...
  return (
    <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="flex items-center justify-between px-2">
        <div className="flex items-center gap-3 italic opacity-30 leading-none">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[6px] font-black uppercase tracking-widest mt-1 opacity-50">Node_v16.STABLE</span>
          </div>
        </div>
      ...
      <div className={cn(
          ...
          <table className="w-full text-left border-collapse table-fixed min-w-[650px]">
            <thead>
              <tr className="bg-white/[0.01] border-b border-white/5">
                <th className="w-[35%] px-6 py-3 text-[7px] font-black uppercase tracking-[0.4em] text-muted-foreground/30 italic">Source_Vector</th>
                <th className="w-[40%] px-6 py-3 text-[7px] font-black uppercase tracking-[0.4em] text-muted-foreground/30 italic">Edge_Protocol</th>
                <th className="w-[25%] px-6 py-3 text-[7px] font-black uppercase tracking-[0.4em] text-muted-foreground/30 italic text-right">Epoch_Sync</th>
              </tr>
            </thead>
              ...
              {isLoading ? (
                [...Array(3)].map((_, i) => (
                  <tr key={i} className="animate-pulse">
                    <td colSpan={3} className="px-6 py-6 h-16 bg-white/[0.01]" />
                  </tr>
                ))
              ) : activities.length > 0 ? (
                  ...
                  >
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-4">
                        <div className={cn(
                          "size-9 shrink-0 rounded-xl border flex items-center justify-center transition-all shadow-inner bg-background",
                        ...
                        <div className="flex flex-col min-w-0 leading-tight">
                          ...
                          <div className="flex items-center gap-1.5 mt-1.5 opacity-20">
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex flex-col items-end gap-1 leading-none">
                        ...
                        <span className="text-[6.5px] font-mono font-black opacity-10 uppercase">
                ...
                <tr>
                  <td colSpan={3} className="py-20 text-center">
                    <div className="flex flex-col items-center justify-center gap-4 opacity-10 italic">
                      <ShieldAlert className="size-10" />
                      <p className="text-[10px] font-black uppercase tracking-[0.4em]">Zero_Ingress_Signal</p>
                    </div>
                  </td>
                </tr>
              )}
        ...
        {/* --- TACTICAL FOOTER --- */}
        <div className="px-6 py-3 bg-white/[0.01] border-t border-white/5 flex items-center justify-between opacity-10 italic">
          <p className="text-[6.5px] font-black uppercase tracking-[0.3em]">Integrity: VERIFIED</p>
          <span className="text-[6px] font-mono">[AES_256_GCM_ENCRYPTED]</span>
        </div>
      </div>
    </div>
  );

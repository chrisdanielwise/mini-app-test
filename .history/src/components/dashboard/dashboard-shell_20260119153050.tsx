...
// ✅ FIX: Strict guard for WebApp availability via global check
...
export function DashboardShell({ children }: { children: React.ReactNode }) {
  ...
  useLayoutEffect(() => {
    // ✅ FIX: Resolve TS2339/TS18048 by safely accessing the global Telegram object
    ...
    if (tg && isReady) {
      try {
        ...
        const syncMobileViewport = () => {
          // ✅ FIX: Safe-fallback to innerHeight for desktop/debug modes
        ...
        // ✅ FIX: Defensive event registration for API 6.0+
        if (typeof tg.onEvent === 'function') {
          tg.onEvent("viewportChanged", syncMobileViewport);
        }
        
        // 🛡️ API 8.0: Hardware Lock for vertical scrolling
        ...
        console.warn("🛰️ [Shell_Sync_Fault]: Handshake failed.", error);
      }
    } else if (isReady) {

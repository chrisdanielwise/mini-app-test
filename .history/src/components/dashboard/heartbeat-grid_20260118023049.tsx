 ...
 * Mission: Real-time oversight for the 30-table Zipha ecosystem.
 */
export function HeartbeatGrid({ services }: { services: ServiceNode[] }) {
  ...
  if (!isReady) return (
    <div className="h-64 w-full bg-zinc-900/10 animate-pulse rounded-2xl border border-white/5" />
  );

  // 🕵️ TOPOLOGY MAPPING: Adjusts columns based on physical hardware width
  const gridCols = {
    xs: "grid-cols-2",
    sm: "grid-cols-3",
    md: "grid-cols-4",
  ...
  return (
    <div className={cn(
      ...
      "rounded-[2rem] border backdrop-blur-3xl overflow-hidden",
      ...
      <div className={cn("grid gap-3 relative z-10", gridCols)}>
        {services.map((node, index) => (
          <div
            ...
            className={cn(
              "group relative flex flex-col items-center justify-center py-4 rounded-2xl border transition-all duration-500",
          ...
          >
            {/* 💓 STATUS PULSE: Morphological Indicator */}
      ...
      {/* --- FOOTER SIGNAL: Global Ingress Status --- */}
      <div className="mt-8 flex items-center justify-between opacity-10 italic border-t border-white/5 pt-5 relative z-10">
        <div className="flex items-center gap-3">
          ...
          <span className="text-[7.5px] font-black uppercase tracking-[0.4em] text-foreground">
...
/** 🛠️ ATOMIC: LEGEND_NODE (Institutional Legend) */

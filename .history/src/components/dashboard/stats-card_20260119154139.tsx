...
/**
 * 🛰️ STATS_CARD_SKELETON (Institutional Ingress)
 * Fix: Exported to resolve TS2305 in loading.tsx.
 */
export function StatsCardSkeleton() {
  return (
    <div className="h-[200px] w-full rounded-[2.5rem] bg-white/[0.02] border border-white/5 animate-pulse flex flex-col p-6 space-y-4">
      <div className="flex justify-between">
        <div className="h-3 w-24 bg-white/5 rounded" />
        <div className="size-10 bg-white/5 rounded-xl" />
      </div>
      <div className="h-10 w-32 bg-white/5 rounded-lg" />
      <div className="h-3 w-20 bg-white/5 rounded mt-auto" />
    </div>
  );
}
...
const StatsCardComponent = ({
  ...
  // 🛡️ HYDRATION_SHIELD
  if (!isReady) return <StatsCardSkeleton />;
...
// ✅ EXPORT SYNC: Exporting both named components

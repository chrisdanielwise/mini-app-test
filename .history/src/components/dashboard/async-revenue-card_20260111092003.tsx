...
import { AlertCircle, Terminal, TrendingUp, DollarSign, Activity, Zap, Globe } from "lucide-react";
import { cn } from "@/lib/utils";

/**
 * 💰 ASYNC REVENUE NODE (Apex Tier)
 * RBAC Fix: merchantId is now optional to allow Global Platform Oversight.
 */
export async function AsyncRevenueCard({
  merchantId,
  type = "stats",
}: {
  merchantId?: string; // ✅ Now optional for Staff roles
  type?: "stats" | "chart";
}) {
  try {
     ...
     * Logic: If merchantId is missing, AnalyticsService must fetch ALL revenue.
     */
    ...
    if (type === "chart") {
      ...
      return (
        <div className={cn(
          ...
          isGlobalOversight ? "bg-amber-500/[0.03] border-amber-500/10" : "bg-card/40 border-border/10"
        )}>
           ...
           <div className="flex flex-row items-center justify-between mb-8 px-1 md:px-2 relative z-10">
            <div className="space-y-1.5">
              <div className="flex items-center gap-2 italic opacity-40">
                {isGlobalOversight ? <Globe className="h-3 w-3 text-amber-500" /> : <Zap className="h-3 w-3 text-primary animate-pulse" />}
                <h3 className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.4em]">
                  {isGlobalOversight ? "Global_Financial_Vector" : "Cluster_Financial_Vector"}
                </h3>
              </div>
              <p className="text-xl sm:text-2xl md:text-3xl font-black uppercase italic tracking-tighter leading-none">
                {isGlobalOversight ? "Platform" : "Revenue"} <span className={isGlobalOversight ? "text-amber-500" : "text-primary"}>Trend</span>
              </p>
            </div>
            <div className={cn(
              "h-10 w-10 md:h-12 md:w-12 rounded-xl border flex items-center justify-center shrink-0 shadow-inner",
              isGlobalOversight ? "bg-amber-500/10 border-amber-500/20 text-amber-500" : "bg-primary/10 border-primary/20 text-primary"
            )}>
    ...
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6 w-full animate-in slide-in-from-bottom-4 duration-700">
        <StatsCard
          title={isGlobalOversight ? "Total Platform Gross" : "Gross Revenue"}
        ...
        />
        <StatsCard
          title={isGlobalOversight ? "Global Tx Count" : "Transaction Volume"}
      ...
      <div className="flex flex-col h-full min-h-[200px] md:min-h-[280px] items-center justify-center rounded-[2.5rem] border border-destructive/20 bg-destructive/5 p-6 md:p-8 text-center backdrop-blur-3xl shadow-inner relative overflow-hidden">
        ...
        <div className="mt-6 flex items-center gap-2 px-3 py-2 rounded-xl bg-card/40 border border-border/10 max-w-full shadow-sm">
          ...
          <p className="text-[7px] md:text-[8px] font-black uppercase text-muted-foreground tracking-[0.2em] truncate">
            {error.message === "REVENUE_FETCH_TIMEOUT" ? "Node_Oversight_Timeout" : "Cluster_Sync_Error"}
          </p>
        </div>

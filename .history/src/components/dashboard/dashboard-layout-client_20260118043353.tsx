...
/**
 * 🛰️ DASHBOARD_LAYOUT_CLIENT (Institutional v16.16.90)
 * Strategy: Viewport-Locked Shell & Tactical Identity Orchestration.
 * Mission: Coordinate HUD, Admin Tunneling, and Financial Settlement Ingress.
 */
export function DashboardLayoutClient({ 
  ...
  /**
   * 🛡️ SETTLEMENT_TRIGGER_PROTOCOL
   * Logic: Tactical Ingress Handshake with selection haptic feedback.
   */
  const toggleSettlement = useCallback(() => {
    selectionChange(); // 🏎️ Physical feedback for portal entry
    setIsSettlementOpen((prev) => !prev);
  }, [selectionChange]);
  ...
  const isPlatformStaff = useMemo(() => 
    ["super_admin", "platform_manager", "platform_support", "amber"].includes(userRole?.toLowerCase() || ""),
    [userRole]
  );

  return (
    // 🏛️ TIER 1: THE SHELL (Viewport Locked at 100dvh)
    <div className="flex flex-col h-[100dvh] w-full bg-black overflow-hidden relative selection:bg-primary/30">
      
      {/* 🛡️ TACTICAL OVERLAY: Active Identity Swap Sentinel */}
      {mounted && (
        <StaffIdentitySwap 
      ...
      {/* 🚀 SETTLEMENT_PORTAL: Portal-mounted decision layer (Z-Index 150) */}
      {mounted && (
      ...
      {/* 🛰️ STATIONARY HORIZON: HUD Navigation Anchor (Z-Index 50) */}
      <header className={cn(
        "shrink-0 z-50 transition-all duration-500",
        impersonationData ? "pt-10" : "pt-0" // 📐 Clearance for Identity Swap Banner
      )}>
        <NavGuard 

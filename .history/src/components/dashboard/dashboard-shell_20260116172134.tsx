...
export function DashboardShell({ children }: { children: React.ReactNode }) {
  ...
  useLayoutEffect(() => {
    if (typeof window !== "undefined" && WebApp && isReady) {
      ...
      // 🛡️ HARDWARE LOCK: Disables the native pull-to-refresh and elastic bounce
  ...
  return (
    <div
      className={cn(
        ...
        // 🚀 PREVENT ELASTIC BOUNCE: Root container must be untouchable
    ...
    >
      {/* 🚀 LAMINAR CONTENT ENGINE: Isolated Scrollable Volume */}
      <main
        className={cn(
          "h-full w-full overflow-y-auto overflow-x-hidden relative z-10",
          "scrollbar-none touch-pan-y", // Only allow vertical movement inside this tag
          "flex flex-col"
        )}
        style={{
          // 📏 SURGICAL SAFE-AREA CLAMPING: Maps to hardware pixels
          paddingTop: `${safeArea.top}px`, 
          // 🛰️ DYNAMIC BUFFER: Clears your Bottom Nav + Hardware Home Indicator
          paddingBottom: `calc(${safeArea.bottom}px + 100px)`, 
          // 🛡️ PREVENT BOTTOM SPACE: Force content to take at least full height
          minHeight: '100%' 
        }}
      >
        <div className="w-full max-w-[1600px] mx-auto flex flex-col p-4 flex-1">
          {children}
        </div>
      </main>
    </div>

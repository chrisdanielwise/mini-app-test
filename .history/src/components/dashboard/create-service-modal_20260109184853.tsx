...
// ✅ FIXED: Using exact hyphenated path for actions
 ...
 * Optimized for hydration safety and Schema V2 compliance.
 */
export function CreateServiceModal({ merchantId }: CreateServiceModalProps) {
  // 🛡️ HYDRATION GUARD: Prevents ID mismatch between Server and Client
  ...
  // 🛡️ Safe-Return: Prevents the trigger from rendering until client-side IDs are stable
  if (!mounted) return null;

  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        {/* ✅ FIXED: Explicit type="button" prevents interference with form logic */}
        <Button 
          type="button"
          className="rounded-xl h-12 px-6 font-black uppercase italic tracking-widest shadow-lg bg-primary text-primary-foreground text-[10px] active:scale-95 transition-all"
        >
      ...
      <DialogContent 
        ...
        className="w-[95vw] max-w-md rounded-[2.5rem] border-border bg-card p-0 overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300"
      >

 ...
 * Strategy: Viewport-Locked Shell & Hardware Ingress Sync.
 * Fix: Resolved "Competing Geometry" by centralizing safe area logic.
 */
export function DashboardLayoutClient({ 
  ...
  return (
    // 🏛️ TIER 1: THE PRIMARY SHELL (Locked at 100dvh for Telegram Safety)
    <div className="flex flex-col h-[100dvh] w-full bg-black overflow-hidden relative selection:bg-primary/30">
      ...
      {/* 🛰️ STATIONARY HUD: Hardware Notch Protection 
          🏁 THE FIX: This is the ONLY place safeArea.top is applied.
      */}
      <header 
      ...
      {/* 🌊 TIER 2: LAMINAR RESERVOIR (Independent Volume) 
          🏁 THE FIX: Flex-1 ensures this area fills the remaining gap exactly.
      */}
      <main 
        className={cn(
        ...
        <div className="relative min-h-full w-full">
          {!mounted || !isReady ? (
            ...
            <div 
              ...
              style={{ 
                // 🏁 THE FIX: Standardized 7.5rem clearance for the Floating BottomNav
                paddingBottom: isMobile 
                  ? `calc(${safeArea.bottom}px + 7.5rem)` 
                  : "4rem" 
              }}
            >
              {children}
      ...
      {/* 🕹️ TIER 3: COMMAND HUB (Stationary Bottom HUD) 
          🏁 THE FIX: Positioned fixed to float ABOVE the main content.
      */}
      {mounted && isMobile && (
        <div 
          className="fixed bottom-0 left-0 right-0 z-[70] animate-in slide-in-from-bottom duration-500 px-4"

 ...
 * Fix: High-density rows (py-4) and stationary header profile prevents blowout.
 */
    ...
    <div className={cn(
      ...
      <div className="flex flex-row justify-between items-start mb-6 md:mb-8 relative z-10 leading-none">
        <div className="space-y-2 min-w-0">
          <div className="flex items-center gap-2.5 italic opacity-30">
            ...
            <div className="flex flex-col">
              <h3 className={cn(
                "text-[7.5px] font-black uppercase tracking-[0.3em] truncate",
                isStaff ? "text-amber-500" : "text-foreground"
              )}>
                {isStaff ? "Expansion_Vector" : "Growth_Telemetry"}
              </h3>
            </div>
          </div>
        ...
        <div className="shrink-0 text-right p-3 rounded-xl bg-white/[0.01] border border-white/5">
          <div className={cn("flex items-center gap-1.5 opacity-30 mb-1 justify-end", isStaff ? "text-amber-500" : "text-emerald-500")}>
            ...
            <span className="text-[6.5px] font-black uppercase tracking-[0.2em]">Net_New</span>
          </div>
          <p className={cn(
      ...
      <div className="flex-1 w-full relative z-10 min-h-0">
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            <Bar dataKey="new" name="Acquired" fill={primaryBarColor} radius={[2, 2, 0, 0]} barSize={isMobile ? 6 : 8} />
            <Bar dataKey="churned" name="Churned" fill={churnBarColor} radius={[2, 2, 0, 0]} barSize={isMobile ? 6 : 8} opacity={0.2} />
          </BarChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="mt-6 flex items-center justify-between opacity-10 italic relative z-10 border-t border-white/5 pt-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[7px] font-black uppercase tracking-[0.3em]">{isStaff ? "Expansion_Active" : "Growth_Sync_Ok"}</span>
        </div>
        <div className="flex items-center gap-2">

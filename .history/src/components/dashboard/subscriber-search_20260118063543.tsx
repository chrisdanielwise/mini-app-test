 ...
 * Integration: Merged Legacy Debounce with Hardened 2026 Chassis.
 */
export function SubscriberSearch() {
  ...
  if (!isReady) return (
    <div className="h-11 w-full lg:max-w-xs bg-white/[0.02] border border-white/5 animate-pulse rounded-xl" />
  );

  return (
    <div className={cn(
      "relative w-full lg:max-w-sm group transition-all duration-700 px-1",
      "animate-in fade-in slide-in-from-left-4"
    )}>
      {/* 🌫️ TACTICAL RADIANCE: Ambient Focus Signal */}
      <div className={cn(
      ...
      {/* --- 🛡️ INGRESS NODE: Status Indicator --- */}
      ...
      {/* --- TERMINAL ACTIONS: High-Friction Clear --- */}
      <div className="absolute right-4 top-1/2 -translate-y-1/2 flex items-center gap-3 z-10">
        {searchTerm && (
          <button 
            onClick={handleClear}
            className="size-7 rounded-lg bg-white/5 hover:bg-rose-500/10 hover:text-rose-500 transition-all flex items-center justify-center border border-white/5 active:scale-90"
          >
            <X className="size-3" />
          </button>
        )}
        <Terminal className={cn(
          "size-3 opacity-10 transition-colors hidden sm:block",
          isStaffFlavor ? "group-focus-within:text-amber-500" : "group-focus-within:text-primary"
        )} />
      </div>

      {/* --- STATUS TELEMETRY: Ingress feedback --- */}
      <div className="absolute -bottom-6 left-2 opacity-0 group-focus-within:opacity-20 transition-all pointer-events-none">
         <div className="flex items-center gap-2 italic">
           ...
           <p className="text-[7px] font-black uppercase tracking-[0.3em] text-foreground">
             {isPending ? "Indexing Subscriber Nodes..." : "Node_Active"}
           </p>
         </div>
      </div>
    </div>

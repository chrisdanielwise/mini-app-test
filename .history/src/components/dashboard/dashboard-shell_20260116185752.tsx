...
export function DashboardShell({ children }: { children: React.ReactNode }) {
  ...
  // We are going to force a 45px gap. This is undeniable.
  const forcedGap = 45; 

  return (
    <div
      className="fixed inset-0 w-full bg-black overflow-hidden touch-none overscroll-none"
      style={{ height: `${viewportHeight}px`, position: 'fixed', top: 0, left: 0, zIndex: 10 }}
    >
      {/* 🚀 THE HEADER: FORCED POSITIONING */}
      <header 
        className="w-full flex flex-col px-4 bg-black/60 backdrop-blur-xl border-b border-white/5"
        style={{ 
          position: 'absolute',
          top: `${forcedGap}px`, // ⚡ HARD DISPLACEMENT
          left: 0,
          right: 0,
          height: '70px',
          zIndex: 100 
      ...
      >
        <div className="flex items-center gap-2 opacity-30 italic mt-2">
          <Terminal size={10} />
          <span className="text-[7px] font-black uppercase tracking-[0.4em]">Mesh_Ingress_v16.31</span>
        </div>
        <h1 className="text-xl font-black uppercase tracking-[0.2em] text-primary italic leading-tight">
          Merchant Terminal
        </h1>
        <p className="text-[8px] opacity-50 font-mono">NODE_ID: 09C55E7C</p>
      </header>

      {/* 🌊 THE CONTENT: STARTING BELOW THE HEADER */}
      <main
        className="w-full overflow-y-auto overflow-x-hidden scrollbar-none"
        style={{
          position: 'absolute',
          top: `${forcedGap + 70}px`, // Starts exactly after the displaced header
          bottom: `${safeArea.bottom + 70}px`,
          touchAction: "pan-y",
          WebkitOverflowScrolling: "touch",
        }}
      >
      ...
      {/* 🛰️ THE NAV: ANCHORED BOTTOM */}
      <nav 
        className="absolute bottom-0 inset-x-0 bg-black/80 backdrop-blur-md border-t border-white/5"
        style={{ height: `${70 + safeArea.bottom}px`, paddingBottom: `${safeArea.bottom}px` }}
      >

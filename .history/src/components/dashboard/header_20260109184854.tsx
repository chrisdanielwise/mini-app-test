...
interface HeaderProps {
  user: {
    telegramId: string; // PRISMA 7 FIX: Changed from bigint to string
    fullName: string;
    username?: string;
  };
  merchant: {
      ...
      <div className="flex items-center gap-6">
        ...
        <div className="relative group">
          ...
          <input
            ...
            className="h-9 w-64 rounded-xl border border-border bg-background/50 pl-9 pr-4 text-sm transition-all focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary lg:w-80"
          />
        </div>
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" className="flex items-center gap-3 rounded-full px-2 hover:bg-primary/5">
              ...
              <div className="hidden flex-col items-start sm:flex">
                ...
                {user.username && <span className="text-[10px] font-medium text-muted-foreground">@{user.username}</span>}
              </div>
              <ChevronDown className="h-4 w-4 text-muted-foreground" />
            </Button>
          </DropdownMenuTrigger>

          <DropdownMenuContent align="end" className="w-56 rounded-xl border-border bg-card p-2 shadow-xl">

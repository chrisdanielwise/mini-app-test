 ...
 * Fix: Replaced fixed h-[100dvh] with flex-1 and added min-h-0 to unlock scroll calculation.
 * Fix: Clamped vertical padding to prevent content cropping.
 */
export function DashboardLayoutClient({ 
  ...
  return (
    <div 
      key={userRole} 
      // 🛡️ FIX: Switched from h-[100dvh] to flex-1 to prevent height collisions with Root Layout
    ...
    >
      ...
      <main 
        className={cn(
          // 🛡️ FIX: Added min-h-0 which is essential for Flexbox children to scroll correctly
          "flex-1 min-h-0 overflow-y-auto overflow-x-hidden transition-all duration-700 ease-[cubic-bezier(0.2,0.8,0.2,1)]",
          "scroll-smooth focus:outline-none overscroll-y-contain custom-scrollbar",
          // 🛡️ TACTICAL DENSITY: Locked horizontal gutters
          (isFullSize || isPlatformStaff) ? "px-4 md:px-8 lg:px-10" : "max-w-[1400px] mx-auto px-5 md:px-10",
          // 🛡️ FIX: Reduced pb-32 to pb-10 to prevent bottom cropping
          "pb-10 pt-0" 
        )}
      >

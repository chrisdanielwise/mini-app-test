...
/**
 * 🛰️ DASHBOARD_HEADER (Institutional v16.16.45)
 * Strategy: Stationary Horizon HUD & Vertical Compression.
 * Mission: High-density identity telemetry for mobile viewports.
 */
export function DashboardHeader({ user, merchant }: HeaderProps) {
  ...
  return (
    <header className={cn(
      ...
      // 🏁 TACTICAL COMPRESSION: h-14 baseline for mobile economy
      ...
      {isMobile ? (
        ...
        <div className="hidden lg:flex relative group flex-1 max-w-sm mx-10">
          ...
          <input
            ...
            className={cn(
              "h-10 w-full rounded-xl border bg-white/[0.01] pl-11 pr-4 text-[9px] font-black uppercase tracking-widest transition-all italic",
              themeAmber 
      ...
      <div className="flex items-center gap-2 md:gap-4 shrink-0">
        ...
        >
          ...
          <span className={cn(
            ...
            themeAmber ? "bg-amber-500 shadow-[0_0_8px_rgba(245,158,11,0.5)]" : "bg-primary"
          )} />
        </Button>
        ...
        <DropdownMenu onOpenChange={(val) => val && selectionChange()}>
          <DropdownMenuTrigger asChild>
            <Button 
              variant="ghost" 
              className="flex items-center gap-3 rounded-xl md:rounded-2xl px-2 h-10 md:h-14 hover:bg-white/5 border border-transparent hover:border-white/5 transition-all"
            >
          ...
          <DropdownMenuContent align="end" className="w-64 rounded-[1.5rem] md:rounded-[2rem] border-white/5 bg-zinc-950/95 backdrop-blur-3xl p-2 shadow-3xl">
            ...
            <DropdownMenuItem
              ...
              onClick={() => { notification("warning"); window.location.href = "/"; }}
            >

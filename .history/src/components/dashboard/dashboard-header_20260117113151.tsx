 ...
 * Fix: Standardized h-14/h-16 footprint and center-aligned mobile telemetry.
 */
export function DashboardHeader({ user, merchant, dashboardContext }: any) {
  ...
  return (
    ...
    >
        ...
        {isMobile ? (
          <Sheet>
            ...
            <SheetContent side="left" className="w-[280px] p-0 bg-zinc-950 border-r border-white/5">
              {/* This slides the sidebar in over the content */}
          ...
          <div className="flex flex-col shrink-0 leading-none">
            <div className="flex items-center gap-2 opacity-30 italic">
              {isStaffTheme ? <Globe className="size-2.5 text-amber-500 animate-pulse" /> : <Activity className="size-2.5 text-primary animate-pulse" />}
              <span className={cn("text-[7px] font-black uppercase tracking-[0.3em]", isStaffTheme ? "text-amber-500" : "text-primary")}>
      ...
      {/* --- 🛰️ CENTER: MOBILE STATUS / DESKTOP SEARCH --- */}
      {isMobile ? (
        ...
        <div className={cn("relative group transition-all duration-500 w-64 lg:w-80 xl:w-[420px]")}>
          <Search className={cn("absolute left-3 top-1/2 -translate-y-1/2 size-3 opacity-20", isStaffTheme ? "group-focus-within:text-amber-500" : "group-focus-within:text-primary")} />
          <input
            type="text"
      ...
      <div className="flex items-center gap-3 shrink-0 ml-4">
        ...
        {isMobile && (
          <Button variant="ghost" size="icon" className="size-9 rounded-lg bg-white/5 border border-white/10" onClick={() => impact("light")}>
        ...
        <DropdownMenu onOpenChange={() => selectionChange()}>
          ...
          <DropdownMenuContent align="end" className="w-56 rounded-xl border-white/5 bg-zinc-950 p-1.5 shadow-3xl backdrop-blur-2xl">
             ...
             <DropdownMenuItem className="rounded-lg py-3 cursor-pointer text-rose-500 focus:bg-rose-500/10 px-3" onClick={() => { impact("heavy"); window.location.href = "/"; }}>

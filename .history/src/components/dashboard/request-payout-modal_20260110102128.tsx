...
export function RequestPayoutModal({ merchantId, availableBalance }: RequestPayoutModalProps) {
  ...
  useEffect(() => {
    if (state?.success && !isPending) {
      toast.success("LIQUIDATION INITIALIZED: Request dispatched to finance node.");
      setOpen(false);
      setAmount("");
    }
  }, [state, isPending]);
            ...
            <div className="space-y-8">
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-end px-1">
                  <Label className="text-[10px] font-black uppercase tracking-[0.2em] text-primary">Liquidity Amount</Label>
                  <button 
                    type="button"
                  ...
                  >
                    MAX_AVAIL: ${balanceValue.toLocaleString(undefined, { minimumFractionDigits: 2 })}
                  </button>
                </div>
                <div className="relative group">
                  <span className="absolute left-6 top-1/2 -translate-y-1/2 font-black text-muted-foreground text-3xl opacity-20 group-focus-within:opacity-100 transition-opacity">$</span>
                  <Input
                    name="amount"
                    type="number"
                    step="0.01"
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div className="space-y-3">
                  <Label className="text-[10px] font-black uppercase tracking-[0.2em] ml-1">Network Protocol</Label>
                ...
                <div className="space-y-3">
                  <Label className="text-[10px] font-black uppercase tracking-[0.2em] ml-1">Target Address</Label>
                  <div className="relative">
              ...
              <div className="rounded-[1.5rem] bg-primary/5 border border-primary/10 p-5 flex items-start gap-4 shadow-sm">
                ...
                <p className="text-[9px] font-black text-muted-foreground uppercase leading-relaxed tracking-wider opacity-80">
                  Withdrawals are audited by <span className="text-foreground">Zipha Finance Nodes</span>. 
                  Standard latency: 24h. Protocol verification is immutable once synchronized.
                </p>
              </div>
            </div>

 ...
 * Fixed: Role normalization to resolve Clearance_Mismatch errors.
 */
export function DashboardSidebar({ context }: { context: any }) {
  ...
  // 🛡️ ROLE NORMALIZATION: Forces comparison to match NAVIGATION_CONFIG lowercase keys
  ...
  // 🛡️ RBAC FILTERING: Check against both raw and normalized role
  const filteredNav = NAVIGATION_CONFIG.filter(item => 
    item.roles.includes(role) || item.roles.includes(rawRole)
  );

  // 🛡️ EMERGENCY RENDER: If filtering fails, default to merchant links so UI stays visible
    ...
    <>
      ...
      <aside className={cn(
        ...
        <nav className="flex-1 space-y-1 p-4 md:p-6 overflow-y-auto custom-scrollbar">
          ...
          {/* Debug Visible only if mapping is still broken */}
          {filteredNav.length === 0 && (
             <div className="mt-4 p-3 rounded-lg bg-destructive/10 border border-destructive/20 flex items-center gap-2">
                <AlertTriangle className="h-3 w-3 text-destructive" />
                <span className="text-[7px] font-black uppercase text-destructive tracking-widest">Clearance_Mismatch: {rawRole}</span>
             </div>
          )}
        </nav>

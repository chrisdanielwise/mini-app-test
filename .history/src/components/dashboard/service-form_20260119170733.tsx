...
export function ServiceForm({ merchantId }: { merchantId: string }) {
  // ✅ FIX: Standardized action state hook for Next.js 15
  ...
  useEffect(() => {
    ...
    // ✅ FIX: Use 'in' operator to narrow the type safely
  ...
  // 🛡️ HYDRATION_SHIELD: Prevents layout snaps
  if (!isReady)
    return (
        ...
        <div className="relative overflow-hidden rounded-2xl md:rounded-[2.5rem] border border-white/5 bg-zinc-950/40 p-5 md:p-8 backdrop-blur-3xl shadow-2xl">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="space-y-1.5">
                <div className="flex items-center gap-2 opacity-20 italic">
                  ...
                  <h2 className="text-[7.5px] md:text-[8px] font-black uppercase tracking-[0.3em]">
                    Identity_v16
                  </h2>
                </div>
          ...
          <div className="grid gap-5 relative z-10">
            <div className="space-y-1.5 md:space-y-2">
              <Label className="text-[7.5px] md:text-[8.5px] font-black uppercase tracking-[0.2em] opacity-40 italic ml-1">
                Service_Label
              </Label>
              <Input
                name="name"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <Label className="text-[7.5px] font-black uppercase tracking-[0.2em] opacity-40 italic ml-1">
                  Asset_Class (Tag)
                </Label>
                <Input
                  name="categoryTag"
                  placeholder="FOREX"
                  className="h-11 px-4 rounded-xl bg-white/[0.02] border-white/5 font-black uppercase text-[10px] tracking-widest text-foreground"
                />
              </div>
              <div className="space-y-1.5">
                <Label className="text-[7.5px] font-black uppercase tracking-[0.2em] opacity-40 italic ml-1">
                ...
                <Input
                  ...
                  placeholder="-100..."
                  required
                  className="h-11 px-4 rounded-xl border-primary/20 bg-primary/5 font-mono text-[11px] text-primary"
                />
              </div>
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  placeholder="0.00"
                  required
                  className="h-11 px-4 rounded-xl bg-white/[0.02] border-white/5 font-black text-sm text-foreground shadow-inner"
                />
              </div>
              <div className="space-y-1.5">
                <Label className="text-[7.5px] font-black uppercase tracking-[0.2em] opacity-40 italic ml-1">
                ...
                {/* ✅ FIX: TS2345 - Standardized Enum values for IntervalUnit */}
                <select
                  name="tierIntervals[]"
              ...
              <div className="space-y-1.5">
                ...
                {/* ✅ FIX: TS2345 - Standardized Enum values for SubscriptionType */}
                <select
                  name="tierTypes[]"
                  className="w-full h-11 rounded-xl bg-zinc-900 border border-white/5 px-3 text-[8px] md:text-[9px] font-black uppercase tracking-[0.2em] outline-none appearance-none cursor-pointer text-foreground shadow-inner"
                >
                  <option value="CUSTOM">Standard Cycle</option>
      ...
      >
        ...
        >
          ...
          <div className="relative flex items-center justify-center gap-3 text-white font-black uppercase italic tracking-[0.2em] text-xs">
            {isPending ? (
              <>
                <Loader2 className="size-5 animate-spin" /> Provisioning Node
                Cluster...
              </>
            ) : (
              <>

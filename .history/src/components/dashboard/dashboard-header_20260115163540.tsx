...
export function DashboardHeader({ user, merchant }: HeaderProps) {
  ...
  if (!isReady) return <div className="h-20 w-full bg-card/20 animate-pulse border-b border-white/5" />;
  ...
  return (
    <header 
      className={cn(
        ...
        themeAmber ? "border-amber-500/20 bg-amber-500/[0.04]" : "border-white/5 bg-card/60 shadow-apex"
      )}
    ...
    >
      ...
      <div className={cn(
        ...
        themeAmber ? "bg-[radial-gradient(circle_at_0%_0%,#f59e0b,transparent_50%)]" : "bg-[radial-gradient(circle_at_0%_0%,var(--primary),transparent_50%)]"
      )} />
      ...
      <div className="flex items-center gap-4 md:gap-8 ml-8 shrink-0 relative z-10">
        ...
        <Button 
          ...
          className={cn(
            "relative size-12 md:size-14 rounded-2xl md:rounded-[1.4rem] bg-white/[0.03] border border-white/5 group transition-all active:scale-90",
            themeAmber ? "hover:bg-amber-500/10" : "hover:bg-primary/10"
          )}
        >
          <Bell className={cn(
            "size-5 md:size-6 text-muted-foreground/40 transition-colors group-hover:text-foreground",
            themeAmber ? "group-hover:text-amber-500" : "group-hover:text-primary"
          )} />
          <span className={cn(
            "absolute right-3.5 top-3.5 size-2 rounded-full",
            themeAmber ? "bg-amber-500 shadow-[0_0_8px_#f59e0b]" : "bg-primary shadow-[0_0_8px_#10b981]"
          )} />
        </Button>

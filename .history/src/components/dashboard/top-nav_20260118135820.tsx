...
/**
 * 🛰️ DASHBOARD_TOP_NAV (Institutional Apex v2026.1.18)
 * Strategy: Vertical Baseline Synchronization.
 * Fix: Restored mt-[6px] to align with Sheet Close +26px logic.
 */
  ...
  return (
    <header
      className={cn(
        ...
        isMobile ? "h-14 px-4" : "h-16 px-6 md:px-8",
        themeAmber
    ...
    >
      ...
      <div className="flex items-center gap-4 min-w-0">
        {isMobile ? (
            ...
            <SheetTrigger asChild>
              <Button
                ...
                className={cn(
                  ...
                  /* 🏁 THE SYNC: Offset matches the Sheet Close button logic */
                  "mt-[6px]" 
                )}
              >
          ...
          <div className="flex items-center gap-3 md:gap-4 min-w-0">
            <div className={cn(
              "size-9 md:size-10 shrink-0 rounded-xl flex items-center justify-center font-black italic", 
        ...
        {!isMobile && (
          <div className="hidden lg:flex items-center gap-4 ml-2 pl-6 border-l border-white/5">
            {themeAmber ? <Globe className="size-3 text-amber-500/40" /> : <Terminal className="size-3 text-white/5" />}
            <span className="text-[7.5px] font-black uppercase tracking-[0.4em] text-muted-foreground/20 italic">
            ...
            <span className={cn(
              ...
              themeAmber ? "text-amber-500" : "text-primary/60"
            )}>
      ...
      {/* --- CENTER: TELEMETRY (Mobile Only Status) --- */}
      {isMobile && (
        <div className="flex flex-col items-center leading-none absolute left-1/2 -translate-x-1/2 mt-[6px]">
          <span className="text-[6px] font-black uppercase tracking-[0.4em] opacity-20 italic leading-none">Status</span>
          <div className="flex items-center gap-1.5 mt-1 leading-none">
            <div className={cn("size-1 rounded-full animate-pulse", themeAmber ? "bg-amber-500" : "bg-primary")} />
            <span className="text-[8px] font-black uppercase tracking-tighter text-foreground italic">Active</span>
          </div>
        </div>
      )}
      ...
      <div className="flex items-center gap-3 shrink-0">
        <div className="flex items-center gap-3 mt-[6px]">
          <Button 
            ...
            className={cn(
              "relative size-9 rounded-xl border transition-all active:scale-90", 
          ...
          >
            ...
            <span className={cn("absolute top-2.5 right-2.5 size-1.5 rounded-full ring-1 ring-background", themeAmber ? "bg-amber-500" : "bg-primary")} />
          </Button>
          
          <div className={cn("h-6 w-px bg-white/5 mx-1", isMobile && "hidden")} />
        </div>

        {!isMobile && (isPlatformStaff || role === 'merchant') && (
          <Link href="/dashboard/services/new" onClick={() => impact("medium")}>
            <Button className={cn(
              "h-10 md:h-11 px-5 rounded-xl font-black uppercase italic tracking-[0.2em] text-[9px] shadow-lg", 
              themeAmber ? "bg-amber-500 text-black shadow-amber-500/10" : "bg-primary text-white shadow-primary/10"
            )}>

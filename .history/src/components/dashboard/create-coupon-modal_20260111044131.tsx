...
/**
 * 🛰️ PROMOTION DEPLOYMENT NODE (Apex Tier)
 * Normalized: World-standard fluid typography and responsive grid constraints.
 * Optimized: Adaptive haptics and touch-safe targets for campaign provisioning.
 */
        ...
        <form action={formAction} className="flex flex-col h-full overflow-hidden">
              ...
              <div className="flex items-center gap-3 md:gap-4">
                ...
                <div className="min-w-0">
                  ...
                  <DialogDescription className="text-[8px] md:text-[10px] font-black uppercase tracking-[0.2em] md:tracking-[0.3em] text-muted-foreground mt-1 opacity-60 truncate">
                    Provisioning discount node: {merchantId.slice(0, 12)}
                  </DialogDescription>
                </div>
              </div>
            </DialogHeader>
            ...
            <div className="space-y-6 md:space-y-8">
                ...
                <div className="flex gap-2 md:gap-3">
                  <div className="relative flex-1">
                    ...
                    <Input
                      ...
                      required
                      className="rounded-xl md:rounded-2xl border-border/40 bg-muted/10 h-12 md:h-14 pl-12 font-mono font-black text-base md:text-lg tracking-tighter text-primary focus:ring-primary/20 transition-all shadow-inner"
                    />
                  </div>
                  <Button
                    type="button"
                    ...
                    className="h-12 w-12 md:h-14 md:w-14 rounded-xl md:rounded-2xl border-border/40 bg-muted/10 hover:bg-primary/10 hover:text-primary transition-all group shrink-0"
                  >
                    <RefreshCw className="h-4 w-4 md:h-5 md:w-5 group-active:rotate-180 transition-transform duration-500" />
                  </Button>
                </div>
              </div>

              {/* --- Financial Variables: Responsive Stack --- */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 md:gap-6">
                <div className="space-y-2 md:space-y-3">
                  ...
                  <Input
                    ...
                    placeholder="10"
                    required
                    className="rounded-xl md:rounded-2xl border-border/40 bg-muted/10 h-12 md:h-14 font-black text-sm italic focus:ring-primary/20 shadow-inner"
                  />
                </div>
                <div className="space-y-2 md:space-y-3">
                  <Label className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.2em] ml-1 text-primary">
              ...
              <div className="space-y-2 md:space-y-3">
                ...
                <div className="relative group">
                  ...
                  <select
                    name="serviceId"
                    className="w-full h-12 md:h-14 rounded-xl md:rounded-2xl border border-border/40 bg-muted/10 pl-12 pr-4 text-[9px] md:text-[10px] font-black uppercase tracking-widest cursor-pointer outline-none focus:ring-2 focus:ring-primary/20 transition-all appearance-none"
                  >
                    <option value="global">
          ...
          <div className="bg-muted/30 p-5 md:p-8 flex flex-col sm:flex-row items-center justify-end gap-3 md:gap-4 border-t border-border/40 backdrop-blur-md">
            ...
            <Button
              type="submit"
              disabled={isPending}
              className="w-full sm:w-auto rounded-xl md:rounded-2xl bg-primary px-10 h-14 md:h-16 text-[10px] md:text-[11px] font-black uppercase italic tracking-[0.15em] shadow-2xl shadow-primary/30 transition-all hover:translate-y-[-2px] active:translate-y-[1px]"
            >
              {isPending ? (
                <>

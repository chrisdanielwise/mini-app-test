...
// ✅ PATH SYNC: Institutional standard imports for financial precision
import { Decimal } from "@prisma/client-runtime-utils";

/**
 * ✅ FIX: Strict Telemetry Interface
 * Ensures that Decimal types are handled via string conversion for hydration safety.
 */
 ...
 * Fix: Resolved TS2554 by standardizing service argument structure.
 * Fix: Resolved TS2339 by adding explicit telemetry data typing.
 */
export async function AsyncRevenueCard({
  ...
  try {
    ...
    // ✅ FIX: TS2554 - Passing a configuration object to the analytics service.
   const rawData = await Promise.race([
      getRevenueAnalytics({
        targetId: targetId ?? null, // Ensure targetId is never undefined
        from: thirtyDaysAgo,
        to: new Date(),
      }),
      ...
      console.warn(`🐢 [Physical_Lag]: Revenue Node Timed Out - ${err.message}`);
      return null;
    });

    // ✅ FIX: TS2339 - Type casting to defined Telemetry interface
    const data = rawData as RevenueTelemetry | null;
          ...
          <div className="flex flex-row items-center justify-between mb-8 relative z-10">
            <div className="space-y-1.5 leading-none">
              <p className="text-xl sm:text-2xl font-black uppercase italic tracking-tighter text-foreground">
                ...
                <span className={isGlobalOversight ? "text-amber-500" : "text-primary"}>
                  Trend
                </span>
              </p>
    ...
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full animate-in slide-in-from-bottom-4 duration-700">
        <StatsCard
          ...
          // ✅ FIX: Safe conversion of Decimal/String to Float for Locale formatting
          value={`$${parseFloat(data.total?.toString() || "0").toLocaleString(undefined, {

...
export function DashboardShell({ children }: { children: React.ReactNode }) {
  ...
  useLayoutEffect(() => {
    if (typeof window !== "undefined" && WebApp && isReady) {
      try {
        WebApp.ready();
        WebApp.expand();
        // 🛡️ HARDWARE LOCK: Forcefully kills the bounce/slide
  ...
  return (
     ...
     * This is the absolute root. It cannot move.
     */
    <div
      className={cn(
    ...
    >
      {/* 📐 TOP_BUFFER: The Physical Wall
          Strategy: We use a hard-coded 48px minimum if safeArea.top is 0.
          This ensures the Merchant Terminal text NEVER touches the status bar. */}
      <div 
        style={{ height: `${safeArea.top > 0 ? safeArea.top : 48}px` }} 
        className="shrink-0 w-full bg-black z-[100]" 
      />

      {/* 🚀 LAYER 1: STATIONARY_HUD_HEADER
          By being second in the flex list, it is physically pushed down. */}

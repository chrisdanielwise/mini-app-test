...
/**
 * 🛰️ DASHBOARD_LAYOUT_CLIENT (Institutional Apex v2026.1.18 - HARDENED)
 * Strategy: Viewport-Locked Chassis & Rigid Hardware Ingress.
 * Fix: Forced h-[100dvh] and flex-col to stop free-page scrolling.
 */
export function DashboardLayoutClient({ 
  ...
  return (
    // 🏛️ TIER 1: THE PRIMARY SHELL (Strictly locked to 100% of the dynamic viewport)
    // 🏁 THE FIX: 'h-[100dvh]' and 'overflow-hidden' are the master anchors.
    <div className="flex flex-col h-[100dvh] w-full bg-black overflow-hidden relative selection:bg-primary/30">
      ...
      {/* 🛰️ STATIONARY HUD: Hardware Notch Protection 
          🏁 THE FIX: 'shrink-0' ensures the header never moves up.
      */}
      <header 
      ...
      {/* 🌊 TIER 2: LAMINAR RESERVOIR (Internal Scroll Engine) 
          🏁 THE FIX: 'flex-1' + 'min-h-0' + 'overflow-y-auto' creates the internal reservoir.
      */}
      <main 
        className={cn(
          "flex-1 min-h-0 w-full overflow-y-auto scrollbar-hide overscroll-contain relative",
      ...
      >
        <div className="relative min-h-full w-full">
          {!mounted || !isReady ? (
            ...
            // 🏎️ PAGE_INGRESS: The only area where scrolling is permitted.
            <div 
              className="animate-in fade-in slide-in-from-bottom-2 duration-700 h-full w-full"
            >
              {/* 🏁 MOBILE DESIGN AWARENESS: 
                  We pass the children directly. The children (ServicesPage) 
                  should use its own flexbox internal scroll for the table.
              */}
              {children}
            </div>
          )}
      ...
      {/* 🕹️ TIER 3: COMMAND HUB (Stationary Bottom HUD) 
          🏁 THE FIX: Fixed positioning ensures it floats above the scrollable reservoir.
      */}
      {mounted && isMobile && (
        <div 

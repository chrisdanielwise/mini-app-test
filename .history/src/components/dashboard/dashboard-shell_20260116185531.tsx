...
export function DashboardShell({ children }: { children: React.ReactNode }) {
  ...
  // 📐 CALCULATION: We subtract the notch height from the total available height
  // to prevent the app from being "pushed" off the bottom of the screen.
  const topGap = Math.max(safeArea.top, 27); 
  const adjustedHeight = viewportHeight - topGap;

  return (
    /**
     * ⬛ LAYER 0: THE SHIFTED_ROOT
     * Instead of starting at 0, we start at the topGap (27px).
     */
    <div
      className={cn(
        "fixed inset-x-0 bg-black antialiased overflow-hidden flex flex-col",
        isInitialized ? "opacity-100" : "opacity-0"
      )}
      style={{ 
        top: `${topGap}px`, // 🚀 THIS IS THE SHIFT: Physically moves the whole app down
        height: `${adjustedHeight}px`,
        maxHeight: `${adjustedHeight}px`,
      }}
    >
      {/* 🚀 LAYER 1: HUD_HEADER
          Now, pixel 0 of this header is physically pixel 27 of the screen. */}
      <header className="shrink-0 pt-2 pb-4 flex flex-col px-4 bg-black/40 backdrop-blur-3xl border-b border-white/5 z-50">
        <div className="flex items-center gap-2 opacity-30 italic leading-none mb-1">
          <Terminal className="size-2.5" />
          <span className="text-[7px] font-black uppercase tracking-[0.4em]">Mesh_Ingress_v16.31</span>
        ...
        <div className="flex items-center gap-2 mt-2 opacity-50">
          <span className="text-[8px] font-black uppercase tracking-widest text-muted-foreground truncate">
            Node_ID: 09C55E7C // Terminal_Active
          </span>
        </div>
      </header>

      {/* 🌊 LAYER 2: LAMINAR_CONTENT */}
      <main
        id="main-scroll-root"
        className="flex-1 w-full overflow-y-auto overflow-x-hidden relative z-10 scrollbar-none"
        style={{

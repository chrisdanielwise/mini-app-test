...
export default async function AsyncSubscribersCard({
  ...
  try {
    ...
    // ✅ FIX: Ensured analytics calls match their expected object-based signatures
    /**
     * ✅ FIX: Institutional Analytics Handshake
     * Logic: Race-condition protection with a 15s latency ceiling.
     * Note: getDashboardStats now uses strict Object Ingress.
     */
    const results = (await Promise.race([
      Promise.allSettled([
        ...
        isGlobalOversight
          ? getGlobalPlatformStats()
          : getDashboardStats({ targetId: targetId! }), 
      ]),
    ...
    // 🛡️ EXTRACTION LOGIC
    // Promise.allSettled returns { status: 'fulfilled', value: T }
    const growthData = results[0].status === 'fulfilled' ? results[0].value : null;

 ...
 * Strategy: High-density volume with Inherited Height Protocol.
 * Fix: Replaced fixed 'hidden' overflow with 'clip' to allow child scroll calculation.
 * Fix: Tightened padding-top and header margins to reclaim vertical real estate.
 */
export function DashboardShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    ...
    >
      ...
      <main
        className={cn(
          "flex-1 w-full min-h-0 overflow-y-auto overflow-x-hidden custom-scrollbar relative z-10",
          isMobile ? "px-5" : "px-8 lg:px-16"
        )}
        style={{
          // 🛡️ FIX: Reduced 1.25rem to 0px as the TopNav/Header provides enough offset
          paddingTop: `calc(${safeArea.top}px + 0.25rem)`, 
          // 🛡️ FIX: Reduced bottom padding to prevent cropping rows behind mobile navigation
 ...
 * Fix: Reduced bottom margins to bring table data closer to the title.
 */
export function DashboardHeader({
  ...
  return (
    <div
      className={cn(
        "flex flex-col gap-3 mb-6 md:mb-8 animate-in fade-in slide-in-from-left-4 duration-700", // 🛡️ FIX: Reduced mb-10/16 to mb-6/8
        screenSize === "xs" ? "px-1" : "px-0"
      )}
    >
      <div className="space-y-1">
        <div className="flex items-center gap-2.5 opacity-20 mb-1italic">
      ...
      <div className="flex items-center gap-4">
        <div
          className={cn(
            "h-px w-8 md:w-12", // 🛡️ FIX: Reduced line width

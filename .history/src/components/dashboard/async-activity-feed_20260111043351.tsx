...
/**
 * 🛰️ ASYNC ACTIVITY FEED (Apex Tier)
 * Normalized: World-standard fluid typography and responsive grid constraints.
 * Optimized: Adaptive scroll-zones and tactile touch targets for mobile staff.
 */
export default async function AsyncActivityFeed({
  ...
  return (
    <div className="flex flex-col h-full min-h-[450px] rounded-[2.5rem] md:rounded-[3rem] border border-border/10 bg-card/40 p-5 md:p-8 shadow-2xl backdrop-blur-3xl overflow-hidden animate-in fade-in duration-1000 relative">
      ...
      <Terminal className="absolute -bottom-10 -left-10 h-48 w-48 opacity-[0.02] -rotate-12 pointer-events-none" />
      ...
      <div className="flex flex-row items-center justify-between mb-8 px-2 relative z-10">
        <div className="space-y-1">
          ...
          <p className="text-base md:text-lg font-black uppercase italic tracking-tighter leading-none truncate">
            Recent <span className="text-primary">Activity</span>
          </p>
        </div>
        <a
          href="/dashboard/payments"
          className="group text-[9px] md:text-[10px] font-black uppercase italic text-primary hover:scale-105 active:scale-95 transition-all flex items-center gap-1.5 shrink-0"
        >
          Full Ledger{" "}
      ...
      <div className="space-y-3 md:space-y-4 overflow-y-auto pr-1 md:pr-2 custom-scrollbar relative z-10 flex-1">
        {activities.map((item: any) => (
          <div
            key={item.id}
            className="group flex items-center justify-between p-4 md:p-5 hover:bg-primary/[0.04] active:scale-[0.99] rounded-2xl md:rounded-[2rem] transition-all border border-transparent hover:border-primary/10 shadow-sm md:shadow-none"
          >
            <div className="flex items-center gap-3 md:gap-5 min-w-0">
              ...
              <div className="flex flex-col min-w-0">
                <span className="text-xs md:text-sm font-black uppercase italic tracking-tighter leading-none group-hover:text-primary transition-colors truncate">
            ...
            <div className="text-right flex flex-col items-end gap-1.5 shrink-0">
              <span className="text-sm md:text-base font-black italic tracking-tighter leading-none">
                +${parseFloat(item.serviceTier?.price || "0").toLocaleString(undefined, { minimumFractionDigits: 2 })}
              </span>
              <Badge
                variant="outline"
                className={cn(
...
/**
 * 🛠️ UI HELPERS: Normalized for Apex Aesthetic
 */

 ...
 * Accessibility Fix: Added SheetTitle within SheetContent to resolve DialogTitle console error.
 */
export function DashboardTopNav({ context }: { context: any }) {
  ...
  // 🛡️ HYDRATION SHIELD: Ultra-Slim Pulse
  if (!isReady) return <header className="h-14 w-full bg-black/40 border-b border-white/5 animate-pulse outline-red-600" />;

  return (
    <header 
      className={cn(
        "sticky top-0 z-[60] flex items-center justify-between w-full transition-all duration-700 backdrop-blur-xl border-b",
        // 🛡️ TACTICAL SLIM: Standardized height profiles
    ...
    >
      ...
      {/* --- LEFT: IDENTITY NODE / MOBILE TRIGGER --- */}
      <div className="flex items-center gap-4 md:gap-6 min-w-0">
        {isMobile ? (
          /* 🍔 MOBILE HAMBURGER: Triggers the Sidebar Drawer */
          <Sheet>
            <SheetTrigger asChild>
              <Button 
                ...
                className="size-10 rounded-xl bg-white/5 border border-white/10 active:scale-90"
              >
            ...
            <SheetContent side="left" className="w-[280px] p-0 bg-zinc-950 border-r border-white/5">
              ...
              {/* 🛡️ ACCESSIBILITY FIX: Radix requires a Title inside Content for screen readers */}
              <SheetHeader className="sr-only">
                <SheetTitle>Terminal_Navigation_Mesh</SheetTitle>
              </SheetHeader>

              {/* Technical context for the drawer header */}
              <div className="px-6 py-5 border-b border-white/5 bg-white/[0.01]">
                <div className="flex items-center gap-2 opacity-30 italic">
                  <Activity className="size-3 text-primary" />
                  <span className="text-[8px] font-black uppercase tracking-[0.4em]">Node_Waypoint</span>
                </div>
              </div>
          ...
          <div className="flex items-center gap-3 md:gap-4 min-w-0">
            ...
            <div className="flex flex-col min-w-0 leading-none">
              <span className="font-black uppercase italic text-[11px] md:text-sm tracking-tighter text-foreground truncate">
        ...
        {/* --- BREADCRUMB TELEMETRY (Desktop Only) --- */}
        {!isMobile && (
      ...
      {/* --- CENTER: TELEMETRY (Mobile) / GLOBAL QUERY NODE (Desktop) --- */}
      {isMobile ? (
        <div className="flex flex-col items-center leading-none">
          <span className="text-[7px] font-black uppercase tracking-[0.4em] opacity-20 italic">Node_Sync</span>
        ...
        <div className="hidden xl:flex relative group px-10 flex-1 max-w-lg transition-all">
          <Search className={cn(
            "absolute left-14 top-1/2 -translate-y-1/2 size-4 opacity-20 transition-colors",
            themeAmber ? "group-focus-within:text-amber-500" : "group-focus-within:text-primary"
          )} />
          <input 
            onFocus={() => impact("light")}
            type="text"
            placeholder={isPlatformStaff ? "QUERY_CLUSTERS..." : "FILTER_NODE..."}
            className={cn(
      ...
      <div className="flex items-center gap-3 md:gap-4 shrink-0 ml-4">
        ...
        {/* 🚀 ACTION GATING: Deployment Command (Desktop-Only logic for clean mobile HUD) */}
        {!isMobile && (isMerchant || role === 'super_admin') && (
          <Link href="/dashboard/services/new" onClick={() => impact("medium")}>
            <Button className={cn(
              "h-10 md:h-11 px-5 rounded-xl font-black uppercase italic tracking-[0.2em] text-[9px] shadow-lg transition-all group",
              themeAmber 
                ? "bg-amber-500 text-black shadow-amber-500/10 hover:scale-105" 
                : "bg-primary text-white shadow-primary/10 active:scale-95"
            )}>
              <Zap className="mr-2.5 size-4 fill-current transition-transform group-hover:rotate-12" />
        ...
        <Button 
          ...
          className={cn(
            "relative size-10 md:size-11 rounded-xl border transition-all active:scale-90 group",
            themeAmber ? "bg-amber-500/5 border-amber-500/10" : "bg-white/[0.01] border-white/5"
          )}
        >
          <Bell className={cn(
            "size-4.5 transition-colors",
            themeAmber ? "text-amber-500/20 group-hover:text-amber-500" : "text-white/10 group-hover:text-primary"
          )} />
          <span className={cn(
            "absolute top-2.5 right-2.5 size-1.5 rounded-full animate-ping shadow-lg",
            themeAmber ? "bg-amber-500" : "bg-primary"
          )} />
        </Button>

        {/* IDENTITY ANCHOR (Mobile) */}

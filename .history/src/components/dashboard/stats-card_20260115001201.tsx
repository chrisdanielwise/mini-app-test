 ...
 * Logic: Haptic-synced KPI telemetry with 24H Sparkline Momentum.
...
interface StatsCardProps {
  ...
  iconColor?: string;
  className?: string;
  sparkData?: { value: number }[]; // 📈 Optional custom telemetry data
}
...
const StatsCardComponent = ({
  ...
  // 🧪 MOCK TELEMETRY GENERATOR: If no sparkData provided, generate a stable waveform
  ...
  if (!mounted) return (
    <div className="min-h-[220px] rounded-[2.5rem] bg-white/[0.03] border border-white/5 animate-pulse" />
  );
  ...
  return (
    ...
    >
      ...
      {/* 🚀 FINANCIAL PULSE: Subsurface Momentum Waveform */}
      <div className="px-8 pb-6 mt-auto flex items-end justify-between gap-4 overflow-hidden">
        <div className="flex-1 h-12">
          <Sparkline 
            data={displaySparkData} 
            trend={isPositive ? "UP" : isNegative ? "DOWN" : "NEUTRAL"} 
            className="opacity-40 group-hover:opacity-100 transition-opacity duration-1000"
          />
        </div>
        <div className="shrink-0 mb-1">
          <span className="text-[7px] font-black uppercase tracking-[0.3em] text-muted-foreground/20 italic group-hover:text-muted-foreground/40 transition-colors">
            24H_Vector
          </span>
        </div>
      </div>

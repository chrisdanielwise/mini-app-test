      ...
      if (!growth) {
        return (
          <div className="flex h-64 flex-col items-center justify-center rounded-[2.5rem] border border-border/40 bg-card/20 backdrop-blur-md">
            ...
            <p className="mt-4 text-[10px] font-black uppercase tracking-[0.3em] opacity-30 italic">Syncing Growth Data...</p>
          </div>
        );
      }
    ...
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full animate-in slide-in-from-bottom-4 duration-700">
        ...
        <StatsCard
          ...
          className={cn(ticketCount > 0 && "border-orange-500/30 shadow-[0_0_20px_rgba(249,115,22,0.1)]")}
        />
      </div>
    );
  } catch (error: any) {

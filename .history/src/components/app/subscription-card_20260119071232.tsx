...
// ✅ FIX: Explicit Interface definition for Type-Check sync
...
/**
 * 🛰️ SUBSCRIPTION_CARD (Institutional Apex v2026.1.20 - HARDENED)
 * Strategy: Vertical Compression & Stationary Horizon.
 * Fix: Prop destructuring corrected to satisfy TS2322 and layout-provider sync.
 */
export function SubscriptionCard({
  ...
  const { hapticFeedback } = useHaptics(); // ✅ Fixed: Destructuring aliased member
  
  // 🛰️ DEVICE INGRESS
  const { isMobile, isPortrait, isReady } = useDeviceContext();
  
  // 🛡️ PROTOCOL NORMALIZATION: Handle uppercase/lowercase drift
  const isActive = useMemo(() => status?.toUpperCase() === "ACTIVE", [status]);
  ...
  const isMerchantOperator = useMemo(() => 
    ...
    auth.user?.role?.toUpperCase() === "SUPER_ADMIN", 
    [auth.user?.role]
  );
  ...
  if (!isReady) return (
    <div className="h-24 w-full bg-white/5 animate-pulse rounded-xl border border-white/5" />
  );

  const handleAction = () => {
    hapticFeedback("heavy"); // ✅ Using the corrected hook member
  ...
  return (
    <div className={cn(
      ...
      <div className={cn(
        ...
        isActive ? (isExpiringSoon ? "bg-amber-500 shadow-[0_0_10px_#f59e0b]" : "bg-primary shadow-[0_0_10px_#10b981]") : "bg-white/10"
      )} />
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2 bg-white/5 px-2.5 py-1.5 rounded-lg border border-white/5">
              ...
              <span className="text-[8px] font-black uppercase tracking-widest text-foreground/80">
                {tier?.name} // {typeof tier?.price === 'number' ? `$${tier.price}` : tier?.price}
              </span>
            </div>
        ...
        {isActive && (
          <Button 
            ...
            className={cn(
              ...
              isMerchantOperator ? "bg-amber-500 text-black hover:bg-amber-400" : "bg-primary text-primary-foreground hover:bg-primary/90"
            )}
          >
            {isMerchantOperator ? (
              <><LayoutDashboard className="mr-2 size-3.5" /><span>Manage</span></>
            ) : (
              <><ExternalLink className="mr-2 size-3.5" /><span>Open</span></>
            )}
          </Button>
        )}
      </div>

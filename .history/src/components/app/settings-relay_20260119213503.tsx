...
export function SettingsRelay() {
  ...
  const { isReady, screenSize, isMobile, isTablet, isPortrait, isDesktop } =
    useDeviceContext();
  ...
  const { execute: saveConfig, loading: isSyncing } = useInstitutionalFetch(
    // ✅ FIX: Added '?' to make payload optional to satisfy the hook's type requirement
    async (payload?: typeof config) => {
      if (!payload) throw new Error("MISSING_CONFIGURATION_PAYLOAD");

      const res = await fetch("/api/merchant/config", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" }, // Added for institutional safety
    ...
    },
    {
      ...
      onError: (err: any) => {
        ...
        toast.error(`RELAY_ERROR: ${err.message || err}`);
      },
    }
  );
  ...
  if (!isReady)
    return (
      <div className="h-64 animate-pulse bg-white/5 rounded-2xl border border-white/5" />
    );
          ...
          <div
            className={cn(
              ...
              isStaff
                ...
                : "bg-card/40 border-white/5 shadow-2xl"
            )}
          >
            <div className="flex items-center gap-3 mb-6 leading-none">
              ...
              <div className="flex flex-col gap-1">
                ...
                <span className="text-[7.5px] font-black uppercase tracking-[0.3em] opacity-20 italic">
                  Telegram_Relay_v16
                </span>
              </div>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="relative group">
                ...
                >
                  {showBotToken ? (
                    <EyeOff className="size-3.5 opacity-20" />
                  ) : (
                    <Eye className="size-3.5 opacity-20" />
                  )}
                </button>
              </div>
            </div>
        ...
        <div className="space-y-4">
          <div className="rounded-2xl border border-white/5 bg-white/[0.01] p-5 space-y-6 shadow-sm">
            ...
            <div className="space-y-2">
              {[
                {
                  label: "IP_Whitelisting",
                  status: "ACTIVE",
                  icon: ShieldCheck,
                  color: "text-emerald-500",
                },
                {
                  label: "SSL_Termination",
                  status: "STABLE",
                  icon: Activity,
                  color: "text-emerald-500",
                },
                ...
                <div
                  key={item.label}
                  className="flex items-center justify-between p-3 rounded-xl bg-black/20 border border-white/5 group leading-none"
                >
                  <div className="flex flex-col gap-1">
                    <span className="text-[6.5px] font-black uppercase text-muted-foreground/30">
                      {item.label}
                    </span>
                    <span
                      className={cn(
                  ...
                  <item.icon
                    className={cn(
                      "size-3 opacity-20 group-hover:opacity-100 transition-opacity",
                      item.color
                    )}
                  />
                </div>
          ...
          <div className="rounded-2xl border border-dashed border-white/10 p-5 flex flex-col items-center justify-center text-center space-y-3 opacity-20 hover:opacity-100 transition-all group">
            ...
            <Button
              variant="ghost"
              className="h-7 text-[7px] font-black uppercase text-primary tracking-widest hover:bg-transparent"
            >
              Initialize_Rotation
            </Button>
          </div>

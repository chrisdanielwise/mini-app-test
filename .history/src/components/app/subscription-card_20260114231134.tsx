...
/**
 * 🛰️ SUBSCRIPTION DEPLOYMENT NODE (Apex Tier v16.16.6)
 * Logic: Synchronized with Universal Handshake & Staff Oversight.
 */
export function SubscriptionCard({
  ...
  // 🏛️ ROLE-AWARE CLEARANCE: Detects if current operator is the merchant
  ...
  const handleAction = () => {
    ...
    // 🚀 ROUTING: Merchants go to Management Node; Users go to Broadcaster
        ...
        <div className="flex-1 space-y-4 md:space-y-5 min-w-0">
          ...
          <div className="space-y-1.5 md:space-y-2">
            <div className="flex items-center gap-2 md:gap-3 flex-wrap">
              ...
              {isMerchantOperator && (
                <Badge className="bg-amber-500/10 text-amber-500 border-amber-500/20 font-black text-[7px] md:text-[8px]">
                  OWNER_NODE
                </Badge>
              )}
            </div>
        ...
        {isActive && (inviteLink || isMerchantOperator) && (
          <div className="relative group/btn shrink-0">
            ...
            <Button 
              ...
              className={cn(
                "relative h-11 w-11 md:h-14 md:w-14 lg:h-auto lg:w-auto lg:px-8 rounded-xl md:rounded-2xl font-black uppercase italic tracking-widest shadow-2xl transition-all hover:translate-y-[-2px] active:scale-95",
                isMerchantOperator ? "bg-amber-500 text-black hover:bg-amber-400" : "bg-primary text-primary-foreground"
              )}
            >
              {isMerchantOperator ? (
                <>
                  <LayoutDashboard className="lg:mr-2 h-4 w-4 md:h-5 md:w-5" />

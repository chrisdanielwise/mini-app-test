...
/**
 * ✅ FIX: TS2304 - Defined strict interface for the Search Protocol
 */
 ...
 */
export function SearchProtocol({ value, onChange, isStaff = false }: SearchProtocolProps) {
  ...
  if (!isReady) return (
    <div className="h-11 w-full bg-zinc-950/40 animate-pulse rounded-xl border border-white/5" />
  );
  ...
  return (
    ...
    >
      ...
      <div className="relative flex-1 group">
        <div className={cn(
          "absolute left-2.5 top-1/2 -translate-y-1/2 transition-all duration-500 z-10",
        ...
        <Input
          ...
          className={cn(
            ...
            "h-8 focus-visible:ring-0 placeholder:text-muted-foreground/10 text-foreground"
          )}
        />
        ...
        {value && (
          <button
            ...
            className="absolute right-2 top-1/2 -translate-y-1/2 size-6 flex items-center justify-center rounded-md hover:bg-white/5 transition-all opacity-20 hover:opacity-100 z-10"
          >
            <X className="size-3" />
          </button>
        )}

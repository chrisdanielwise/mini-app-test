...
/**
 * ✅ FIX: Defined strict interface for the financial payload
 */
 ...
 * Fix: Resolved TS2322 by synchronizing startWithdrawal payload.
 */
export function AssetVault({ balance = 0, pending = 0 }: { balance?: number; pending?: number }) {
  ...
  /**
   * ✅ FIX: Standardized institutional fetch signature
   */
  const { execute: startWithdrawal, loading: isSyncing } = useInstitutionalFetch<any, WithdrawalPayload>(
    async (payload) => {
    ...
    },
    {
      ...
      onError: (err) => {
        ...
        toast.error(`VAULT_ERROR: ${err.message || "Unknown_Failure"}`);
      }
    }
  );
  ...
  const handleWithdrawal = () => {
    ...
    // ✅ FIX: TS2322 - Passing the structured payload to the execute trigger
        ...
        <div className="grid gap-4 relative z-10">
          ...
          <div className="space-y-3">
            ...
            <div className="relative">
              ...
              <Button 
                variant="ghost" 
                onClick={() => { setAmount(balance.toString()); if(impact) impact("light"); }}

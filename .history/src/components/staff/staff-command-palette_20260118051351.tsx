...
/**
 * 🛰️ STAFF_COMMAND_PALETTE (Institutional v16.42.05)
 * Strategy: Global Keyboard Listener & Rapid Node Ingress.
 * Mission: Zero-latency navigation for platform operators.
 */
      ...
      >
        ...
        <div className="max-h-[60vh] overflow-y-auto p-2 scrollbar-hide">
          <CommandGroup label="Identity_Protocols">
            <CommandItem 
              ...
              onClick={() => handleAction(() => console.log("SWAP_TRIGGERED"))} 
            />
            <CommandItem 
              icon={Users} 
          ...
          <CommandGroup label="System_Tactical">
            <CommandItem 
              ...
              severity="rose"
              onClick={() => handleAction(() => console.log("LOCKDOWN"))} 
            />
            <CommandItem 
              icon={Settings} 
              label="Global_Config" 
        ...
        <div className="px-6 py-3 border-t border-white/5 bg-white/[0.01] flex items-center justify-between">
           <div className="flex items-center gap-2 opacity-20 italic">
              ...
              <span className="text-[8px] font-black uppercase tracking-widest">Command_Horizon_Active</span>
           </div>
           <p className="text-[7px] font-bold text-muted-foreground/30 uppercase tracking-[0.2em]">Institutional_Apex // v16.42</p>
        </div>
      </div>
  ...
  return (
    ...
    >
      <div className={cn(
        ...
        severity === "rose" ? "bg-rose-500/10 border-rose-500/20 text-rose-500" : "bg-white/5 border-white/10 text-muted-foreground group-hover:text-primary group-hover:border-primary/20"
      )}>

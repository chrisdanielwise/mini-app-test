...
export function AppClientProvider({ children }: { children: React.ReactNode }) {
  ...
  const initTMA = useCallback((force = false) => {
    // 🛡️ THE TERMINATOR: Stand down at the gate to prevent hydration loops.
    // If the server redirected with a 'reason', this bypass is mandatory.
    if (pathname === "/dashboard/login" || typeof window === "undefined") {
      setIsReady(true);
    ...
    // 🛡️ VERSION GATE (Critical Stability Fix for v6.0)
    // Checks semantic versioning to prevent calling modern APIs on legacy nodes.

...
import { useDeviceContext } from "@/components/providers/device-provider"; // 🏛️ Apex Integration
 ...
 * Logic: Merged Identity + Hardware Telemetry.
 */
 ...
 * Logic: Hardware-Aware identity gateway.
 */
function TelegramHandshakeGate({ children }: { children: ReactNode }) {
  ...
  const { isPortrait, screenSize, safeArea } = useDeviceContext(); // 🛰️ Hardware Ingress
  const router = useRouter();
  const pathname = usePathname();
  const [isRedirecting, setIsRedirecting] = useState(false);
  const handshakeLock = useRef(false);

  // 🛠️ Hardware Feedback Bridge
  ...
  // 🏛️ INGRESS SHIELD: Morphology-aware loading
  const showLoader = isRedirecting || (isTelegram && !isReady && !auth.isAuthenticated);

  if (showLoader) {
    return (
      <div 
        className="flex flex-col w-full bg-background transition-all duration-1000"
        style={{ 
      ...
      <TelegramHandshakeGate>
        <div 
          className={cn(
            ...
            "transition-colors duration-1000 ease-[cubic-bezier(0.2,0.8,0.2,1)]" // 🌊 Water-Ease
          )}
          style={{ minHeight: `calc(var(--vh, 1vh) * 100)` }}
        >
          {children}
          {/* Hardware grid overlay */}
          <div className="fixed inset-0 pointer-events-none z-[-1] opacity-[0.01] bg-[url('/assets/grid.svg')] bg-center transition-opacity duration-1000" />
        </div>
      </TelegramHandshakeGate>
    </TelegramContext.Provider>
  );

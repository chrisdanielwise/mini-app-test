...
function TelegramHandshakeGate({ children }: { children: ReactNode }) {
  const { auth, isTelegram, isReady, getInitData, hapticFeedback } = useTelegramContext();
...
export function TelegramProvider({ children }: { children: ReactNode }) {
  ...
  // ✅ FIX: HARDENED MAIN BUTTON BRIDGE
  const setMainButton = useCallback((options: { 
    ...
    // 🛡️ GATEKEEPER: Prevent TMA Text-Required Crash
    if (!options.text || options.text.trim().length === 0) {
      if (options.isVisible === false) btn.hide();
      return;
    }

    btn.setText(options.text); // ✅ Now safe from 151:9 crash

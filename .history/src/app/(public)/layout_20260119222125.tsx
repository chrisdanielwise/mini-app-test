...
export default function PublicLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div 
      ...
      // ✅ CHANGE: Ensure overflow-y is set to auto/visible so the browser knows to scroll
      "selection:bg-primary/20 antialiased overflow-x-hidden overflow-y-auto relative",
      isStaff && "selection:bg-amber-500/20"
    )}
    // ✅ CHANGE: Use min-height but ensure it's not locking the container
    style={{ minHeight: "100vh" }} 
    >
      
      {/* 🌫️ TACTICAL RADIANCE */}
     {/* This is fixed, so it won't move, which is correct */}
    <div 
      className={cn(

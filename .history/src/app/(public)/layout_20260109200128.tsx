      ...
      <header className="fixed top-0 z-50 w-full border-b border-border/40 bg-background/60 backdrop-blur-xl transition-all">
        <div className="container mx-auto flex h-20 items-center justify-between px-6">
          ...
          <nav className="hidden md:flex items-center gap-10">
            {['Features', 'Pricing', 'Infrastructure'].map((item) => (
              <Link 
                key={item} 
            ...
            <Button asChild variant="ghost" className="rounded-xl font-black uppercase tracking-widest text-[10px] h-10 px-6">
              <Link href="/dashboard/login">Sign In</Link>
            </Button>
            <Button asChild className="rounded-xl font-black uppercase tracking-widest text-[10px] h-10 px-6 shadow-xl shadow-primary/10">

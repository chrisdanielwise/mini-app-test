...
/**
 * 🛰️ DASHBOARD_PAGE (Hardened v16.32.90)
 * Strategy: Stationary HUD & Independent Tactical Scroll.
 * Mission: Unified Ingress for Staff (Amber) and Merchants (Emerald).
 */
export default function DashboardPage({ session }: { session: any }) {
  const { impact } = useHaptics();
  const { isReady, isMobile, safeArea } = useDeviceContext();

  // 🛡️ SECURITY HANDSHAKE (Handled via Props/Client-Side check for 2026 consistency)
  ...
  return (
    <div className="w-full h-full flex flex-col min-w-0 overflow-hidden text-foreground bg-black">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 px-6">
          <div className="space-y-1 min-w-0">
            ...
            <div className="flex items-center gap-3 text-[7px] font-black text-muted-foreground/20 italic uppercase tracking-[0.2em]">
              ...
              <span>Verified Node: {session.user.fullName || "Staff"} // ID: {realMerchantId?.slice(0, 8).toUpperCase() || "ROOT"}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div 
        className="flex-1 min-h-0 w-full relative overflow-y-auto scrollbar-hide overscroll-contain px-6 py-6 space-y-6"
      ...
      >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <StatusNode 
            ...
            status={isPlatformStaff ? "Healthy" : "Stable"} 
              ...
              <div className="px-6 py-4 border-b border-white/5 bg-white/[0.02] flex items-center justify-between">
                <div className="flex items-center gap-3 opacity-30">
                   ...
                   <h3 className="text-[8px] font-black uppercase tracking-[0.4em] italic">
                    {isPlatformStaff ? "Global_Network_Telemetry" : "Local_Telemetry_Feed"}
                  </h3>
                </div>
                <div className={cn(
        ...
        <div className="flex items-center justify-center gap-4 opacity-10 pt-4 pb-4">
          ...
          <p className="text-[7px] font-black uppercase tracking-[0.5em] italic text-center">
            Command Center Synchronized // [v16.32_STABLE]
          </p>
        </div>
      </div>
    </div>

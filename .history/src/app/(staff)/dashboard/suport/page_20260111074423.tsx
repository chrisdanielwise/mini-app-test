...
export default async function SupportPage() {
  ...
  // 🏁 Data Fetch: Using 'ticket' model from schema.prisma
        ...
        <div className="flex flex-col gap-3 min-w-fit flex-1">
          ...
          <div className="space-y-1">
            ...
            <p className="text-[9px] md:text-[10px] font-bold text-muted-foreground uppercase tracking-widest mt-2 opacity-40 italic">
              Node Status: <span className="text-foreground tabular-nums">{openCount}</span> Pending Intervention(s)
            </p>
          </div>
        </div>
              ...
              {tickets.length === 0 ? (
                <tr>
                  <td colSpan={5} className="px-6 py-24 text-center opacity-20">
                    <p className="text-xs font-black uppercase italic tracking-widest">System Clear. Protocols resolved.</p>
                  </td>
                </tr>
              ) : (
                tickets.map((ticket) => (
                    ...
                    <td className="px-6 py-4 text-center w-20">
                      {ticket.status === 'OPEN' ? (
                        <div className="relative mx-auto h-2.5 w-2.5">
                           ...
                           <div className="relative h-2 w-2 rounded-full bg-primary ring-2 ring-background" />
                        </div>
                      ) : (

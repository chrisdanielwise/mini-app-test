...
export default async function DashboardPage() {
  ...
  // 🛰️ HYBRID_IDENTIFICATION
  const userId = headerList.get("x-user-id");
  const role = (headerList.get("x-user-role") || "user").toLowerCase();
  
  // 🛡️ FALLBACK_LOGIC: If session hydration is lagging, use header data
  if (!userId || !session) {
    redirect("/login?reason=session_invalid");
  }

  // Ensure targetId is never undefined for the Async cards
  const targetId = session.isStaff ? null : (session.merchantId || userId);
  return (

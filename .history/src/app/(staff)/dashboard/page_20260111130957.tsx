...
import { /**
 * 🛰️ THE GATEKEEPERS (Universal Tier)
 * Centralized redirection logic for the entire Zipha V2 ecosystem.
 */

export async function requireAuth() {
  const session = await getSession();
  
  // If no session exists at all, they aren't even a Guest
  if (!session) {
...
export async function requireStaff() {
  ...
  // Barrier 1: Must be logged in
  if (!session) {
    redirect("/dashboard/login?reason=unauthorized");
  }

  // Barrier 2: Must have Staff clearance
  if (!session.isStaff) {
    redirect("/dashboard/login?reason=forbidden");
  }

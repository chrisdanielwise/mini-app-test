...
/**
 * 🌊 ANALYTICS_PAGE (Institutional Apex v2026.1.16)
 * Strategy: Defensive Ingress Logic with Restored Chroma Protocol.
 * Fix: Explicitly handles undefined 'stats' to prevent runtime crashes.
 */
export default function AnalyticsPage({ 
  stats = { activeSubscriptions: 0, pendingTickets: 0, analytics: { totalRevenue: 0, chartData: [] } }, // 🛡️ Default parameter guard
  categoryData = [] 
}: any) {
  ...
  // 🛡️ CRASH SHIELD: Safe evaluation of nested telemetry
  ...
  return (
    <div 
      className={cn(
        "max-w-[1600px] mx-auto transition-all duration-1000 ease-out",
        "animate-in fade-in slide-in-from-bottom-12",
        // 🧪 TACTICAL CLAMPING: Stabilizing vertical rhythm
    ...
    >
      {/* --- COMMAND HUD HEADER: Clamped Institutional Style --- */}
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 border-b border-white/5 pb-8 relative group">
          ...
          <div className="flex items-center gap-3 italic opacity-40">
            ...
            <div className="flex flex-col">
              <span className={cn(
                ...
                {isPlatformStaff ? "Global_Intelligence_Oversight" : "Node_Telemetry_Vector"}
              </span>
        ...
        {/* --- DYNAMIC STATUS NODE: Restricted Scale --- */}
        <div className={cn(
          ...
          isMobile && "w-full justify-between"
           ...
           <div className="text-right">
              ...
              <span className="text-[7px] font-mono opacity-20 uppercase tracking-widest mt-1 block">Latency: 24ms</span>
           </div>
           <ShieldCheck className={cn("size-5", isPlatformStaff ? "text-amber-500 animate-pulse" : "text-emerald-500")} />
        </div>
      </div>

      {/* --- METRIC GRID: Morphology-Aware Column Clamping --- */}
      <div className={cn("grid gap-4 md:gap-6 w-full", {
        ...
        <MetricNode title="Revenue" value={`$${Number(analytics.totalRevenue).toLocaleString()}`} icon={CreditCard} isAmber={isPlatformStaff} tier={screenSize} />
        <MetricNode title="Nodes" value={safeStats.activeSubscriptions} icon={Users} isAmber={isPlatformStaff} tier={screenSize} />
        <MetricNode title="Alerts" value={safeStats.pendingTickets} icon={Ticket} isWarning={safeStats.pendingTickets > 0} tier={screenSize} />
      ...
      <div className={cn("grid gap-6 md:gap-8", isMobile ? "grid-cols-1" : "grid-cols-1 xl:grid-cols-3")}>
        ...
        <div className={cn(
          ...
          <div className="flex items-center justify-between mb-8">
            <div className="flex items-center gap-4">
              ...
              <h2 className="text-lg md:text-xl font-black uppercase italic tracking-tighter">Growth_Velocity</h2>
            </div>
            <div className="flex items-center gap-3 opacity-10 italic">
              <Cpu className="size-3" />
              <span className="text-[7px] font-black uppercase tracking-[0.4em]">Mesh_Processing_Active</span>
            </div>
          </div>
        ...
        <div className={cn(
          ...
          <div className="flex items-center gap-4 mb-8">
            ...
            <h2 className="text-lg md:text-xl font-black uppercase italic tracking-tighter">Network_Spread</h2>
          </div>
      ...
      <div className="flex items-center justify-center gap-5 opacity-10 py-12 border-t border-white/5">
        ...
        <p className="text-[9px] font-black uppercase tracking-[0.5em] text-foreground italic">
          Intelligence_Core_Synchronized // Handshake_v16.31
        </p>
      </div>
    </div>
  );
...
function MetricNode({ title, value, icon: Icon, isAmber, isWarning, tier }: any) {
  ...
  return (
    <div className={cn(
      "rounded-[1.8rem] md:rounded-[2.2rem] border shadow-lg relative overflow-hidden group flex flex-col justify-between transition-all duration-700",
      isCompact ? "p-5 min-h-[120px]" : "p-8 min-h-[180px]",
      isAmber ? "border-amber-500/10 bg-amber-500/[0.02]" : "border-white/5 bg-white/[0.01]",
      ...
      <div className="space-y-1 md:space-y-2">
        ...
        <div className="flex items-center gap-2 opacity-10 italic">
          ...
          <span className="text-[7px] font-black uppercase tracking-[0.1em]">Telemetry_Live</span>
        </div>
      </div>

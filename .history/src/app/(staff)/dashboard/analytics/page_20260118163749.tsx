...
/**
 * 🛰️ ANALYTICS_PAGE (Institutional Apex v16.31.20 - HARDENED)
 * Strategy: Locked Viewport Chassis & Independent Scroll Reservoir.
 * Fix: Removed page-level scroll to lock Header + Added etched grid lines.
 */
export default function AnalyticsPage({ stats, categoryData }: any) {
  ...
  return (
    /* 🏛️ PRIMARY CHASSIS: Locked at 100% height. No page-level scroll. */
    <div className="absolute inset-0 flex flex-col min-w-0 overflow-hidden bg-black text-foreground">
      ...
      {/* --- 🚀 INTERNAL SCROLL VOLUME: flex-1 + min-h-0 triggers the reservoir --- */}
      <div className="flex-1 min-h-0 w-full relative overflow-y-auto custom-scrollbar overscroll-contain px-6 py-6 space-y-8">
        
        {/* --- METRIC GRID --- */}
        <div className={cn("grid gap-0 w-full rounded-2xl border border-white/5 overflow-hidden divide-x divide-y divide-white/5 shadow-2xl bg-white/[0.01]", {
          "grid-cols-1 divide-y divide-x-0": screenSize === 'xs',
        ...
        <div className={cn("grid gap-6", isMobile ? "grid-cols-1" : "grid-cols-1 xl:grid-cols-3")}>
          <div className={cn(
            ...
            isPlatformStaff ? "bg-amber-500/[0.01] border-amber-500/10" : "bg-card/40 border-white/5"
          )}>
            ...
            {/* ETCHED CHART BORDER */}
            <div className="w-full rounded-2xl border border-white/5 bg-black/20 p-4 shadow-inner" style={{ height: isMobile ? "240px" : "320px" }}>
          ...
          <div className={cn(
            ...
            <div className="flex-1 min-h-[200px] flex items-center justify-center relative bg-white/[0.01] rounded-2xl border border-white/5">
              <CategoryChart data={categoryData} />
            </div>
          </div>
        </div>

        {/* 🌫️ MOBILE CLEARANCE: Ensures chart footers clear the BottomNav */}
        {isMobile && (
          <div 
...
/** 🧪 ATOMIC COMPONENT: ETCHED METRIC NODE */
function MetricNode({ title, value, icon: Icon, isAmber, isWarning, hasTrend }: any) {
  return (
    <div className={cn(
      "relative overflow-hidden group flex flex-col justify-between transition-all duration-700 p-5 min-h-[110px]",
      "hover:bg-white/[0.03]",
      isWarning && "bg-rose-500/[0.04]"
    )}>
      <div className="flex justify-between items-start z-10">
        <p className={cn("text-[7px] font-black uppercase tracking-[0.3em] italic opacity-20", isAmber && "text-amber-500 opacity-40")}>
          {title}
        </p>
      ...
      <div className="space-y-0.5 z-10">
        <h3 className={cn(
          ...
          isWarning ? "text-rose-500" : "text-foreground"
        )}>
          {value}
        </h3>
        {hasTrend && (
      ...
      {/* RADIANCE EFFECT */}
      <div className={cn(
        "absolute -bottom-6 -right-6 size-24 blur-[40px] rounded-full pointer-events-none group-hover:scale-125 transition-all duration-1000 opacity-20",

...
import { getDashboardStats, getCategoryDistribution } from "@/lib/services/merchant.service";
import {
  TrendingUp,
  CreditCard,
  Activity,
...
/**
 * 🛰️ PERFORMANCE INTELLIGENCE (Institutional v9.3.7)
 * Fix: Null-safe Node_ID display for Super Admin compatibility.
 * Fix: Role-aware data ingress logic using session context.
 */
export default async function AnalyticsPage() {
  ...
  const realMerchantId = session.merchantId; // 🚀 Use the resolved merchantId from session
  ...
  // Staff see global aggregation (targetId is undefined)
  const targetId = isPlatformStaff ? undefined : realMerchantId;

  const [stats, categoryData] = await Promise.all([
    getDashboardStats(targetId),
    getCategoryDistribution(targetId),
  ]);
    ...
    <div className="max-w-7xl mx-auto space-y-6 md:space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500 pb-10 px-4 text-foreground">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-border/40 pb-6">
        <div className="space-y-1">
          ...
          <h1 className="text-2xl md:text-3xl lg:text-4xl font-black tracking-tight uppercase italic">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4 w-full">
        ...
        <div className="rounded-2xl border border-border/10 bg-card/40 p-5 shadow-sm min-h-[140px] flex flex-col justify-between">
          ...
          <h3 className="text-2xl md:text-3xl font-black tracking-tighter italic uppercase leading-none">
            {isPlatformStaff ? "SYSTEM" : "APEX"}<span className={cn("text-sm", isPlatformStaff ? "text-amber-500" : "text-primary")}>_V2</span>
          </h3>
          <p className="text-[8px] font-bold text-muted-foreground uppercase opacity-40 italic">Schema: Optimal</p>
        </div>
      </div>

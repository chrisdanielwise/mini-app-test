...
/**
 * 🛰️ TICKET RESOLUTION TERMINAL (Institutional v9.4.2)
 * Fix: Multi-tenant Data Isolation protocol.
 * Fix: RBAC-Aware response gating (Staff only).
 */
export default async function TicketResponsePage({
  ...
  if (!isPlatformStaff && ticket.merchantId !== realMerchantId) {
    ...
    <div className="max-w-6xl mx-auto space-y-6 md:space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500 pb-20 px-4 text-foreground">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
        ...
        <div className="space-y-6">
              ...
              <div className="space-y-6">
                 <div className="flex items-center gap-4">
                    ...
                    <div className="min-w-0">
                       ...
                       <div className="flex items-center gap-1.5 mt-1.5 opacity-30">
                          ...
                          <p className="text-[8px] font-mono font-bold tracking-widest uppercase leading-none truncate">
                            {ticket.merchantId ? `CLUSTER_${ticket.merchantId.slice(-6)}` : "GLOBAL_HUB"}
                          </p>
                       </div>
                    </div>
                 </div>
                 ...
                 <div className="pt-6 border-t border-border/10 space-y-3">
                    ...
                    <div className="flex justify-between items-center gap-4">
                       ...
                       <span className={cn(
                         "text-[9px] font-black uppercase italic tracking-widest truncate text-right",
           ...
           {!isPlatformStaff && (
              <div className="rounded-2xl border border-amber-500/20 bg-amber-500/5 p-6 space-y-2 shadow-lg">
                <div className="flex items-center gap-2 text-amber-500">
                   <AlertCircle className="h-3.5 w-3.5" />
                   <h4 className="text-[9px] font-black uppercase tracking-widest leading-none">Oversight Mode</h4>
                </div>
                <p className="text-[9px] font-bold text-muted-foreground leading-relaxed uppercase tracking-widest opacity-60 italic">
                  Resolution is managed by <span className="text-amber-500">Platform Hub</span>. Merchant nodes are in audit-only state.
                </p>
              </div>
           )}
        </div>
           ...
           <div className="rounded-2xl border border-border/40 bg-card/40 backdrop-blur-3xl overflow-hidden shadow-xl">
              <div className="p-6 border-b border-border/10 bg-muted/10">
                 ...
                 <div className="space-y-4">
                    ...
                    <div className="p-5 rounded-xl bg-background/50 border border-border/10 italic text-[11px] md:text-xs text-foreground/80 leading-relaxed shadow-inner">
                       {/* 🚀 SCHEMA SYNC: Ensure description field exists or fallback */}
                       {(ticket as any).description || "MANIFEST_EMPTY: No textual telemetry provided."}
                    </div>
                 </div>
              </div>
                 ...
                 {isPlatformStaff ? (
                   <>
                     ...
                     <div className="space-y-3">
                        <Textarea 
                           ...
                           className="min-h-[160px] md:min-h-[180px] rounded-xl border-border/40 bg-muted/10 p-5 font-bold text-[11px] md:text-xs leading-relaxed focus:ring-primary/20 transition-all resize-none shadow-inner text-foreground"
                        />
                        <div className="flex items-center justify-between gap-2 px-1">
                           ...
                           <div className="flex items-center gap-2">
                              ...
                              <span className="text-[8px] font-black uppercase tracking-widest text-primary/40 italic">Clearance: {role.toUpperCase()}</span>
                           </div>
                        </div>
                     </div>
                   ...
                   <div className="py-16 flex flex-col items-center justify-center text-center space-y-4 opacity-30">
                      ...
                      <p className="text-xs font-black uppercase tracking-[0.2em] italic max-w-xs leading-relaxed">
                         Terminal Locked: Access restricted to Platform Core nodes.
                      </p>
                   </div>
                 )}
              </div>

...
/**
 * 🛰️ TICKET RESOLUTION TERMINAL (Apex Tier)
 * Fixed: Identity isolation and proper relation mapping.
 */
export default async function TicketResponsePage({
  ...
  // Ensure the include matches your schema: 'user' is likely the customer.
  const ticket = await prisma.ticket.findUnique({
    where: { id: ticketId },
    include: {
      user: true,
      // If your schema doesn't have a direct 'merchant' relation on Ticket,
      // the error will trigger. Ensure you have 'merchantId' column.
    }
  });
  ...
  // Merchants can only view tickets belonging to their 'merchantId' cluster.
    ...
    <div className="max-w-6xl mx-auto space-y-6 md:space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500 pb-20 px-4">
              ...
              <div className="space-y-6">
                 <div className="flex items-center gap-4">
                    ...
                    <div className="min-w-0">
                       ...
                       <p className="text-[8px] font-mono font-bold opacity-30 tracking-widest mt-1 uppercase leading-none">
                         Origin_Cluster: {ticket.merchantId?.slice(0, 12) || "GLOBAL"}
                       </p>
                    </div>
                 </div>
           ...
           {!isPlatformStaff && (
              <div className="rounded-2xl border border-amber-500/20 bg-amber-500/5 p-6 space-y-2 shadow-lg">
                <div className="flex items-center gap-2 text-amber-500">
                   ...
                   <h4 className="text-[9px] font-black uppercase tracking-widest leading-none">Audit Mode</h4>
                </div>
                <p className="text-[9px] font-bold text-muted-foreground leading-relaxed uppercase tracking-widest opacity-60 italic">
                  Access restricted to <span className="text-amber-500">Read-Only</span>. Platform staff are managing this protocol.
                </p>
              </div>
           )}
        </div>
              ...
              <div className="p-6 border-b border-border/10 bg-muted/10">
                 ...
                 <div className="space-y-4">
                    ...
                    <div className="p-5 rounded-xl bg-background/50 border border-border/10 italic text-[11px] md:text-xs text-foreground/80 leading-relaxed shadow-inner">
                       {ticket.description || "MANIFEST_EMPTY: No textual data provided."}
                    </div>
                 </div>
              </div>
                 ...
                 {isPlatformStaff ? (
                     ...
                     <div className="space-y-3">
                        ...
                        <div className="flex items-center justify-between gap-2 px-1">
                           <p className="text-[8px] font-black uppercase tracking-widest text-muted-foreground opacity-20 italic">
                              Markdown and Signal formatting active.
                           </p>
                           <div className="flex items-center gap-2">
                              <ShieldCheck className="h-3 w-3 text-primary opacity-40" />
                              <span className="text-[8px] font-black uppercase tracking-widest text-primary/40 italic">Node Clearance: {role}</span>
                           </div>
                        </div>
                     </div>
                   ...
                   <div className="py-16 flex flex-col items-center justify-center text-center space-y-4 opacity-30">
                      ...
                      <p className="text-xs font-black uppercase tracking-[0.2em] italic max-w-xs leading-relaxed">
                         The terminal is locked for Merchant nodes. Oversight in progress.
                      </p>
                   </div>
                 )}
              </div>
      ...
      <div className="flex items-center justify-center gap-3 opacity-20 py-4">
         ...
         <p className="text-[8px] font-black uppercase tracking-[0.4em] text-muted-foreground italic text-center leading-none">
           Centralized Resolution Core synchronized // Node_ID: {ticket.id.slice(0, 12)}
         </p>
      </div>
    </div>
  );

...
/**
 * 🛰️ SUPPORT_CLIENT_PAGE (Institutional Apex v2026.1.20 - HARDENED)
    ...
    <div className="absolute inset-0 flex flex-col min-w-0 overflow-hidden bg-black text-foreground">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 px-6">
          <div className="space-y-1.5 min-w-0">
            ...
            <div className="flex items-center gap-2 text-[7px] font-black text-muted-foreground/20 italic uppercase tracking-[0.2em]">
              ...
              <span className="truncate leading-none">Identity: {role.toUpperCase()} // Signal: {initialTickets.length > 0 ? 'Active_Queue' : 'Idle'}</span>
            </div>
          </div>
          ...
          <div className="relative min-w-0 w-48 md:w-64 lg:w-80 group shrink-0 scale-90 origin-bottom-right">
             ...
             <Input 
               ...
               className="h-10 w-full pl-10 rounded-xl border-white/5 bg-white/5 font-black text-[9px] uppercase tracking-widest focus:ring-primary/20 placeholder:opacity-20"
             />
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 min-h-0 w-full relative flex flex-col p-3 pb-6">
            ...
            <table className="w-full text-left border-collapse min-w-[1000px] table-fixed relative">
              <thead className="bg-zinc-950 sticky top-0 z-20 border-b border-white/5 backdrop-blur-md">
                <tr className="divide-x divide-white/5">
                  ...
                  {isPlatformStaff && <th className="w-[18%] px-6 py-3 text-[7.5px] font-black uppercase tracking-[0.5em] text-amber-500/30 italic">Origin Cluster</th>}
                  <th className="w-[35%] px-6 py-3 text-[7.5px] font-black uppercase tracking-[0.5em] text-muted-foreground/30 italic">Manifest / Subject</th>
                  <th className="w-[17%] px-6 py-3 text-right text-[7.5px] font-black uppercase tracking-[0.5em] text-muted-foreground/30 italic">Protocol</th>
                </tr>
              </thead>
              ...
              <tbody className="divide-y divide-white/5">
                {initialTickets.length === 0 ? (
                  <tr>
                    <td colSpan={isPlatformStaff ? 4 : 3} className="py-24 text-center opacity-10">
        ...
        <div className="shrink-0 flex items-center justify-center gap-4 opacity-10 pt-4 pb-4">
          ...
          <p className="text-[7px] font-black uppercase tracking-[0.5em] text-foreground italic text-center">

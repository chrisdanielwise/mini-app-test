        ...
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-4 px-6">
          ...
          <div className="shrink-0 scale-90 origin-bottom-right">
            <Button
              ...
              variant="outline"
              className="h-9 px-4 border-white/5 bg-white/[0.02] text-[8px] font-black uppercase tracking-widest italic rounded-lg"
            >
      ...
      <div className="flex-1 min-h-0 w-full relative overflow-y-auto custom-scrollbar px-6 py-6">
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          ...
          <div className="xl:col-span-2 space-y-4">
            {safeServices.length === 0 ? (
              <div className="rounded-[2rem] border border-dashed border-white/5 bg-black/20 p-16 text-center opacity-20">
                <p className="text-[10px] font-black uppercase italic tracking-[0.5em]">Zero_Signal_Nodes</p>
              </div>
            ) : (
              safeServices.map((service: any) => (
                <div
                  key={service.id}
                  className={cn(
                    "group relative overflow-hidden rounded-[2rem] border backdrop-blur-3xl shadow-2xl transition-all duration-500",
                  ...
                  <div className="p-6 md:p-8 space-y-8">
                    <div className="flex justify-between items-start gap-4">
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          ...
                          <p className="text-[8px] font-black uppercase tracking-widest text-primary/40 italic">
                            NODE: {service.id.slice(0, 8)} // {service.categoryTag || "GENERAL"}
                          </p>
                        </div>
                        <h3 className="text-lg md:text-xl font-black uppercase italic tracking-tighter text-foreground group-hover:text-primary transition-colors">
                          {service.name}
                        </h3>
                      </div>
                      <Badge className="rounded-lg bg-emerald-500/5 text-emerald-500 border-emerald-500/10 text-[7px] font-black uppercase px-2 py-0.5">
                        ACTIVE
                      </Badge>
                    </div>
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex gap-2">
                        <div className="flex-1 bg-black/60 border border-white/5 rounded-xl px-4 h-10 flex items-center font-mono text-[9px] text-emerald-500/60 truncate italic shadow-inner">
                        ...
                        <Button
                          ...
                          variant="ghost" size="icon" className="rounded-xl border border-white/5 bg-white/[0.02] h-10 w-10 shrink-0 hover:bg-primary"
                        >
                    ...
                    <div className="space-y-2">
                      ...
                      <pre className="bg-zinc-950 p-5 rounded-xl border border-white/5 font-mono text-[9px] leading-relaxed text-zinc-500 shadow-inner">
          ...
          <div className="space-y-6">
            <div className={cn(
              "rounded-[2rem] p-6 text-primary-foreground shadow-2xl relative overflow-hidden group transition-all",
              isPlatformStaff ? "bg-amber-500" : "bg-primary"
            )}>
              <h3 className="text-xl font-black uppercase italic tracking-tighter mb-2">Node <span className="opacity-40">Manual</span></h3>
              <p className="text-[8px] font-bold uppercase tracking-[0.1em] opacity-80 mb-6 italic">Integrate alerts directly with Zipha nodes.</p>
            ...
            <div className="rounded-[1.8rem] border border-white/5 bg-black/40 p-6 backdrop-blur-3xl shadow-xl space-y-6">
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-3 opacity-40">
                  ...
                  <span className="text-[8px] font-black uppercase tracking-widest italic">TLS_Verified</span>
                </div>
                <div className="flex items-center gap-3 cursor-pointer group" onClick={() => impact("medium")}>
                  <RefreshCcw className="size-4 text-primary group-hover:rotate-180 transition-transform duration-700" />
                  <span className="text-[8px] font-black uppercase tracking-widest text-foreground/30 italic group-hover:text-foreground">Rotate_Keys</span>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="flex items-center justify-center gap-4 opacity-10 pt-8 pb-12">
          ...
          <p className="text-[7px] font-black uppercase tracking-[0.5em] italic">
            Handshake Sync: Stable // Session: {role}
          </p>
        </div>
      </div>
    </div>

 ...
 * Fixed: 'INVALID_NODE' UUID syntax error by implementing dynamic where-filtering.
 * Fixed: Safe session access for botUsername to prevent staff-role crashes.
 */
export default async function BotConfigPage() {
  ...
  // ✅ SAFE DATA NORMALIZATION: Safely access session.merchant via optional chaining
  const activeBotNode = session.merchant?.botUsername || "SYSTEM_GLOBAL_CORE";

  // 🏁 DATA INGRESS: Dynamic filtering
  // Logic: Staff fetch all services (empty filter); Merchants fetch their specific cluster.
  // This avoids passing "INVALID_NODE" to a UUID field.
  const services = await prisma.service.findMany({
    where: isPlatformStaff ? {} : { merchantId: merchantId as string },

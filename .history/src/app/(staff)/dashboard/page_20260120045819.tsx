...
export default async function DashboardPage() {
  ...
  const targetDataId = isPlatformStaff
    ? null
    : session?.user?.merchantId || userId;

  return (
    <DashboardClientView session={session}>
      {/* 🚀 THE RESERVOIR: This div fixes the "Shrink" and "Empty Feed" issues.
          🏁 THE FIX: 'flex-1' takes the space, 'overflow-y-auto' enables scrolling.
      */}
      <div className="flex-1 w-full overflow-y-auto custom-scrollbar flex flex-col min-h-0 overscroll-contain">
        <div className="px-4 md:px-6 py-6 space-y-8 flex-1">
          {/* 📊 RESTORED: BOLD TACTICAL STATS LEDGER */}
...
function StatusNode({ title, status, sub, isAmber }: any) {
  return (
    ...
    >
      <p
        className={cn(
          "text-[7px] font-black uppercase tracking-[0.3em] italic opacity-30",
          isAmber && "text-amber-500 opacity-50",
        )}
      >
        {title}
      </p>
      <div className="space-y-1 mt-6">
        <p
          className={cn(
            "text-2xl font-black italic uppercase leading-none tracking-tighter tabular-nums",
            isAmber ? "text-foreground" : "text-emerald-500",
          )}
        >
          {status}
        </p>
...
function StatsCardSkeleton() {
  return (
    <div className="min-h-[140px] rounded-[1.8rem] bg-white/[0.01] border border-white/5 animate-pulse" />
  );
}
function FeedSkeleton() {
  return (
    <div className="h-[400px] rounded-[2.5rem] bg-white/[0.01] animate-pulse border border-white/5" />
  );
}

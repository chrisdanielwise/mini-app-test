 ...
 * Fixed: Aligned with PayoutRequest model name and destination fields.
 * Optimized: High-density industrial scale for institutional liquidity audits.
 */
export default async function PayoutsPage() {
  const session = await getSession();
  const realMerchantId = session.merchantId;

  // 🏁 Data Fetch: Using the correct schema model 'payoutRequest'
  let payouts = [];
  try {
    ...
    console.error("PRISMA_SYNC_ERROR: Handshake failed with PayoutRequest model.");
  }
    ...
    <div className="max-w-7xl mx-auto space-y-6 md:space-y-8 animate-in fade-in duration-500 pb-20 px-4 md:px-8">
        ...
        <div className="flex flex-col gap-3 min-w-fit flex-1">
          ...
          <div className="space-y-1">
            ...
            <p className="text-[9px] md:text-[10px] font-bold text-muted-foreground uppercase tracking-widest opacity-40 italic">
              Validated Cycles: <span className="text-foreground tabular-nums">{payouts.length}</span> // Schema_v2.0.0
            </p>
          </div>
        </div>
          ...
          <Table className="w-full text-left border-collapse min-w-[850px]">
            <TableHeader className="bg-muted/30 border-b border-border/10">
              <TableRow className="hover:bg-transparent border-none"> 
                ...
                <TableHead className="h-12 px-6 text-[9px] font-black uppercase tracking-widest text-muted-foreground/60 leading-none">Destination</TableHead>
              ...
              {payouts.length === 0 ? (
                ...
                payouts.map((payout) => (
                  <TableRow key={payout.id} className="hover:bg-primary/[0.02] transition-colors group border-none">
                    ...
                    {/* Destination Node: Fixed fallback logic */}
                    <TableCell className="px-6 py-4">
                    ...
                    {/* Protocol Status: Using PayoutStatus Enum */}
      ...
      <div className="flex items-center justify-center gap-3 opacity-20 py-4">
         ...
         <p className="text-[8px] font-black uppercase tracking-[0.3em] text-muted-foreground italic text-center leading-none">
           Audit Core synchronized // Node_ID: {realMerchantId.slice(0, 8)}
         </p>
      </div>
    </div>
  );

...
import { SubscriberSearch } from "@/components/dashboard/subscriber-search"; // ✅ Search Node Integrated
import { cn } from "@/lib/utils";
import { format } from "date-fns";

/**
 * 🛰️ SUBSCRIBER IDENTITY NODE (Deep View)
 * Integrated: SubscriberSearch for rapid node pivoting.
      ...
      <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between border-b border-border/40 pb-8 lg:pb-10">
        <div className="space-y-3 lg:space-y-4 flex-1">
          <Link
            ...
            className="group inline-flex items-center gap-2 text-[9px] font-black uppercase tracking-[0.3em] text-muted-foreground hover:text-primary transition-all mb-2"
          >
            <ArrowLeft className="h-3 w-3 group-hover:-translate-x-1 transition-transform" />
            Back to User Ledger
          </Link>
          <div className="flex flex-wrap items-center gap-4 lg:gap-6">
            <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-black tracking-tighter uppercase italic leading-none truncate max-w-full">
        ...
        {/* 🔍 RAPID PIVOT SEARCH NODE */}
        <div className="flex flex-col sm:flex-row items-center gap-3 w-full lg:w-auto">
          <SubscriberSearch />
          <div className="flex gap-2 w-full sm:w-auto">
            <Button variant="outline" className="flex-1 lg:flex-none rounded-xl h-12 lg:h-14 px-6 border-border/40 bg-muted/10 font-black uppercase text-[10px] tracking-widest">
              Update Expiry
            </Button>
            <Button className="flex-1 lg:flex-none rounded-xl h-12 lg:h-14 px-6 bg-rose-500 text-white font-black uppercase italic tracking-widest text-[10px] shadow-2xl shadow-rose-500/20">
          ...
          <section className="rounded-3xl border border-border/40 bg-card/40 p-5 sm:p-8 backdrop-blur-3xl shadow-2xl relative overflow-hidden">
             ...
             <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 lg:gap-10">
                ...
                <div className="space-y-1">
                   ...
                   <p className="text-xl lg:text-2xl font-black italic uppercase tracking-tighter text-primary truncate">{subscription.tier.name}</p>
                </div>
                <div className="space-y-1">
                   <p className="text-[8px] font-black uppercase text-muted-foreground opacity-40 tracking-widest">Identity ID (Telegram)</p>
                   <p className="text-base lg:text-lg font-mono font-black tracking-widest opacity-80 truncate">{subscription.user.id.toString()}</p>
                </div>
                <div className="space-y-1">
                   <p className="text-[8px] font-black uppercase text-muted-foreground opacity-40 tracking-widest">Protocol Start</p>
                   <p className="text-base lg:text-lg font-black italic uppercase tracking-tighter">
             ...
             <div className="mt-8 lg:mt-12 p-5 sm:p-6 rounded-2xl bg-muted/10 border border-border/40 flex flex-col sm:flex-row items-center justify-between gap-4">
                <div className="flex items-center gap-4">
                   ...
                   <div>
                      ...
                      <p className="text-base lg:text-lg font-black uppercase italic tracking-tighter mt-1">
                        {subscription.expiresAt ? format(new Date(subscription.expiresAt), "dd MMMM yyyy") : "LIFETIME_PROTOCOL"}
                      </p>
                   </div>
                </div>
                <Badge variant="outline" className="rounded-lg h-8 px-3 bg-background text-[9px] font-black uppercase italic tracking-widest">
              ...
              <div className="relative z-10">
                 ...
                 <div className="mt-6 sm:mt-8 pt-6 sm:pt-8 border-t border-primary/10 space-y-4">
                    <div className="flex justify-between items-center gap-4">
                       ...
                       <span className="text-[10px] font-black uppercase italic tracking-widest text-right">{subscription.tier.interval} Renewal</span>
                    </div>
                    <div className="flex justify-between items-center gap-4">

...
import { 
  ...
  Zap, Trash2, Calendar, ExternalLink, History, Globe, Activity
} from "lucide-react";
...
/**
 * 🛰️ SUBSCRIBER_CLIENT_VIEW (Institutional Apex v2026.1.20)
 * Strategy: Stationary HUD & Independent Tactical Scroll.
 * Fix: High-density geometry prevents header cropping and maximizes telemetry visibility.
 */
export default function SubscriberClientView({ subscription, isSuperAdmin, id }: any) {
  ...
  return (
    <div className="w-full h-full flex flex-col min-w-0 overflow-hidden text-foreground bg-black">
        ...
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-4 px-6">
          <div className="space-y-1.5 min-w-0">
            <Link
              ...
              className="group inline-flex items-center gap-2 text-[8px] font-black uppercase tracking-[0.2em] text-muted-foreground/30 hover:text-primary transition-all italic"
            >
              <ArrowLeft className="size-2.5 group-hover:-translate-x-1" /> NODE_LEDGER
            </Link>
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <Badge className={cn(
                "rounded-lg text-[7px] font-black uppercase tracking-[0.2em] px-2 py-0.5 border shadow-sm italic",
      ...
      {/* --- 🚀 INTERNAL SCROLL VOLUME: Independent Telemetry Nodes --- */}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <section className={cn(
              "rounded-[2rem] border p-6 md:p-8 backdrop-blur-3xl shadow-2xl relative overflow-hidden group",
              isSuperAdmin ? "bg-amber-500/[0.01] border-amber-500/10" : "bg-card/30 border-white/5"
               ...
               <div className="flex items-center gap-2.5 text-primary/40 mb-6 opacity-30">
               ...
               <div className="mt-8 p-4 rounded-2xl bg-white/[0.01] border border-white/5 flex items-center justify-between gap-4">
                  <div className="flex items-center gap-3">
                     <Calendar className="size-4 text-muted-foreground/20" />
                     <div className="leading-tight">
                        <p className="text-[7px] font-black uppercase tracking-[0.1em] text-muted-foreground/20 italic">Expiration_Horizon</p>
            ...
            <section className="space-y-3">
              ...
              <div className="rounded-[1.8rem] border border-white/5 bg-white/[0.01] overflow-hidden backdrop-blur-3xl">
                 <div className="overflow-x-auto scrollbar-hide">
                    <table className="w-full text-left min-w-[500px] table-fixed">
                       <thead className="bg-white/[0.03] border-b border-white/5">
                          <tr>
                             <th className="w-[30%] px-6 py-2.5 text-[7px] font-black uppercase tracking-[0.2em] text-muted-foreground/20 italic">Liquidity</th>
                             <th className="w-[20%] px-6 py-2.5 text-[7px] font-black uppercase tracking-[0.2em] text-muted-foreground/20 italic">Protocol</th>
                             <th className="w-[30%] px-6 py-2.5 text-[7px] font-black uppercase tracking-[0.2em] text-muted-foreground/20 italic">Status</th>
                             <th className="w-[20%] px-6 py-2.5"></th>
                          </tr>
                       </thead>
                       <tbody className="divide-y divide-white/5 text-foreground">
                          {subscription.payments.map((p: any) => (
                             <tr key={p.id} onMouseEnter={() => impact("light")} className="hover:bg-white/[0.01] transition-colors border-none group">
                                <td className="px-6 py-3 font-black text-sm italic tracking-tighter tabular-nums leading-none">${Number(p.amount).toFixed(2)}</td>
                                <td className="px-6 py-3 text-[9px] font-bold uppercase opacity-30 italic">{p.currency}</td>
                                ...
                                <td className="px-6 py-3 text-right">
                                   <Button variant="ghost" size="icon" className="h-7 w-7 rounded-lg border border-white/5 bg-white/5 scale-90">
          ...
          <div className="space-y-6">
              <div className={cn(
                "rounded-[1.8rem] border p-6 md:p-8 shadow-2xl relative overflow-hidden group",
                isSuperAdmin ? "bg-amber-500/[0.02] border-amber-500/20 shadow-amber-500/5" : "bg-primary/[0.02] border-primary/20 shadow-primary/5"
              )}>
              ...
              <div className="rounded-[1.5rem] border border-white/5 bg-white/[0.01] p-6 space-y-3 shadow-xl opacity-30 italic">

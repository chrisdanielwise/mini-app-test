...
export default async function SubscribersPage() {
  ...
  return (
    ...
    /**
 * 🛰️ COMMAND HUD HEADER (Sync Tier)
 * Fixed: intrinsic flex-wrap logic to prevent horizontal cropping.
 * Optimized: Adaptive typographic scale for zero-bleed rendering.
 */
<div className="flex flex-wrap items-start justify-between gap-x-6 gap-y-8 w-full px-1">
  
  {/* Branding & Title Cluster: intrinsic width, wraps on collision */}
  <div className="flex flex-col gap-4 min-w-fit flex-1">
    <div className="flex items-center gap-3 text-primary">
      <div className="h-7 w-7 rounded-xl bg-primary/10 flex items-center justify-center border border-primary/20 shrink-0">
        <UserPlus className="h-4 w-4 fill-primary" />
      </div>
      <span className="text-[9px] font-black uppercase tracking-[0.4em] italic opacity-80 whitespace-nowrap">
        Network Identity Hub
      </span>
    </div>
    
    <div className="space-y-1">
      {/* FIX: Removed truncate, added whitespace-normal.
         Text size is capped to ensure it doesn't "break" the viewport on mobile.
      */}
  ...
  {/* Utility Actions Cluster: Intrinsic scaling, no fixed widths */}
  <div className="flex flex-col sm:flex-row items-stretch gap-3 w-full lg:w-auto shrink-0 lg:mt-auto">
    ...
    <div className="flex gap-2 h-11 md:h-13">
      <Button variant="outline" className="flex-1 lg:flex-none h-full px-4 rounded-2xl border-border/40 bg-muted/10 text-[9px] md:text-[10px] font-black uppercase tracking-widest hover:bg-primary/5 hover:text-primary transition-all">
        <Filter className="mr-2 h-3.5 w-3.5" /> Filter
      </Button>
      <Button className="flex-1 lg:flex-none h-full px-5 rounded-2xl font-black uppercase tracking-widest text-[9px] md:text-[10px] shadow-xl shadow-primary/10 bg-primary text-primary-foreground active:scale-95 transition-all">

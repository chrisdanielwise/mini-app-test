    ...
    <div className="space-y-8 md:space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-1000">
      ...
      <div className="flex flex-col gap-8 xl:flex-row xl:items-end xl:justify-between">
        <div className="space-y-3">
          ...
          <p className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.3em] mt-2 opacity-40">
            Total Validated Identities: <span className="text-foreground">{subscriptions.length}</span>
          </p>
        </div>

        {/* Global Utility Actions - Optimized for Mobile & Tablet Width */}
        <div className="flex flex-col sm:flex-row gap-3 w-full xl:w-auto">
          <div className="relative flex-1 xl:w-80 group">
            ...
            <Input 
              ...
              className="h-12 md:h-14 w-full pl-12 rounded-2xl border-border/40 bg-card/40 font-black text-[10px] uppercase tracking-[0.2em] focus:ring-primary/20 transition-all placeholder:opacity-20"
            />
          </div>
          <div className="flex gap-2 w-full sm:w-auto">
            <Button variant="outline" className="flex-1 sm:flex-none h-12 md:h-14 px-6 rounded-2xl border-border/40 bg-muted/10 text-[10px] font-black uppercase tracking-widest hover:bg-muted/20">
              <Filter className="mr-2 h-4 w-4" /> Filter
            </Button>
            <Button className="flex-1 sm:flex-none h-12 md:h-14 px-6 rounded-2xl font-black uppercase tracking-widest text-[10px] shadow-2xl shadow-primary/20 bg-primary text-primary-foreground">
            ...
            <thead>
              <tr className="bg-muted/30 border-b border-border/10">
                {["Identity Node", "Service // Tier", "Protocol Status", "Expiry Horizon", ""].map((head) => (
                  <th key={head} className="px-6 py-6 text-[10px] font-black uppercase tracking-[0.3em] text-muted-foreground/60">
                    {head}
                  </th>
                ))}
              </tr>
                  ...
                  <tr key={sub.id} className="hover:bg-primary/[0.02] transition-all duration-500 group cursor-default">
                    ...
                    <td className="px-6 py-8">
                      <div className="flex items-center gap-5">
                        <div className="h-12 w-12 md:h-14 md:w-14 rounded-2xl bg-primary/10 flex items-center justify-center text-primary font-black italic shadow-inner group-hover:scale-105 transition-transform border border-primary/20">
                          {(sub.user.username || "U")[0].toUpperCase()}
                        </div>
                        <div className="flex flex-col min-w-0">
                          <p className="font-black text-foreground uppercase italic tracking-tighter text-xl md:text-2xl group-hover:text-primary transition-colors leading-none truncate">
                            @{sub.user.username || 'unknown_node'}
                          </p>
                          <p className="text-[9px] font-mono font-bold text-muted-foreground uppercase opacity-30 tracking-widest mt-1">
                    ...
                    <td className="px-6 py-8 text-right">
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                          <Button variant="ghost" size="icon" className="h-10 w-10 md:h-12 md:w-12 rounded-2xl bg-muted/20 border border-border/40 hover:bg-primary hover:text-white transition-all">
                            <MoreHorizontal className="h-5 w-5" />
                          </Button>
                        </DropdownMenuTrigger>
                        <DropdownMenuContent align="end" className="w-[240px] rounded-2xl border-border/40 bg-card/95 backdrop-blur-3xl p-3 shadow-2xl z-[100] animate-in zoom-in-95 duration-300">
                          <div className="px-3 py-2 mb-2 border-b border-border/20">
                             <p className="text-[8px] font-black uppercase tracking-[0.3em] text-muted-foreground opacity-40">Identity Ops</p>
                          </div>
                          <DropdownMenuItem asChild className="rounded-xl py-3 px-4 cursor-pointer focus:bg-primary/10 group">
                          ...
                          <DropdownMenuItem className="rounded-xl py-3 px-4 cursor-pointer text-rose-500 focus:bg-rose-500/10 focus:text-rose-500">
      ...
      <div className="flex items-center gap-3 px-6 opacity-20">
         ...
         <p className="text-[8px] font-black uppercase tracking-[0.3em] text-muted-foreground italic">
           All identity nodes are subject to audit protocols.
         </p>
      </div>
    </div>
  );

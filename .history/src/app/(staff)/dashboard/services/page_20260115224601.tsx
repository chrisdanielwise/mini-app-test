 ...
 * Fix: Defensive session.config evaluation prevents Super Admin crashes.
 */
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8 border-b border-white/5 pb-10 relative group">
        <div className="space-y-6">
          ...
          <div className="flex items-center gap-4 text-[10px] font-black text-muted-foreground/30 italic">
            ...
            {/* ✅ FIXED: Defensive optional chaining prevents object evaluation crash */}
            <span className="tracking-[0.2em]">NODE: {session?.config?.companyName || "PLATFORM_ROOT"}</span>
            <div className="h-px w-8 bg-white/5" />
            <span className="tracking-[0.1em]">{services?.length || 0} Nodes Hydrated</span>
          </div>
        </div>
          ...
          <div className="overflow-x-auto scrollbar-hide">
            <table className="w-full text-left border-collapse min-w-[1000px]">
              ...
              <tbody className="divide-y divide-white/5">
                {!services || services.length === 0 ? <tr><td colSpan={6}><NoData /></td></tr> : services.map((service: any, index: number) => (
                  <tr 
                    key={service.id} 
...
/** 📱 MOBILE ATOMIC COMPONENTS (Truncated for brevity) */
function ServiceCard({ service, isPlatformStaff, index, onClick }: any) {
  return (
    <div 
      onClick={onClick}

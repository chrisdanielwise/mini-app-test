...
export default function ServicesClientPage({ 
  ...
  role = "user", // 🛡️ Safe default prevents .toUpperCase() crash
  realMerchantId, 
  ...
  // 🛡️ HYDRATION SHIELD: Prevents the "snap" that causes content to jump/crop
  if (!isReady) return (
  ...
  return (
    <div 
      // 🛡️ FIX: Changed max-width and added min-w-0 to the master container
      className="w-full max-w-[1600px] mx-auto space-y-8 md:space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700 min-w-0"
      style={{ 
    ...
    >
      {/* --- COMMAND HUD HEADER --- */}
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8 border-b border-white/5 pb-8 min-w-0">
        <div className="space-y-4 flex-1 min-w-0"> {/* 🛡️ min-w-0 allows the title to shrink/truncate */}
          ...
          {/* 🛡️ FIX: Clamped typography with truncation to prevent header overflow */}
          <h1 className="text-4xl md:text-6xl font-black tracking-tighter uppercase italic leading-[0.85] text-foreground truncate max-w-full">
          ...
          <div className="flex flex-wrap items-center gap-x-6 gap-y-2 pt-2 opacity-30 italic">
            ...
            <p className="text-[10px] font-black uppercase tracking-widest flex items-center gap-2">
              ...
              {/* 🛡️ FIX: Safe evaluation of role prevents the crash */}
              Identity: <span className="text-foreground uppercase">{role?.toUpperCase() || "NULL"}</span>
            </p>
          </div>
        </div>
      ...
      {/* --- FLEET DATA GRID --- */}
      <div className={cn(
        "rounded-[2.5rem] border backdrop-blur-3xl overflow-hidden shadow-2xl relative z-10 w-full min-w-0",
        isPlatformStaff ? "bg-amber-500/[0.01] border-amber-500/10" : "bg-black/40 border-white/5"
      )}>
        {/* 🛡️ FIX: Outer wrapper with overflow-x-auto allows table scrolling instead of cropping */}
        <div className="overflow-x-auto scrollbar-hide w-full">
          {/* 🛡️ FIX: Removed fixed table alg to allow columns to naturally fit the min-width */}
              ...
              {initialServices.length === 0 ? (
                ...
                initialServices.map((service: any) => (
                  ...
                  >
                    ...
                    {isPlatformStaff && (
                      <td className="px-10 py-8">
                        <div className="flex items-center gap-3 text-amber-500/60 font-black uppercase italic text-[11px] tracking-widest leading-none truncate max-w-[200px]">

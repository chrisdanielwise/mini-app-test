    ...
    <div className="space-y-12 p-4 sm:p-8 pb-32 animate-in fade-in slide-in-from-bottom-4 duration-700 max-w-7xl mx-auto">
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap items-center gap-4">
            ...
            <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-primary/10 border border-primary/20">
              ...
              <span className="text-[10px] font-black uppercase tracking-widest text-primary">Node Active</span>
            </div>
          </div>
          <p className="text-[11px] font-bold text-muted-foreground uppercase tracking-[0.2em] mt-4">
            Type: {service.categoryTag || "General Signal"} • UUID: <span className="text-foreground">{service.id.slice(0, 12)}</span>
          </p>
        </div>
      ...
      <div className={cn(
        ...
        <div className="absolute top-0 right-0 p-12 opacity-[0.03] pointer-events-none">
          {service.vipChannelId ? <ShieldCheck className="h-64 w-64" /> : <ShieldAlert className="h-64 w-64" />}
        </div>
        ...
        <div className="relative z-10 grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div className="space-y-6">
            <h4 className={cn(
              ...
              {service.vipChannelId ? "Connection Encrypted" : "Connection Pending"}
            </h4>
            
            <div className="space-y-4">
               <Label className="text-[10px] font-black uppercase opacity-60">Target Telegram Node ID</Label>
               <div className="flex items-center gap-3">
                  ...
                  {!service.vipChannelId && (
                    <Badge className="bg-amber-500 text-[8px] font-black uppercase animate-bounce">Required</Badge>
                  )}
               </div>
               <p className="text-[10px] font-bold text-muted-foreground uppercase leading-relaxed max-w-sm">
                 Automation will trigger invites to this ID upon successful payment verification.
               </p>
            </div>
          </div>
          ...
          <div className={cn(
            ...
            service.vipChannelId ? "bg-emerald-500/10 border-emerald-500/20" : "bg-muted/40 border-border/40"
          )}>
            <div className={cn(
                "h-16 w-16 rounded-2xl flex items-center justify-center shadow-lg",
                service.vipChannelId ? "bg-emerald-500/20 text-emerald-500" : "bg-muted text-muted-foreground"
            )}>
            ...
            <div>
              <p className={cn(
                ...
                service.vipChannelId ? "text-emerald-500" : "text-muted-foreground"
              )}>
                {service.vipChannelId ? "Signal Verified" : "System Offline"}
              </p>
              <p className="text-[10px] font-bold text-muted-foreground uppercase mt-1">
                {service.vipChannelId ? "Handshake protocol completed." : "Verify Bot Admin rights."}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="space-y-8">
        <div className="flex items-center gap-3 px-2">
            ...
            <h2 className="text-[12px] font-black uppercase tracking-[0.3em] text-foreground">Pricing Nodes</h2>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
            ...
            <div className="col-span-full py-32 text-center border-2 border-dashed border-border/40 rounded-[3.5rem] bg-muted/5 backdrop-blur-sm">
                ...
                <p className="text-sm font-black uppercase italic text-muted-foreground tracking-widest">No Active Revenue Tiers</p>
                <p className="text-[10px] font-bold uppercase text-muted-foreground/50 mt-3 tracking-widest">Initialize a tier to begin user onboarding</p>
            </div>
            ) : (
            service.tiers.map((tier) => (
                ...
                className={cn(
                    ...
                    tier.isActive ? "border-border/60" : "border-dashed border-border/40 opacity-50 grayscale"
                )}
                >
                ...
                <div className="flex items-baseline gap-2 mb-12 border-b border-border/40 pb-10">
                    <span className="text-5xl font-black tracking-tighter italic text-foreground">${Number(tier.price).toFixed(2)}</span>
                ...
                <div className="grid grid-cols-1 gap-4">
                    ...
                    <Button variant="ghost" className="w-full rounded-2xl font-black uppercase text-[10px] text-rose-500/60 hover:text-rose-500 hover:bg-rose-500/5 h-14 transition-colors">

 ...
 * Strategy: Internalized Scroll Volume.
 * Fix: Forced the table into an isolated overflow container. This stops the table 
 * from physically pushing the page width and cropping the headers.
 */
export default function ServicesClientPage({ 
  ...
  return (
    <div 
      // 🛡️ FIX: 'max-w-full' and 'min-w-0' are mandatory to prevent horizontal blowout
      className="w-full max-w-full mx-auto space-y-6 md:space-y-8 animate-in fade-in duration-700 min-w-0 overflow-hidden"
      style={{ 
    ...
    >
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 border-b border-white/5 pb-6 min-w-0">
        <div className="space-y-3 flex-1 min-w-0"> 
          ...
          {/* 🛡️ FIX: Added 'truncate' to the H1 to prevent long titles from pushing the page width */}
      ...
      {/* --- FLEET DATA GRID --- */}
      <div className={cn(
        "rounded-[2.2rem] border backdrop-blur-3xl shadow-2xl relative z-10 w-full min-w-0 overflow-hidden",
        isPlatformStaff ? "bg-amber-500/[0.01] border-amber-500/10" : "bg-black/40 border-white/5"
      )}>
        {/* 🛡️ CRITICAL FIX: The table now scrolls INTERNALLY. It cannot push the page width anymore */}
                ...
                <tr key={service.id} className="hover:bg-white/[0.02] transition-colors border-none group">
                  <td className="px-8 py-6">
                    <div className="flex items-center gap-4 min-w-0">
                      <div className={cn(
                        "size-10 shrink-0 rounded-xl border flex items-center justify-center transition-transform group-hover:rotate-12",
                  ...
                  <td className="px-8 py-6">
                    <div className={cn(
                      "inline-flex items-center rounded-lg px-2.5 py-1 text-[7px] font-black uppercase border",

...
export default function ServicesClientPage({ 
  initialServices = [], isPlatformStaff, role, realMerchantId, companyName 
}: any) {
    ...
    >
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 border-b border-white/5 pb-8 relative group">
        <div className="space-y-4 flex-1 min-w-0">
          ...
          <h1 className="text-[clamp(2.5rem,8vw,4.5rem)] font-black tracking-tighter uppercase italic leading-[0.85] text-foreground truncate">
          ...
          <div className="flex flex-wrap items-center gap-x-6 gap-y-2 pt-2 opacity-30 italic">
            <p className="text-[10px] font-black uppercase tracking-widest flex items-center gap-2">
              ...
              Node: <span className="text-foreground">{companyName || "PLATFORM_ROOT"}</span>
            </p>
            <p className="text-[10px] font-black uppercase tracking-widest flex items-center gap-2">
              <Terminal className="size-3.5" />
              Identity: <span className="text-foreground">{role.toUpperCase()}</span>
            </p>
          </div>
        </div>

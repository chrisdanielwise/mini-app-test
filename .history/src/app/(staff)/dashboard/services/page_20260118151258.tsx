...
/**
 * 🛰️ SERVICE FLEET COMMAND (Institutional Apex v2026.1.18)
 * Strategy: Rigid Chassis & Independent Reservoir.
 * Fix: Stationed HUD Header + Internal Sticky Table Scroll.
 */
export default async function ServicesPage() {
  ...
  return (
    ...
    <div className="w-full h-full flex flex-col min-w-0 overflow-hidden text-foreground bg-black">
      ...
      {/* --- 🛡️ FIXED COMMAND HUD: Stationary Layer (shrink-0 ensures it stays put) --- */}
            ...
            <div className="space-y-0.5">
              ...
              <div className="flex items-center gap-x-4 pt-1 opacity-20 italic">
                <p className="text-[8px] font-black uppercase tracking-widest flex items-center gap-1.5">
                  <Globe className="h-3 w-3" /> Node: <span className="text-foreground">{session.config?.companyName || "PLATFORM_ROOT"}</span>
                </p>
              </div>
            </div>
          </div>
        ...
        <div className={cn(
          ...
          <div className="flex-1 w-full overflow-auto custom-scrollbar overscroll-contain">
            <table className="w-full text-left border-collapse min-w-[900px] table-fixed relative">
              ...
              {/* 🏁 THE LINES: divide-y creates the row separation you like */}
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-4 min-w-0">
                        ...
                        <div className="flex flex-col min-w-0 leading-tight">
                          ...
                          <p className="text-[6px] font-mono text-muted-foreground/20 mt-0.5 uppercase">ID: {service.id.slice(0, 8).toUpperCase()}</p>
                        </div>
                      </div>
                    </td>
                    {isPlatformStaff && (

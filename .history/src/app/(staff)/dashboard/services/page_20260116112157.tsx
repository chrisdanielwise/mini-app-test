...
/**
 * 🛰️ SERVICE FLEET COMMAND (Institutional v16.1.20)
 * Strategy: High-Density Terminal Compression.
 * Fix: Reduced padding from px-10 to px-6 and py-8 to py-3 for minimalist density.
 * Fix: Scaled down header title to prevent "bogus" oversized appearance.
 */
export default async function ServicesPage() {
  ...
  return (
    <div className="w-full h-full flex flex-col min-w-0 overflow-hidden text-foreground">
      ...
      {/* --- 🛡️ MINIMALIST COMMAND HUD HEADER: Stationary --- */}
      <div className="shrink-0 z-30 bg-black/40 backdrop-blur-xl border-b border-white/5 pb-4 pt-2"> {/* Reduced pb-8 to pb-4 */}
        <div className="flex flex-wrap items-end justify-between gap-x-6 gap-y-4 w-full relative group px-6">
          <div className="flex flex-col gap-2 min-w-fit flex-1">
            ...
            <div className="space-y-0.5">
              {/* 🛡️ TIGHTENED typography scale from 5xl to 3xl */}
      ...
      <div className="flex-1 min-h-0 w-full relative p-3 pb-6"> {/* Reduced padding */}
        <div className={cn(
          ...
          <div className="flex-1 w-full overflow-auto custom-scrollbar">
            <table className="w-full text-left border-collapse min-w-[900px] table-fixed">
              <thead className="bg-white/[0.04] border-b border-white/5 sticky top-0 z-20 backdrop-blur-md">
                <tr>
                  {/* 🛡️ MINIMALIST TABLE HEADER: Reduced px-10 py-6 to px-6 py-2.5 */}
              ...
              <tbody className="divide-y divide-white/5">
                {services.map((service: any) => (
                  <tr key={service.id} className="hover:bg-white/[0.01] transition-colors border-none group">
                    {/* 🛡️ TACTICAL SLIM ROWS: Reduced py-8 to py-3.5 */}

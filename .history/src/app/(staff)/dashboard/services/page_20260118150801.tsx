...
import { Zap, Globe, Activity, Terminal, Building2, Users } from "lucide-react";
import { CreateServiceModal } from "@/components/dashboard/create-service-modal";
import { ServiceActionWrapper } from "@/components/dashboard/service-action-wrapper";
import { cn } from "@/lib/utils";

/**
 * 🛰️ SERVICE FLEET COMMAND (Apex v2026.1.18)
 * Strategy: Rigid Chassis & Sticky Ingress.
 * Fix: Locked Viewport height to ensure headers remain stationary.
 */
export default async function ServicesPage() {
  ...
  return (
    /* 🏛️ PRIMARY CHASSIS: Locked to 100% height. No page-level scroll. */
    <div className="w-full h-full flex flex-col min-w-0 overflow-hidden text-foreground bg-black">
      ...
      <div className="shrink-0 z-30 bg-black/40 backdrop-blur-xl border-b border-white/5 pb-4 pt-2">
        <div className="flex flex-wrap items-end justify-between gap-x-6 gap-y-4 w-full relative px-6">
          ...
          <div className="shrink-0 relative z-10 pb-0.5 scale-90 origin-bottom-right">
      ...
      <div className="flex-1 min-h-0 w-full relative p-3 pb-6">
        <div className={cn(
          ...
          {/* 🌊 THE SCROLL ENGINE: Only this div and table rows scroll. */}
          <div className="flex-1 w-full overflow-auto custom-scrollbar overscroll-contain">
            <table className="w-full text-left border-collapse min-w-[900px] table-fixed relative">
              ...
              {/* STICKY THEAD: Fixed inside the reservoir while rows slide underneath. */}
              <thead className="bg-zinc-950 sticky top-0 z-20 border-b border-white/5 backdrop-blur-md">
                <tr>
                  <th className="w-[35%] px-6 py-2.5 text-[7.5px] font-black uppercase tracking-[0.5em] text-muted-foreground/30 italic">Signal_Identity</th>
                  ...
                  <th className="w-[12%] px-6 py-2.5 text-[7.5px] font-black uppercase tracking-[0.5em] text-muted-foreground/30 italic">Load</th>
                  <th className="w-[12%] px-6 py-2.5 text-[7.5px] font-black uppercase tracking-[0.5em] text-muted-foreground/30 italic">Arch</th>
                  <th className="w-[15%] px-6 py-2.5 text-[7.5px] font-black uppercase tracking-[0.5em] text-muted-foreground/30 italic">Status</th>
              ...
              <tbody className="divide-y divide-white/5">
                {services.map((service: any) => (
                  <tr key={service.id} className="hover:bg-white/[0.01] transition-colors border-none group">
                    ...
                    <td className="px-6 py-3.5">
                      <div className="flex items-center gap-2 text-lg font-black italic text-foreground leading-none tabular-nums">
                        <Users className="size-3 text-muted-foreground/20" />
                        {(service._count?.subscriptions || 0).toLocaleString()}
                      </div>
                    </td>
                    <td className="px-6 py-3.5 text-md font-black italic text-emerald-500/40">

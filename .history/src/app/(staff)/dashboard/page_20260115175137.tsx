 ...
 * Design: morphology-aware safe-area clamping with Hardware-Pulse sync.
 */
export default async function DashboardPage() {
  ...
  // 🛰️ DEVICE INGRESS: Platform detection for spatial recalibration
  ...
  // 🛡️ 3. ROLE_AUTHORIZATION_PROTOCOL
    ...
    <div className={cn(
      ...
      <div className="relative group p-0">
        ...
        <div className="flex items-center gap-4 mt-8">
          <div className={cn(
            "flex items-center gap-3 px-4 py-1.5 rounded-2xl border text-[9px] font-black uppercase tracking-[0.4em] italic shadow-apex transition-all duration-700",
            isPlatformStaff ? "bg-amber-500/10 border-amber-500/20 text-amber-500" : "bg-primary/10 border-primary/20 text-primary"
          )}>
            {isSuperAdmin ? <Crown className="size-3" /> : <ActivityIcon className="size-3 animate-pulse" />}
            {role.replace('_', ' ')}
          </div>
...
function StatusNode({ title, status, sub, isAmber }: { title: string, status: string, sub: string, isAmber: boolean }) {
  return (
    <div className={cn(
      "rounded-[2.5rem] border p-8 flex flex-col justify-between shadow-inner transition-all duration-1000",
      ...
      <div className="space-y-2 mt-8">
        <p className={cn("text-4xl font-black italic uppercase leading-none tracking-tighter tabular-nums", isAmber ? "text-foreground" : "text-emerald-500")}>
          {status}
        </p>

...
// 📊 Async Data Hydration (These should be Server Components used inside Suspense)
...
export function DashboardClientView({ session }: { session: any }) {
  // ✅ Hooks are now safely inside a Client Component
  ...
  // 🛡️ HYDRATION_SHIELD
  if (!isReady) return <div className="h-screen w-full bg-black animate-pulse" />;
            ...
            {isPlatformStaff && (
              <div className="rounded-[2rem] border border-amber-500/10 bg-amber-500/[0.03] p-6 shadow-2xl">
                ...
                <p className="text-[8px] font-bold text-muted-foreground/30 uppercase leading-relaxed tracking-widest mb-6 italic">
                  Audit and resolve incoming tickets across all nodes.
                </p>
                <Link
...
function ActionBtn({ href, icon: Icon, title, sub, impact }: any) {
  return (
    ...
    >
      <div className="flex size-10 shrink-0 items-center justify-center rounded-lg bg-primary/10 transition-all group-hover:bg-primary/20">

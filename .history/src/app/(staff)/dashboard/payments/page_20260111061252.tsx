...
/**
 * 🏛️ CAPITAL INGRESS LEDGER (Apex Sync Tier)
 * Fixed: JSX parsing logic for Table components.
 * Normalized: Fluid typographic scaling and zero-bleed layout.
 */
    ...
    <div className="max-w-full overflow-x-hidden space-y-8 md:space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-1000 pb-10">
      ...
      <div className="flex flex-wrap items-start justify-between gap-x-6 gap-y-8 w-full px-1">
        <div className="flex flex-col gap-4 min-w-fit flex-1">
          ...
          <div className="space-y-1">
            <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-black tracking-[-0.05em] uppercase italic leading-[0.8] text-foreground">
            ...
            <p className="text-[9px] md:text-[10px] font-black text-muted-foreground uppercase tracking-[0.3em] mt-3 opacity-40">
              Validated Transactions: <span className="text-foreground tabular-nums">{transactions.length}</span>
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-stretch gap-3 w-full lg:w-auto shrink-0 lg:mt-auto">
          <div className="relative min-w-0 sm:w-64 xl:w-80 group">
            ...
            <Input 
              ...
              className="h-11 md:h-13 w-full pl-11 rounded-2xl border-border/40 bg-card/40 font-black text-[9px] md:text-[10px] uppercase tracking-[0.2em] focus:ring-primary/20 transition-all"
            />
          </div>
          <Button variant="outline" className="h-11 md:h-13 px-5 rounded-2xl border-border/40 bg-muted/10 text-[9px] md:text-[10px] font-black uppercase tracking-widest hover:bg-primary/5 transition-all">
          ...
          <Table className="w-full text-left border-collapse min-w-[850px]">
            <TableHeader className="bg-muted/30 border-b border-border/10">
              <TableRow className="hover:bg-transparent border-none"> 
                {["Identity Node", "Service // Access", "Liquidity", "Timestamp", "Status"].map((head) => (
                  <TableHead 
                    key={head} 
              ...
              {transactions.length === 0 ? (
                <TableRow className="hover:bg-transparent border-none">
                  <TableCell colSpan={5} className="h-64 text-center">
                    <p className="text-xs font-black uppercase italic tracking-tighter opacity-20">Zero ingress activity detected.</p>
                  </TableCell>
                </TableRow>
              ) : (
                transactions.map((tx) => (
                  <TableRow key={tx.id} className="hover:bg-primary/[0.02] transition-all group duration-500 border-none">
                    ...
                    <TableCell className="px-6 py-6">
                      <div className="flex flex-col gap-1.5">
                        ...
                        <p className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest ml-5 opacity-40">
                          INFLOW BROADCAST
                        </p>
                      </div>
                    </TableCell>
      ...
      <div className="flex items-center gap-3 px-2 opacity-20">
         ...
         <p className="text-[8px] font-black uppercase tracking-[0.3em] text-muted-foreground italic leading-none">
           Capital Ledger Synced // Node Identity Cluster {realMerchantId.slice(0, 8)}
         </p>
      </div>
    </div>
  );

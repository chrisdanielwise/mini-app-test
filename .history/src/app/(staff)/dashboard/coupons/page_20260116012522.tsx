...
import { 
  ...
  Terminal, Globe, Tag, Building2, Activity, Search
} from "lucide-react";
...
/**
 * 🎫 PROMOTION_LEDGER (Institutional Apex v2026.1.16)
 * Strategy: Defensive Ingress Logic with Restored Chroma Protocol.
 * Fix: Explicitly handles undefined 'coupons' and RBAC filters to prevent empty data states.
 */
export default function CouponsPage({ 
  ...
  // 🛡️ DATA RECOVERY: Ensure data is mapped correctly even if nested or latent
    ...
    >
          ...
          <table className="w-full text-left border-collapse min-w-[1000px]">
            <thead className="bg-white/[0.02] border-b border-white/5">
              <tr>
                ...
                <th className="px-10 py-6 text-[9px] font-black uppercase tracking-[0.4em] text-muted-foreground/30 italic">Reduction</th>
                      ...
                      <div className="w-32 space-y-3">
                         ...
                         <div className="h-1.5 w-full bg-white/5 rounded-full overflow-hidden">
                            <div 
                              ...
                              style={{ width: `${coupon.maxUses ? ((coupon.currentUses || 0) / coupon.maxUses) * 100 : 100}%` }} 
                            />
                         </div>
                      </div>
                    </td>
      ...
      <div className="flex items-center justify-center gap-5 opacity-10 py-12 border-t border-white/5">
        ...
        <p className="text-[9px] font-black uppercase tracking-[0.5em] text-foreground italic">
           Promotion deployment synchronized // Node: {user?.merchantId ? user.merchantId.slice(0, 8).toUpperCase() : "ROOT"}
        </p>
      </div>
    </div>
  );

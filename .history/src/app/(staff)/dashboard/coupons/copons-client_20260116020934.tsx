...
/**
 * 🎫 PROMOTION_LEDGER_CLIENT
 * Strategy: Device-aware layout with Role-based Chroma (Amber/Emerald).
 */
export default function CouponsClientPage({ 
  coupons = [], // Default to empty array to prevent .length errors
  totalCount, 
  query, 
  pagination, 
  isSuperAdmin, 
  ...
  // 🛡️ Prevent layout snapping before device telemetry is ready
  if (!isReady) return (
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 border-b border-white/5 pb-8 relative group">
        <div className="space-y-4">
          <div className="flex items-center gap-3 italic opacity-40">
            <Tag className={cn("size-3.5", isSuperAdmin ? "text-amber-500 fill-amber-500" : "text-primary fill-primary")} />
            <span className={cn(
          ...
          <table className="w-full text-left border-collapse min-w-[1000px]">
            <thead className="bg-white/[0.02] border-b border-white/5">
              <tr>
                ...
                <th className="px-6 py-4 text-[9px] font-black uppercase tracking-[0.4em] text-muted-foreground/30 italic">Adjustment</th>
                <th className="px-6 py-4 text-[9px] font-black uppercase tracking-[0.4em] text-muted-foreground/30 italic">Utilization</th>
                <th className="px-6 py-4 text-[9px] font-black uppercase tracking-[0.4em] text-muted-foreground/30 italic">Target Node</th>
              ...
              {coupons.length === 0 ? (
                ...
                coupons.map((coupon: any, index: number) => (
                  ...
                  >
                    ...
                    <td className="px-10 py-8">
                      <Badge variant="outline" className="rounded-xl text-[8px] font-black uppercase italic tracking-tighter text-muted-foreground/60 bg-white/5 px-2 py-0.5 border-white/5 truncate max-w-[140px]">

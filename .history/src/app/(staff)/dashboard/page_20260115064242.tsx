...
import { headers } from "next/headers"; // ✅ Fixed ReferenceError
...
/**
 * 🛰️ MERCHANT COMMAND CENTER (v16.16.23)
 * Architecture: Header-Hydrated Ingress with Async Component Streaming.
 * Fix: Removed 'requireStaff' to prevent 10s DB lag and loop.
 */
export default async function DashboardPage() {
  const headerList = await headers();
  
  // 🛡️ 1. EXTRACT PROXY IDENTITY (Zero Latency)
  const userId = headerList.get("x-user-id");
  const role = (headerList.get("x-user-role") || "user").toLowerCase();
  const fullName = headerList.get("x-user-name") || "Verified Merchant";
  
  // Tactical ID for merchant components (Proxy ensures this is the authenticated ID)
  const realMerchantId = userId; 

  // 🚀 TACTICAL DEBUG
  console.log(`📄 [Page_Render]: User ${userId} (${role}) content hydration starting.`);

  // 🛡️ 2. SECURITY HANDSHAKE
  if (!userId) {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 md:space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-700 pb-10 px-4 md:px-8 text-foreground">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
        {/* 🚀 ASYNC STREAMING: DB work happens inside these components via Suspense */}
          ...
          {isMerchant && (
            <div className="rounded-3xl border border-border/10 bg-card/40 backdrop-blur-md p-6 md:p-8 shadow-xl">
              ...
              <div className="grid gap-3">
                ...
                <ActionBtn href="/dashboard/coupons" icon={TicketPercent} title="Deployment Promotion" sub="Incentive Node" />
              </div>
            </div>
          )}

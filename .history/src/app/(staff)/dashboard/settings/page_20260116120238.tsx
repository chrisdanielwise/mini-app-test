...
import { 
  ...
  CheckCircle2, Layout, Terminal, ShieldAlert, ServerCog,
  Globe, Zap, ShieldX, Activity, Cpu, Fingerprint, Save, Loader2
} from "lucide-react";
 ...
 * Fix: High-density geometry prevents vertical blowout of configuration nodes.
 */
export default function SettingsPage() {
  ...
  return (
    <div className="w-full h-full flex flex-col min-w-0 overflow-hidden text-foreground bg-black">
      ...
      >
        <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-4 px-6">
          <div className="space-y-1.5 min-w-0">
            <Link href="/dashboard" className="group inline-flex items-center gap-2 text-[8px] font-black uppercase tracking-[0.2em] text-muted-foreground/30 hover:text-primary transition-all italic">
              <ArrowLeft className="size-2.5 group-hover:-translate-x-1" /> Back
            </Link>
            <div className="space-y-1">
              ...
              <div className="flex items-center gap-2.5 opacity-20 italic">
                ...
                <p className="text-[7px] font-black uppercase tracking-[0.4em]">
                  Session: {user?.role || "GUEST"} // v16.31_STABLE
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="shrink-0 scale-90 origin-bottom-right">
              <Button 
                ...
                className={cn(
                  ...
                  isSuperAdmin ? "bg-amber-500 text-black shadow-amber-500/20" : "bg-primary text-white shadow-primary/20"
                )}
              >
      ...
      {/* --- 🚀 INTERNAL SCROLL VOLUME: Independent Configuration matrix --- */}
      <div className="flex-1 min-h-0 w-full relative overflow-y-auto custom-scrollbar px-6 py-6">
        <div className={cn(
          ...
          <div className="lg:col-span-2 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <div className="relative group">
                    ...
                    <Input 
                      ...
                      className="h-10 rounded-xl bg-black/40 border-white/5 pl-10 font-black text-[10px] uppercase italic tracking-widest placeholder:opacity-10" 
                ...
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    <Input type="password" defaultValue="••••••••••••••••" className="h-10 rounded-xl bg-black/40 border-white/5 pr-10 font-mono text-[9px] opacity-10 cursor-not-allowed italic" disabled />
                    <Key className="absolute right-4 top-1/2 -translate-y-1/2 size-3 text-muted-foreground/10" />
                  </div>
                </div>
              </div>

              {/* Compressed Session Table Integration */}
              <div className="pt-2">
            ...
            <section className="space-y-4">
              <div className="flex items-center gap-3 text-emerald-500/30 border-b border-white/5 pb-3">
                <Maximize className="size-3.5" />
                <h2 className="text-[8px] font-black uppercase tracking-[0.4em] italic">Architecture</h2>
              </div>
              <div className="flex items-center justify-between p-6 rounded-2xl bg-black/40 border border-white/5 hover:bg-white/[0.01] transition-all">
                  <div className="space-y-1 leading-none">
                    <p className="text-sm font-black uppercase italic tracking-tighter text-foreground">Ultra-Wide Flow</p>
                    <p className="text-[7px] font-black uppercase tracking-[0.2em] text-muted-foreground/20">1920PX_MESH_SYNC</p>
                  </div>
                  <Switch checked={isFullSize} onCheckedChange={toggleFullSize} className="scale-75 data-[state=checked]:bg-emerald-500" />
              </div>
            </section>
          </div>
          ...
          <div className="space-y-6">
            <div className={cn(
              ...
              <ShieldCheck className="absolute -top-6 -right-6 size-32 opacity-[0.01] rotate-12 pointer-events-none group-hover:rotate-0 transition-transform duration-1000" />
              ...
              <div className="space-y-6 relative z-10">
                ...
                <div className="flex items-center gap-3">
                  <div className={cn(
                    "size-8 rounded-lg flex items-center justify-center border shadow-inner",
                    isSuperAdmin ? "bg-amber-500/10 border-amber-500/20 text-amber-500" : "bg-primary/10 border-primary/20 text-primary"
                  )}>
                     <CheckCircle2 className="size-3.5" />
                  </div>
                  <span className="text-[8px] font-black uppercase tracking-[0.2em] text-foreground/20 italic">Node_Sync: Live</span>
                </div>

                <div className="pt-6 border-t border-white/5 space-y-4">
                  <div className="flex items-center gap-3 text-rose-500/20 italic">
                     <ShieldAlert className="size-3" />
                     <span className="text-[7px] font-black uppercase tracking-[0.3em]">Danger_Zone</span>
                  </div>
                  <Button 
                    onClick={handleGlobalRevocation}
                    disabled={isRevoking}
                    variant="outline"
                    className="w-full h-11 rounded-xl border-rose-500/10 bg-rose-500/[0.02] text-rose-500 hover:bg-rose-500/5 transition-all font-black uppercase italic text-[8px] tracking-[0.2em]"
                  >
                    <ShieldX className="mr-2 size-3.5" />
                    {isRevoking ? "REVOKING..." : "Remote_Wipe"}
                  </Button>
                  <p className="text-[6px] text-muted-foreground/10 uppercase font-black tracking-[0.2em] text-center italic px-1">
                    Invalidates every active session cluster-wide.
                  </p>
                </div>
              </div>
            </div>

            <div className="flex flex-col items-center gap-3 py-6 opacity-10 italic">
               <Cpu className="size-3" />
               <p className="text-[7px] font-black uppercase tracking-[0.5em] leading-none">Mesh_Processing // [v16.31_STABLE]</p>
            </div>
          </div>
        </div>
      </div>

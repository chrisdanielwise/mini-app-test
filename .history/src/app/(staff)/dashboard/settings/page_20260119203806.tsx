...
export default function SettingsPage() {
  const { mounted, flavor } = useLayout();
  const { user } = useInstitutionalAuth(); 
  const router = useRouter();
  
  const { isReady, isMobile, safeArea } = useDeviceContext();
  
  // ✅ FIX: TS2339 - Properties now mapped to standard TelegramContextValue interface
  ...
  useEffect(() => {
    // Defensive check to ensure context methods exist
    if (!isTelegram || !isReady || typeof setMainButton !== 'function') return;

    setMainButton({
      text: isSaving ? "🛰️ SYNCHRONIZING..." : "💾 COMMIT_CONFIGURATION",
      isVisible: isMobile,
      isActive: !isSaving,
      // @ts-ignore - API 8.0 support for color hex strings
      color: isSuperAdmin ? "#f59e0b" : "#10b981",
      onClick: () => handleSaveSequence()
    });

    // Cleanup: Ensure button is hidden when navigating away from settings node
    return () => {
      if (setMainButton) {
        setMainButton({ text: "", isVisible: false });
      }
    };
  ...
  const handleSaveSequence = async () => {
    ...
    // ✅ FIX: TS2339 - Using safe haptic check
    if (hapticFeedback) hapticFeedback("medium");

    try {
      // Simulate institutional sync latency

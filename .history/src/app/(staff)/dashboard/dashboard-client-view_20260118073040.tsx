...
interface DashboardClientViewProps {
  session: any;
  children: React.ReactNode; // 🛰️ INJECTED SERVER DATA
}

/**
 * 🛰️ DASHBOARD_CLIENT_VIEW (Institutional Apex v2026.1.20)
 * Strategy: Boundary Isolation via Composition.
 * Fix: Removed direct imports of Async nodes to prevent 'fs/tls' leakage.
 */
export function DashboardClientView({ session, children }: DashboardClientViewProps) {
  ...
  // 🛡️ HYDRATION_SHIELD
  if (!isReady) return (
    <div className="h-screen w-full bg-zinc-950 flex flex-col items-center justify-center space-y-4">
      <Cpu className="size-8 text-primary/20 animate-spin" />
      <p className="text-[10px] font-black uppercase tracking-[0.4em] text-muted-foreground/20">Establishing_Hardware_Bridge...</p>
    </div>
  );
  ...
  return (
    <div className="w-full h-full flex flex-col min-w-0 overflow-hidden text-foreground bg-black">
      ...
      >
        ...
        {/* 🛰️ COMPOSITION SLOT: Renders AsyncRevenueCard & AsyncSubscribersCard from Server */}
        {children}
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          <div className="lg:col-span-2 space-y-6">
            ...
            {/* 📊 The Activity Feed is now handled inside {children} in the Server Page */}
          </div>
          ...
          <div className="space-y-6">
            {isMerchant && (
              <div className="rounded-[2rem] border border-white/5 bg-card/30 backdrop-blur-3xl p-6 shadow-2xl">
                ...
                <div className="grid gap-3">
                  <ActionBtn href="/dashboard/services" icon={PlusCircle} title="New Service" sub="Expansion" impact={impact} />
                  <ActionBtn href="/dashboard/coupons" icon={TicketPercent} title="Campaign" sub="Incentive" impact={impact} />
                </div>
              </div>
            )}
            ...
            {/* 🏎️ PERFORMANCE HUD */}
            <div className="p-6 rounded-2xl border border-white/5 bg-white/[0.01] flex flex-col gap-4">
               <div className="flex items-center justify-between">
                  <span className="text-[7px] font-black uppercase tracking-widest opacity-20">Hardware_Sync</span>
                  <Globe className={cn("size-3", isPlatformStaff ? "text-amber-500" : "text-primary")} />
               </div>
               <div className="space-y-1">
                  <div className="h-1 w-full bg-white/5 rounded-full overflow-hidden">
                     <div className={cn("h-full w-[88%] animate-pulse", isPlatformStaff ? "bg-amber-500" : "bg-primary")} />
                  </div>
...
function ActionBtn({ href, icon: Icon, title, sub, impact }: any) {
  return (
    <Link
      ...
      className="group flex items-center gap-4 rounded-xl border border-white/5 p-4 transition-all duration-500 hover:bg-white/[0.05] bg-white/[0.01] active:scale-95"
    >

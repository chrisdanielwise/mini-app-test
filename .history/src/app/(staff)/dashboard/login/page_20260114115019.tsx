...
export function LoginContent() {
  ...
  const handleTelegramLogin = () => {
     ...
     * This creates a focused window that feels like a native app modal.
     */
    const width = 550;
    const height = 750;
    ...
    // 🛡️ WATCHDOG: Reset state if the user manually closes the popup window
    const checkClosed = setInterval(() => {
      if (authWindow?.closed) {
        clearInterval(checkClosed);
        // Only reset if we haven't already verified (to avoid race conditions)

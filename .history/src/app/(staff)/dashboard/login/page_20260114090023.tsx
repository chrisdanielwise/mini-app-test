...
/**
 * 🛰️ STAFF GATEWAY TERMINAL (Institutional v14.82.0)
...
function LoginContent() {
  ...
  useEffect(() => {
    ...
    authChannel.onmessage = (event) => {
      if (event.data.action === "RELOAD_SESSION") {
        ...
        console.log("✅ [Sync_Handshake]: Identity pulse received.");
        setStatus("VERIFIED");
        hapticFeedback?.("success");
        toast.success("Identity Anchored via Remote Node");

        // Force Atomic Reload to the requested target
        setTimeout(() => {
          window.location.href = event.data.target || "/dashboard";
        }, 1000);
      }
    };
  ...
  return (
    <div className="flex min-h-[100dvh] flex-col items-center justify-center p-6 bg-background relative selection:bg-amber-500/30 text-foreground overflow-hidden">
      ...
      {status === "VERIFIED" && <HandshakeSuccess />}

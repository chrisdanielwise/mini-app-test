...
/**
 * 🛰️ STAFF GATEWAY TERMINAL (Institutional v15.3.5)
 * Architecture: Bidirectional Handshake with Native Haptics & Pulse Polling.
 */
export function LoginContent() {
  ...
  // 🛰️ IDENTITY STATE: Anchors user data for the success component
  ...
  /**
   * 🛰️ 1. BIDIRECTIONAL BROADCAST LISTENER
   * Architecture: Listens for pulses from Tab B and sends Acknowledgment.
   */
  useEffect(() => {
    ...
    authChannel.onmessage = (event) => {
      ...
      if (action === "RELOAD_SESSION") {
        // 🛡️ SEND ACKNOWLEDGMENT: Tells Tab B we received the signal
        authChannel.postMessage({ action: "HANDSHAKE_ACKNOWLEDGED" });

        // 🧹 CLEAR WATCHDOG: Stop the login timeout
        if (timeoutRef.current) clearTimeout(timeoutRef.current);

        console.log(`✅ [Sync_Handshake]: Identity pulse received at ${timestamp}.`);

        // 👤 CAPTURE IDENTITY: Satisfies ts(2741) in HandshakeSuccess
        setVerifiedUser(user || { fullName: "Verified Operator" });
        setStatus("VERIFIED");

        // 📳 PHYSICAL FEEDBACK: Native Browser Vibration
        ...
        /**
         * 🚀 ATOMIC RELOAD
         * Forces the browser to refresh and pick up new HttpOnly cookies.
         */
        setTimeout(() => {
  ...
  /**
   * 🚀 2. AUTOMATED MAGIC HANDSHAKE
   * Handles direct landings from magic links or one-tab auth.
   */
  useEffect(() => {
   ...
   * Opens the link in a new window to keep the anchor tab alive.
   */
  const handleTelegramLogin = () => {
    ...
    // 🚀 NEW WINDOW: Prevents current terminal from being replaced
    setTimeout(() => { 
      window.open(telegramUrl, "_blank", "width=600,height=750"); 
    }, 150);
  };
        ...
        <div className="flex flex-col items-center text-center space-y-8 relative z-10">
          ...
          <div className="space-y-2">
            ...
            <p className="text-[9px] font-black text-muted-foreground uppercase tracking-[0.4em] flex items-center justify-center gap-2 opacity-60">
              ...
              {status === "SYNCING" ? "Awaiting Remote Pulse" : "Secure Identity Handshake"}
            </p>
          </div>

          {status === "TIMEOUT" && (
            <Alert variant="destructive" className="rounded-2xl border-rose-500/20 bg-rose-500/5 text-rose-500 py-4">
              <div className="flex items-center gap-3">
                ...
                <Button variant="ghost" size="sm" onClick={handleTelegramLogin} className="h-6 text-[8px] font-black uppercase tracking-widest bg-rose-500/10 hover:bg-rose-500/20">
                  Retry
                </Button>
              </div>
            </Alert>
            ...
            >
              {status === "SYNCING" ? (
                ...
                <span className="flex items-center gap-2">
                  {status === "TIMEOUT" ? "Re-Initialize Handshake" : "Authorize Identity"}

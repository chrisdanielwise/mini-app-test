...
function LoginContent() {
  ...
  useEffect(() => {
    // 🛡️ THE KILL SWITCH: Client-Side Token Ingress
    // If we land here with a token, we move to the callback API immediately.
    const token = searchParams.get("token");
    const reason = searchParams.get("reason");

    if (token) {
      setIsLoading(true);
      // Redirecting to the callback API to exchange the token for a JWT session
      router.replace(`/api/auth/callback?token=${token}`);
      return;
    }

    if (reason === "identity_denied" || reason === "link_invalid" || reason === "auth_required") {
      setError(
        reason === "identity_denied" 
          ? "CLEARANCE_REVOKED: Role mismatch or insufficient permissions." 
          : "HANDSHAKE_FAILED: Token expired or invalid signature."
      );
    }
  }, [searchParams, router]);

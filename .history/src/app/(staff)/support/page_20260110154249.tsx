...
export default async function SupportPage() {
  // 🔐 1. Identity Handshake (Using the fixed session utility)
  ...
  // Ensure the model name matches your schema.prisma (usually camelCase in client)
  const tickets = await prisma.supportTicket.findMany({
    where: { 
      merchantId: session.merchantId // ✅ Alignment fix: session.merchantId exists, session.merchant does not.
    },
    include: { 
      adminUser: { // ✅ Alignment fix: renamed 'user' to 'adminUser' to match your schema's User relation
        select: {
          id: true,
          fullName: true,
          telegramId: true,
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="text-[9px] font-black text-muted-foreground uppercase tracking-widest">
            NODE: {session.config.companyName || "SYSTEM_ROOT"}
          </span>
        </div>
      </div>

      {/* --- LEDGER TABLE --- */}
      <div className="rounded-[2.5rem] border border-border bg-card/50 backdrop-blur-md overflow-hidden shadow-sm">
                ...
                tickets.map((ticket) => (
                  <tr key={ticket.id} className="hover:bg-muted/10 transition-colors group">
                    <td className="px-8 py-6">
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col">
                          <span className="font-bold text-sm leading-none">{ticket.adminUser?.fullName || "Verified User"}</span>
                    ...
                    <td className="px-8 py-6 max-w-md">
                      <p className="text-xs font-medium line-clamp-2 opacity-80 uppercase italic">{ticket.subject}</p>
                    </td>
                    <td className="px-8 py-6">

...
export default async function SubscribersPage() {
  ...
  const subscriptions = await prisma.subscription.findMany({
    ...
    include: {
      adminUser: {
        // ✅ Relation fix: matching your User relation name 'adminUser'
        select: {
          id: true,
          fullName: true,
          telegramId: true,
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead className="bg-muted/20 border-b border-border/40">
              <tr>
                <th className="px-8 py-6 text-[9px] font-black uppercase tracking-widest text-muted-foreground">
                  Identity
                </th>
                <th className="px-8 py-6 text-[9px] font-black uppercase tracking-widest text-muted-foreground">
                  Assigned Service
            ...
            <tbody className="divide-y divide-border/20">
              {subscriptions.length === 0 ? (
                <tr>
                  <td
                    colSpan={4}
                    className="py-20 text-center text-muted-foreground italic uppercase text-[10px] font-black tracking-[0.2em]"
                  >
                    ...
                    <td className="px-8 py-7">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex flex-col">
                          ...
                          <Badge
                            variant="secondary"
                            className="w-fit text-[8px] font-black px-1.5 py-0 mt-1 uppercase tracking-tighter bg-emerald-500/10 text-emerald-500"
                          >
                            {sub.status}
                          </Badge>
                        </div>
                      ...
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                          <Button
                            variant="ghost"
                            size="icon"
                            className="h-10 w-10 rounded-xl bg-muted/20 hover:bg-primary/10 transition-all"
                          >
                        ...
                        <DropdownMenuContent
                          align="end"
                          className="w-56 rounded-[1.5rem] p-2 shadow-2xl border-border/40 bg-card/95 backdrop-blur-2xl"
                        >

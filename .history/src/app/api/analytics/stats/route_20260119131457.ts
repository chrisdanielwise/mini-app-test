...
// ✅ CORRECTED DECIMAL PATH: Ensuring high-precision financial logic
import { Decimal } from "@prisma/client-runtime-utils";

/**
 * 📊 ANALYTICS AGGREGATOR (Institutional v2026.1.20)
 * Hardened: Role-normalized aggregation for Global vs. Tenant isolation.
 * Standard: Using UPPERCASE Prisma Enums to satisfy generated types.
 */
export async function GET() {
  ...
  try {
    ...
    // 🏗️ 2. DYNAMIC SCOPE
    // Staff see the whole cluster; Merchants are locked to their own tenant node.
    const baseFilter = isStaff ? {} : { merchantId };
    ...
    const [currentRevenue, lastRevenue] = await Promise.all([
      prisma.payment.aggregate({
        where: {
          ...baseFilter,
          // ✅ FIX: Switched to UPPERCASE Prisma Enum
    ...
    const [activeSubCount, newUsersCount] = await Promise.all([
      ...
      prisma.user.count({
        where: {
          createdAt: { gte: currentMonthStart },
          // Filter users who have touched this merchant's services
          ...(isStaff ? {} : { subscriptions: { some: { merchantId } } })
        }
      })
    ]);

    // 🏁 5. FINANCIAL PRECISION LOGIC
      ...
      stats: [
        {
          ...
          value: `$${revNow.toNumber().toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,

...
export async function POST(request: NextRequest) {
  try {
    ...
    if (merchantId) {
      const node = await prisma.merchant.findUnique({ where: { id: merchantId }, select: { botToken: true }});
      if (node?.botToken) botTokenOverride = node.botToken;
    }

    const validated = validateTelegramInitData(initData, botTokenOverride);
    if (!validated || !validated.user) return errorResponse("Invalid identity node.", 401);

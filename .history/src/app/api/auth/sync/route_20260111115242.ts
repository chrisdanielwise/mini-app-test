...
export async function POST(request: Request) {
  try {
    ...
    const [merchant, user] = await Promise.all([
      MerchantService.getByAdminTelegramId(bigIntId),
      UserService.getUserByTelegramId(bigIntId),
    ]);

    // Check if the user has platform clearance (Staff roles)
    const isStaff =
      user &&
      ["super_admin", "platform_manager", "platform_support"].includes(
        user.role
      );
    ...
    if (!merchant && !isStaff) {
      console.warn(
        `⚠️ [Auth Sync] Identity Node ${telegramId} lacks clearance.`
      );
      return NextResponse.json({
        authorized: false,

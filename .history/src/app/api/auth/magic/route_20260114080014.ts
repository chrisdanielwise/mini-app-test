...
export async function GET(request: NextRequest) {
  ...
  console.log(
    `🔑 [Magic_Handshake]: Initializing sequence for: ${publicBaseUrl}`
  );

  if (!token) {
    return NextResponse.redirect(
      new URL("/dashboard/login?reason=no_token", publicBaseUrl)
    );
  }

  try {
    // 1. 🛡️ VERIFY & ROTATE TOKEN
    const user = (await AuthService.verifyMagicToken(
      token
    ...
    if (!user) {
      ...
      return NextResponse.redirect(
        new URL("/dashboard/login?reason=invalid_token", publicBaseUrl)
      );
    }
    ...
    // 🛰️ DYNAMIC ROUTING LOGIC (Nested Ternary)
    const targetPath =
      pathname === "/" ? "/" : isPrivileged ? "/dashboard" : "/home";
    ...
    console.log(
      `✅ [Magic_Success]: Identity anchored for Node ${user.id.slice(
        0,
        8
      )} [${userRole}]`
    );
    ...
    return NextResponse.redirect(
      new URL("/dashboard/login?reason=system_error", publicBaseUrl)
    );
  }
}

...
export async function POST(request: NextRequest) {
  try {
    ...
    // 6. SECURE COOKIE DEPLOYMENT (Tunnel Hardened)
    const cookieStore = await cookies();
    
    // 🛡️ Detect Cloudflare/Ngrok/Production
    const host = request.headers.get("host") || "";
    const protocol = request.headers.get("x-forwarded-proto") || "";
    const isProxy = host.includes("trycloudflare.com") || host.includes("ngrok") || protocol === "https";
    const isProd = process.env.NODE_ENV === "production";

    const useSecure = isProd || isProxy;

    cookieStore.set(JWT_CONFIG.cookieName, jwt, {
      httpOnly: true,
      path: "/",
      maxAge: 60 * 60 * 24 * 7,
      // 🚀 THE FIX: If on Cloudflare, we MUST use 'none' + 'secure' + 'partitioned'
      secure: useSecure,
      sameSite: useSecure ? "none" : "lax",
      // @ts-ignore - CHIPS/Partitioned is essential for Safari/Telegram 2026

...
export async function POST(request: NextRequest) {
  try {
    ...
    // 2. EXECUTION VIA HARDENED SERVICE
    // ✅ FIX: We now pass 'shipping' and 'customerNote' as top-level properties
    // to match the refactored createOrder service signature.
    const order = await createOrder({
      ...
      // Mapped to match your Schema's flattened columns
      shipping: {
        name: shipping?.name,

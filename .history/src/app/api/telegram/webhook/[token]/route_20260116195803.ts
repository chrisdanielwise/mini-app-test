...
export async function POST(
  ...
  try {
    ...
    /**
     * 🏎️ STRATEGY: BACKGROUND_DISPATCH
     * We wrap the heavy logic in an IIFE (Immediately Invoked Function Expression)
     * but we DO NOT 'await' it. This lets the POST request finish instantly.
     */
    (async () => {
      try {
        // --- 💳 1. PAYMENT RECONCILIATION ---
        ...
        // We use the "std/http" callback but invoke it manually inside our background task
        const handleUpdate = webhookCallback(telegramBot, "std/http");
    ...
    // 🚀 THE MAGIC LINE: We respond to Telegram immediately.
    // This stops the 11-second compile wait and the Prisma quota burn.
    return NextResponse.json({ ok: true });

  } catch (error: any) {
    console.error("🔥 [Webhook_Ingress_Fault]:", error.message);
    // Always return 200/ok to Telegram to stop retry loops
    return NextResponse.json({ ok: true });
  }
}
/**

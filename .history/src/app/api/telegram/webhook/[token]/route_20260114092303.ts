...
async function handleIdentityHandshake(message: any) {
  ...
  const keyboard = new InlineKeyboard()
    .webApp("🛰️ ACCESS APP", `${host}/home`)
    // 🔑 Ghost Link trigger
    .callback("🔑 SECURE LOGIN", "request_magic_link") 
    .row()
    .url("🌐 WEBSITE", `${host}/`);
 ...
 * Logic: Generates token on-click and pushes to browser via answerCallbackQuery.
 */
async function handleGhostLinkRequest(callback: any) {
  ...
  // 2. Push URL to browser (Link remains invisible in chat)
  await telegramBot.api.answerCallbackQuery(callback.id, {
    url: magicLink,
    text: "🚀 Identity Link Generated. Redirecting..."
  });

...
import { UserRole, PaymentStatus, SubscriptionStatus } from "@generate/prisma/client";
...
async function handleSuccessfulPayment(message: any) {
  ...
  try {
    await prisma.$transaction(async (tx) => {
      // ✅ FIXED: Using PaymentStatus.SUCCESS
      await tx.payment.updateMany({
        where: { userId: payload.userId, serviceTierId: payload.tierId, status: PaymentStatus.PENDING },
        data: { status: PaymentStatus.SUCCESS, gatewayReference: paymentData.telegram_payment_charge_id }
      });

      const tier = await tx.serviceTier.findUnique({ where: { id: payload.tierId } });
      const expirationDate = new Date();
      // Use logical defaults if tier is missing
      const monthsToAdd = tier?.interval === "YEAR" ? 12 : 1; 
      expirationDate.setMonth(expirationDate.getMonth() + monthsToAdd);

      await tx.subscription.upsert({
        where: { userId_serviceId: { userId: payload.userId, serviceId: payload.serviceId } },
        update: { status: SubscriptionStatus.ACTIVE, expiresAt: expirationDate, renewals: { increment: 1 } },
        create: {
          userId: payload.userId,
          serviceId: payload.serviceId,
          merchantId: payload.merchantId,
          serviceTierId: payload.tierId,
          status: SubscriptionStatus.ACTIVE, // ✅ FIXED: UPPERCASE
    ...
    console.error("🔥 [Payment_Fault]:", err.message); 
  }
}
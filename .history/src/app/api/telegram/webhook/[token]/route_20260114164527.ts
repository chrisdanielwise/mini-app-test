...
export async function POST(
  ...
  try {
    ...
    if (body.callback_query) {
      const data = body.callback_query.data;
      if (data === "request_magic_link") {
        // 🚀 CRITICAL FIX: Answer the query IMMEDIATELY to prevent "Query too old"
        await telegramBot.api.answerCallbackQuery(body.callback_query.id, {
          text: "🛰️ Initializing Secure Handshake...",
        });

        // Now do the heavy lifting in the background
        await handleGhostLinkRequest(body.callback_query);
        return NextResponse.json({ ok: true }); 
      }

...
/**
 * 👻 HANDLER: Ghost Link Generation (Hardened)
 * Logic: Generates token on-click and pushes to browser with sanitization.
 */
async function handleGhostLinkRequest(callback: any) {
  ...
  if (!host.startsWith('http')) {
    console.error("🔥 [Geo_Fault]: NEXT_PUBLIC_APP_URL must start with http/https");
    return NextResponse.json({ ok: true });
  }
  ...
  try {
    ...
    const magicLink = `${host}/api/auth/magic?token=${encodeURIComponent(loginToken)}&redirect=/`;

    console.log(`🛰️ [Ghost_Link]: Pushing URL: ${magicLink}`);

    // 4. Push to browser
    // Telegram requires this URL to be absolute and accessible
    await telegramBot.api.answerCallbackQuery(callback.id, {
      url: magicLink,
    });

  } catch (err: any) {
    console.error("🔥 [Ghost_Link_Failed]:", err.message);
    
    // Fallback: Notify the user if the browser push fails
    await telegramBot.api.answerCallbackQuery(callback.id, {
      text: "⚠️ Handshake failed. Please try again or use the Web App.",
      show_alert: true
    });
  }

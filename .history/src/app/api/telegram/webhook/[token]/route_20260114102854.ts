...
/**
 * 🚀 HANDLER: Identity Handshake (Direct URL Mode)
 * Logic: Sends raw Magic Link directly as a URL button.
 */
async function handleIdentityHandshake(message: any) {
  ...
  // 🔗 Ensure the path points to the 'magic' route with a dashboard redirect
  const magicLink = `${host}/api/auth/magic?token=${loginToken}&redirect=/dashboard`;

  const keyboard = new InlineKeyboard()
    .webApp("🛰️ ACCESS APP", `${host}/home`)
    // 🚀 BACK TO URL: Bypasses the 'answerCallbackQuery' push
    .url("🔑 ONE-TAP LOGIN", magicLink) 
    .row()
    .url("🌐 WEBSITE", `${host}/`);

  if (["SUPER_ADMIN", "MERCHANT"].includes(user.role)) {
    // Note: Remote wipe still uses a callback because it doesn't need to open a browser
  ...
  await telegramBot.api.sendMessage(message.chat.id,
    ...
    `Click below to open your secure terminal.`,

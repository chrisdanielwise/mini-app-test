...
export async function POST(
  ...
  try {
    ...
    if (!botToken || webhookToken !== botToken) {
      console.warn("⚠️ [Webhook_Security]: Unauthorized token ingress attempt.");
    ...
    if (text.startsWith("/start")) {
      ...
      console.log(`🔍 [Start_Logic]: Attempting identity sync for ${telegramId}...`);
      ...
      // 🕵️ DEBUG: Log identified role and merchant status
      ...
      // 🛡️ DEBUG: Final Permission Resolution
      ...
      if (isStaff || isMerchant) {
        ...
        const response = await telegramFetch("sendMessage", botToken, {
          ...
          reply_markup: {
            inline_keyboard: [
              // 🚀 Points to /dashboard/login to ensure the Telegram SDK Handshake triggers
        ...
        console.log(`🏁 [Dispatch_Status]: ${response?.ok ? "✅ SUCCESS" : "❌ FAILED"}`);
      } 
      // --- 📱 CASE B: STANDARD USER (Customer Access) ---
      else {
        console.log(`📱 [Standard_Path]: Dispatching basic launch menu.`);
        await telegramFetch("sendMessage", botToken, {
          chat_id: chatId,

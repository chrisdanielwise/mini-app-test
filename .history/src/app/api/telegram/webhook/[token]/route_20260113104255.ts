...
export async function POST(
  ...
  try {
    ...
    //   return NextResponse.json({ ok: true });
    // }
    // Replace your /start logic with this debugged version:
if (text.startsWith("/start")) {
  console.log(`🚀 [Debug] Processing /start for User: ${firstName}`);

  const payload = {
    chat_id: chatId,
    text: "🛰️ *CONNECTION TEST*\n\nIf you see this, your server successfully talked back to Telegram.",
    parse_mode: "Markdown",
  };

  console.log("📡 [Debug] Sending to Telegram API...");
  const response = await telegramFetch("sendMessage", botToken, payload);

  if (response && response.ok) {
    console.log("✅ [Debug] Telegram accepted the message.");
  } else {
    const errorBody = response ? await response.text() : "No Response Object";
    console.error("❌ [Debug] Telegram REJECTED the message:", errorBody);
  }
}

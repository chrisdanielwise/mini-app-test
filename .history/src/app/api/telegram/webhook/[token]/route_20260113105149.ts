...
/**
 * 🛰️ TELEGRAM WEBHOOK HANDLER (Institutional v13.9.35)
 * Logic: Role-Based Routing with Fixed Variable Mapping.
 */
export async function POST(
  ...
  try {
    ...
    // ✅ FIX: Extract firstName safely from the message sender
    ...
    if (message.successful_payment) {
      // Logic for processing successful payments
      return NextResponse.json({ ok: true });
    }
    ...
    if (text.startsWith("/start")) {
      ...
      // 1. IDENTITY SYNC
      // Using 'as any' to bypass merchantProfile TS property error
      ...
      // --- 🏛️ CASE A: STAFF & MERCHANT (Privileged) ---
      if (isStaff || isMerchant) {
        ...
        await telegramFetch("sendMessage", botToken, {
          ...
          reply_markup: {
            inline_keyboard: [
              // Points to /dashboard/login to ensure Handshake Sync works
  ...
  await telegramFetch("sendMessage", botToken, {
    ...
    text: `🖥️ *SYSTEM ARCHITECTURE STATUS*\n\n` +
          ...
          `*Latency:* ⚡ ${latency}ms\n` +
          `*Environment:* 🏗️ ${process.env.NODE_ENV}\n` +

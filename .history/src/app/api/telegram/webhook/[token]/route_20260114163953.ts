...
async function handleGhostLinkRequest(callback: any) {
  ...
  try {
    ...
    // 🚀 1. Clear the "loading" state in Telegram
    await telegramBot.api.answerCallbackQuery(callback.id, {
      text: "🔑 Security Handshake Initialized.",
    });

    // 🚀 2. Send the actual clickable link (The only safe way)
    await telegramBot.api.sendMessage(callback.from.id, 
      "<b>🔐 SECURE BROWSER SYNC</b>\n\nClick below to anchor your identity to this device terminal:",
      { 
        parse_mode: "HTML",
        reply_markup: new InlineKeyboard().url("🔗 AUTHORIZE BROWSER", magicLink)
      }
    );
  } catch (err: any) {
    console.error("🔥 [Ghost_Link_Fault]:", err.message);
  }
}

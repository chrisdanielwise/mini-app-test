...
async function handleGhostLinkRequest(callback: any) {
  ...
  try {
    ...
    // We encode the return path to ensure the browser knows where to land
    const magicLink = `${host}/api/auth/magic?token=${loginToken}&redirect=/home`;

    // 🚀 THE MAGIC: Opens the default system browser for MacOS/Windows
    await telegramBot.api.answerCallbackQuery(callback.id, {
      url: magicLink,
    });
    ...
    await telegramBot.api.answerCallbackQuery(callback.id, { text: "⚠️ Handshake Error." });
  }

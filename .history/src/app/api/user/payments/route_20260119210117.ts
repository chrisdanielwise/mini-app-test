...
import {
  ...
  type AuthContext,
} from "@/lib/auth/with-auth"; // 🚀 Unified Institutional Wrapper
// ✅ FIX: TS2305 - Ensure this service is updated or call the correct export
...
export async function GET(request: NextRequest) {
  ...
  return withAuth(request, async (req: AuthenticatedRequest, context: AuthContext) => {
    try {
      ...
      // ✅ FIX: Ensure context user object matches your AuthContext definition
      // Usually context.user.id or context.user.userId based on your withAuth implementation
      const userId = context.user.id || (context.user as any).userId;
      ...
      // ✅ FIX: Passing limit to service for optimized ledger pagination
      ...
      const response = successResponse({
        payments: payments.map((p: any) => ({
          id: p.id,
          // Ensure decimals are stringified for hydration safety if not done in service

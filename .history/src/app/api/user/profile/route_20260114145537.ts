...
// 🚀 THE FIX: Standard Singleton Import to survive hot-reloads
...
export async function GET(request: Request) {
  // 🛡️ TUNNEL & CACHE SHIELD: Prevents caching unauthorized or maintenance states
  ...
  try {
    /**
     * 🛡️ DEFENSIVE GUARD
     * Ensures Prisma is hydrated before attempting model queries.
     */
    if (!prisma?.systemConfig) {
      console.error("🔥 [Profile_Me_Critical]: SystemConfig model is unreachable in Prisma Client.");
    ...
    // ⛔ MAINTENANCE CIRCUIT BREAKER: Triggers the client-side maintenance diversion
    ...
    /**
     * 🚀 SOFT AUTH TRIGGER
     * Returns 401 if token is missing to trigger client-side /home redirection.
     */
    if (!token) {
    ...
    const profile = {
      ...
      // Injected for real-time NavGuard reactive updates.

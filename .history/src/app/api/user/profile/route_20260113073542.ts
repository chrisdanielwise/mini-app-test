...
/**
 * 🔒 INTERNAL IDENTITY NODE (Institutional v13.1.5)
 * Logic: Polymorphic Identity Resolution (Cookie-Primary).
 * Performance: Optimized for 2026 TMA Native Hybrid Protocol.
 */
export async function GET(request: Request) {
  ...
  try {
    ...
    if (!token) {
      console.warn("⚠️ [Profile_Me] Unauthorized Ingress: No Token Cookie.");
    ...
    // We fetch the latest data from the DB to ensure role changes take effect immediately.
    ...
    console.log(`✅ [Profile_Me] Identity Verified: ${user.id} [${normalizedRole}]`);
    return response;

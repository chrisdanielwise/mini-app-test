...
/**
 * 🛰️ HOME TERMINAL (Institutional v16.15.0)
 * Logic: Synchronized with Universal Login Node & Soft Auth protocol.
 */
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    const verifySessionNode = async () => {
      try {
        ...
        // 🚨 SOFT AUTH TRIGGER: Redirect to Universal Login Node
        if (res.status === 401) {
          console.warn("🛰️ [Home_Auth]: Session de-synchronized. Redirecting to login terminal.");
  ...
  // 🛡️ STAFF CLEARANCE DETECTION (Includes Staff & Amber flavors)
  ...
  // 3. CRYPTOGRAPHIC GATE: Polymorphic Identity Null Screen
    ...
    return (
      <div className="flex min-h-[100dvh] flex-col items-center justify-center p-6 bg-background animate-in fade-in zoom-in duration-700">
        <div className="rounded-[2.5rem] bg-card border border-rose-500/10 p-10 shadow-2xl text-center space-y-6 max-w-sm relative overflow-hidden">
          ...
          <Button 
            onClick={() => router.push("/login?redirect=/home")} // 🚀 Points to Global Login Node
            variant="outline" 
  ...
  return (
    <div className="flex flex-col min-h-[100dvh] animate-in fade-in duration-700 max-w-3xl mx-auto text-foreground selection:bg-primary/30">
      ...
      <footer className="mt-auto flex flex-col items-center gap-3 opacity-20 py-10">
         ...
         <p className="text-[8px] font-black uppercase tracking-[0.5em] text-muted-foreground italic text-center leading-none">Institutional Ingress Node // Protocol: Zipha_V16</p>
      </footer>
    </div>
  );
}
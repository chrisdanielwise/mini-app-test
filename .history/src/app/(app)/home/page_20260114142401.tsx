...
export default function HomePage() {
  ...
  // 🛡️ 1. SOFT AUTH PROTOCOL: Identity Verification Logic
  useEffect(() => {
    if (!mounted) return;
    setTunnelReady(true);

   const verifySessionNode = async () => {
  // We ping the profile API to check for session health and system status
  try {
    const res = await fetch("/api/user/profile", { cache: 'no-store' });
    
    // 🛠️ MAINTENANCE INTERCEPTOR: Node offline for system updates
    if (res.status === 503) {
      const data = await res.json();
      router.replace(`/maintenance?message=${encodeURIComponent(data.message || "Node offline for scheduled maintenance.")}`);
      return;
    }

    // 🚨 SOFT AUTH TRIGGER: Session de-synchronized
    if (res.status === 401) {
      console.warn("🛰️ [Home_Auth]: Session de-synchronized. Redirecting to anchor.");
      // Trigger the recovery redirect to re-anchor the identity node
      router.replace("/dashboard/login?reason=session_expired&redirect=/home");
    }
  } catch (err) {
    ...
    // Only run verification if the context thinks we are authenticated
  ...
  // 2. INITIALIZATION LOADER: Waiting for the identity link
  ...
  // 3. CRYPTOGRAPHIC GATE: Terminal fallback if auth fails
    ...
    return (
      <div className="flex min-h-[100dvh] flex-col items-center justify-center p-6 bg-background animate-in fade-in zoom-in duration-700">
        <div className="rounded-[2.5rem] bg-card border border-rose-500/10 p-10 shadow-2xl text-center space-y-6 max-w-sm relative overflow-hidden">
          ...
          <Button 
            onClick={() => window.location.replace(`https://t.me/${process.env.NEXT_PUBLIC_BOT_USERNAME}`)}
            variant="outline" 
    ...
    <div className="flex flex-col min-h-[100dvh] animate-in fade-in duration-700 max-w-3xl mx-auto text-foreground selection:bg-primary/30">
      ...
      <div className="px-5 py-8 space-y-10 pb-36">
        ...
        <section className="space-y-5">
          ...
          {subsLoading ? <SkeletonList count={2} /> : activeSubscriptions.length > 0 ? (
      ...
      <footer className="mt-auto flex flex-col items-center gap-3 opacity-20 py-10">
         ...
         <p className="text-[8px] font-black uppercase tracking-[0.5em] text-muted-foreground italic text-center leading-none">Institutional Ingress Node // Protocol: Zipha_V2</p>
      </footer>
    </div>
  );
}
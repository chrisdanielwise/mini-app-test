...
import { SkeletonList } from "@/components/ui/skeleton-list"; // 🛰️ Updated Import
 ...
 * Logic: morphology-aware ingress with hardware-clamped safe areas.
 */
export default function HomePage() {
   ...
   * Logic: Grid density adjusts based on XL/XXL hardware tiers.
   */
  const navGridCols = (isDesktop || (isTablet && !isPortrait)) ? "grid-cols-2 gap-8" : "grid-cols-2 gap-4";

  return (
    <div className="flex flex-col animate-in fade-in duration-1000 max-w-5xl mx-auto">
      
      {/* 🌊 EMERGENCY BROADCAST: Hardware-clamped Top Ingress */}
      {systemConfig?.broadcastActive && (
        <div 
          className={cn(
            "w-full py-4 px-6 flex items-center justify-center gap-3 z-[60] border-b transition-all duration-700 rounded-2xl mb-6",
      ...
      {/* --- HUD HEADER: Role-Aware Branding --- */}
      <header className="px-6 pb-10 rounded-[3rem] md:rounded-[4rem] border border-white/5 bg-card/40 backdrop-blur-3xl shadow-apex relative overflow-hidden">
        {/* Kinetic Radiance Overlay */}
        <div className={cn(
          "absolute inset-0 opacity-10 blur-[100px] pointer-events-none transition-colors duration-[2000ms]",
          isStaff ? "bg-amber-500" : "bg-primary"
        )} />

        <div className="flex items-center gap-6 relative z-10 pt-10">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-3 flex-wrap">
              <h1 className="text-[clamp(1.5rem,6vw,2.5rem)] font-black tracking-tighter truncate uppercase italic leading-none">
      ...
      <main className="py-10 space-y-10">
        ...
        <section className="space-y-6">
          ...
          {subsLoading ? (
            <SkeletonList count={2} template="card" /> 
      ...
      <footer className="mt-auto flex flex-col items-center gap-4 py-12 opacity-20 italic">
         ...
         <p className="text-[8px] font-black uppercase tracking-[0.5em] text-center leading-relaxed">
           Institutional Ingress Node // Protocol: Zipha_V16.30_Apex
         </p>
...
// --- SUB-COMPONENTS (Refined for DeviceState) ---
...
function EmptyState({ isMerchant }: { isMerchant: boolean }) {
  return (
    <div className="flex flex-col items-center justify-center rounded-[3rem] border border-dashed border-white/5 bg-card/5 p-20 text-center shadow-inner relative overflow-hidden animate-in zoom-in duration-700">

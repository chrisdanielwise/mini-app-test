 ...
 * Strategy: Absolute Hydration Gate to prevent "Application Error" crashes.
 */
export default function ServiceDetailPage({
  ...
  // 🚀 LOCK 1: ASYNC PARAMETERS
  // Params in Next.js 16 are Promises and must be unwrapped via 'use'.
  ...
  // 🚀 LOCK 2: HYDRATION BARRIER
  // Prevents rendering until the client environment is 100% stable.
  useEffect(() => {
    if (!mounted) return;
  ...
  // 🛰️ DATA INGRESS: Consuming nodes from the Tommy Tactical cluster
  ...
  // 🚀 LOCK 3: DATA SANITIZATION
  // Forces all BigInt and Decimal fields to Strings to prevent React rendering crashes.
  const service = useMemo(() => {
    ...
    try {
      return {
        ...
        name: String(rawData.name || "Unknown Node"),
        ...
        tiers: Array.isArray(rawData.tiers) 
          ? rawData.tiers.map((t: any) => ({
              ...t,
              id: String(t.id),
              price: String(t.price), // Force Decimal price to String
              name: String(t.name)
            }))
          : []
      };
    } catch (e) {
      console.error("🔥 [Sanitization_Crash]:", e);
      return null;
    }
  }, [rawData]);
  ...
  }, [service, selectedTierId, isSubmitting, isTelegram, isReady, auth?.isAuthenticated, tunnelReady, isStaff, mounted, setMainButton]);
  ...
  // --- RENDERING BARRIER ---
  // 🚀 CRITICAL: Returns nothing until fully mounted to stop "Application Error".
  ...
  if (!service) {
    return (
      <div className="flex min-h-[80vh] flex-col items-center justify-center p-6 text-center">
        ...
        <p className="text-[10px] font-bold text-muted-foreground uppercase mb-10 opacity-30">The requested node ID is invalid or inactive.</p>
        <Button onClick={() => router.push("/services")} className="mt-6 rounded-xl h-12 px-8 font-black tracking-widest text-[10px]">Return to Hub</Button>
      </div>
    );
  }

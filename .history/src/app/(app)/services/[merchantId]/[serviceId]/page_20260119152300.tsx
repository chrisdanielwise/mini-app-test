...
import { type Service } from "@prisma";
...
export default function ServiceDetailPage({
  ...
  const { data: service, loading: apiLoading } = useInstitutionalFetch<Service & { tiers: any[] }>(
    isAuthenticated ? `/api/service/${resolvedParams.merchantId}/${resolvedParams.serviceId}` : null,
    { 
      onSuccess: (data) => { 
        if (data?.tiers && data.tiers.length > 0) setSelectedTierId(data.tiers[0].id); 
      } 
    }
  );

  // ✅ FIX: TS2345 - Standardizing the fetcher signature for Next.js 15 compliance
  const { execute: triggerCheckout, loading: isSubmitting } = useInstitutionalFetch(
    async (payload: { merchantId: string; serviceId: string; tierId: string }) => {
    ...
    <div className="flex flex-col min-h-screen animate-in fade-in duration-700 leading-none bg-black">
      ...
      >
        <Button 
          ...
          // ✅ FIX: TS2345 - Passing the object payload to the trigger function
          onClick={() => { 
            impact("heavy"); 
            if (service?.id && selectedTierId) {
              triggerCheckout({ 
...
function IdentityNullFallback() {
  return (
    <div className="flex min-h-screen flex-col items-center justify-center p-6 bg-black text-center space-y-6">
      ...
      <Button onClick={() => window.location.reload()} className="h-11 px-8 rounded-xl bg-rose-500 text-white font-black text-[9px] uppercase italic">Initiate_Re-Sync</Button>
    </div>
  );
}
...
function NodeOfflineFallback({ router }: { router: any }) {
  return (
    <div className="flex min-h-screen flex-col items-center justify-center p-6 text-center space-y-6 bg-black">
      ...
      <Button onClick={() => router.push("/home")} className="h-11 px-8 rounded-xl bg-primary text-primary-foreground font-black text-[9px] uppercase italic">Return_to_Hub</Button>
    </div>
  );
}
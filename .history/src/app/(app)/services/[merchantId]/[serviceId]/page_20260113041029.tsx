...
/**
 * 🛰️ SERVICE DEPLOYMENT TERMINAL (Institutional v12.26.1)
...
export default function ServiceDetailPage({
  ...
  // 🚀 FIX: Unwrap params Promise using 'use'
  ...
  if (!auth.isAuthenticated && !auth.isLoading) {
    return (
      <div className="flex min-h-[80dvh] flex-col items-center justify-center p-6 text-center animate-in fade-in zoom-in">
        ...
        <Button onClick={() => window.location.reload()} className="mt-8">Re-Sync Node</Button>
      </div>
    );
  }
  ...
  if (!service && !apiLoading) {
    return (
      <div className="flex min-h-[80vh] flex-col items-center justify-center p-6 text-center">
        ...
        <h1 className="text-xl font-black uppercase italic opacity-40">Node_Offline</h1>
        <Button onClick={() => router.push("/home")} className="mt-6">Return to Hub</Button>
      </div>
    );
  }
      ...
      <main className="px-5 py-8 space-y-8 pb-44">
        ...
        <section className="space-y-4">
          <div className="flex items-center gap-3 px-2 opacity-30 italic">
             ...
             <h3 className="text-[9px] font-black uppercase tracking-[0.5em]">{isStaff ? "Audit_Node_Selection" : "Identity_Options"}</h3>
          </div>

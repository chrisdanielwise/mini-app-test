...
/**
 * 🛰️ SERVICE DEPLOYMENT TERMINAL (Institutional v12.41.0)
 * Logic: Hardened Ingress with BigInt-Safe Rendering.
 * Architecture: Optimized for Next.js 16 Promise unwrapping.
 */
export default function ServiceDetailPage({
  ...
  // 🛡️ LAYER 1: UNWRAP PARAMS (Promise must be unwrapped via 'use')
  ...
  // 🛰️ DATA INGRESS: Pulling individual node telemetry
  ...
  // 🛡️ LAYER 3: BIGINT-SAFE DATA SANITIZATION
  // Prevents crashes by explicitly casting database decimals/bigints to strings.
    ...
    return {
      ...
      tiers: Array.isArray(rawData.tiers) 
        ? rawData.tiers.map((t: any) => ({
            ...t,
            id: String(t.id),
            price: String(t.price), // Force Decimal to String
            name: String(t.name)
          }))
        : []
    };
  ...
  useEffect(() => {
    if (!isTelegram || !isReady || !auth?.isAuthenticated || !service)
      return;
  ...
  }, [service, selectedTierId, isSubmitting, isTelegram, isReady, auth?.isAuthenticated, isStaff, setMainButton]);

  // --- RENDERING BARRIERS ---
  if (!mounted || (!isReady && !isStuck) || apiLoading || auth?.isLoading) {

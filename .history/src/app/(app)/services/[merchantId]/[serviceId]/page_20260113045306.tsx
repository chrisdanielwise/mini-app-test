...
export default function ServiceDetailPage({
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      if (!isReady) {
        console.warn(
          "🛰️ [Service_Node] SDK Handshake timeout. Force-mounting UI via Auth."
        );
        setIsStuck(true);
      }
    }, 4000);
  ...
  useEffect(() => {
    ...
    if (!selectedTierId) {
      ...
      if (tier) {
        setMainButton({
          text: `ACTIVATE ${tier.name.toUpperCase()} - ${service.currency} ${
            tier.price
          }`,
          visible: true,
          active: !isSubmitting,
    ...
    return (
      <div className="flex min-h-[80dvh] flex-col items-center justify-center p-6 text-center animate-in fade-in zoom-in duration-700">
        ...
        <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mt-3 opacity-50">
          Identity node unverified.
          <br />
          Re-launch via official Bot terminal.
        </p>
        <Button
          onClick={() => window.location.reload()}
          variant="outline"
          className="mt-8 rounded-xl h-12 px-8 text-[10px] font-black uppercase tracking-widest"
        >
          Force Re-Sync
        </Button>
      </div>
  ...
  if (!service && !apiLoading) {
    return (
      <div className="flex min-h-[80vh] flex-col items-center justify-center p-6 text-center animate-in fade-in duration-500">
        ...
        <h1 className="text-xl font-black uppercase italic tracking-tight opacity-40">
          Node_Offline
        </h1>
        <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest mb-10 opacity-30 italic">
          Service ID {serviceId} not found in cluster.
        </p>
        <Button
      ...
      <header className="px-6 py-8 md:py-10 rounded-b-[2rem] border-b border-border/10 bg-card/40 backdrop-blur-2xl shadow-xl relative overflow-hidden">
        ...
        <div className="relative z-10 space-y-4">
          <div className="flex items-center justify-between">
            ...
            {isStaff && (
              <Badge className="bg-amber-500/10 text-amber-500 border-amber-500/20 text-[8px] font-black px-2 py-0.5">
                STAFF_OVERRIDE
              </Badge>
            )}
          </div>
      ...
      <main className="px-5 py-8 space-y-8 pb-44">
          ...
          {[
            {
              ...
              bg: isStaff ? "bg-amber-500/5" : "bg-primary/5",
            },
            {
              icon: Zap,
              label: "Instant",
            ...
            <div
              key={i}
              className="flex flex-col items-center gap-3 rounded-2xl border border-border/40 bg-card/40 p-4 shadow-lg backdrop-blur-md"
            >
        ...
        {!isTelegram && (
          <div className="fixed bottom-0 left-0 right-0 p-6 bg-gradient-to-t from-background via-background/95 to-transparent backdrop-blur-xl z-50">
            <Button
              className={cn(
                "h-14 md:h-16 w-full rounded-2xl text-xs font-black uppercase italic tracking-widest shadow-2xl transition-all",
                isStaff

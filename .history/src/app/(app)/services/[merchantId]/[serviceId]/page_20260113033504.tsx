...
/**
 * 🛰️ APEX DISCOVERY CORE (Institutional v12.27.0)
...
function ServicesTerminalContent() {
  ...
  const [isStuck, setIsStuck] = useState(false); // 🛡️ Unstick Safety Net

  // 🛡️ HYDRATION & TIMEOUT SHIELD
  useEffect(() => {
    if (mounted) {
      setTunnelReady(true);
      // 🚩 CRITICAL: Reset MainButton to prevent bleed-through from ServiceDetail
      setMainButton({ text: "", visible: false });
    }

    // Safety Net: Unstick page if Telegram SDK hangs > 4s
    const timer = setTimeout(() => {
       if (!isReady) {
         console.warn("🛰️ [Mesh_Node] SDK Handshake delay. Proceeding via Auth state.");
         setIsStuck(true);
       }
    }, 4000);

    return () => clearTimeout(timer);
  }, [mounted, isReady, setMainButton]);

  const urlMerchantId = searchParams.get("merchant");
  const targetMerchantId = urlMerchantId || auth?.user?.merchantId || tgUser?.merchantId;

  // 🛡️ DATA AUDIT
  const { data, isLoading: apiLoading, error } = useApi<MerchantData>(
  ...
  if (!auth?.isAuthenticated && (!isReady && !isStuck || !tunnelReady || auth?.isLoading || apiLoading)) {
    return <LoadingScreen message="ESTABLISHING IDENTITY LINK..." subtext="CALIBRATING MESH TELEMETRY" />;
  }
    ...
    <div className="flex flex-col min-h-[100dvh] pb-40 animate-in fade-in slide-in-from-bottom-6 duration-1000 max-w-7xl mx-auto text-foreground selection:bg-primary/30">
      ...
      <div className="px-5 md:px-10 space-y-10 mt-10">
        <div className="relative group">
          <Search className={cn("absolute left-6 top-1/2 h-5 w-5 -translate-y-1/2 text-muted-foreground transition-all duration-500 z-10 group-hover:scale-110", isStaff ? "group-focus-within:text-amber-500" : "group-focus-within:text-primary")} />
          <Input type="search" placeholder="FILTER SIGNAL NODES..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className={cn("pl-14 h-16 rounded-2xl bg-card/60 border-border/40 font-black uppercase italic text-xs tracking-tighter shadow-2xl backdrop-blur-md", isStaff ? "focus-visible:ring-amber-500/50" : "focus-visible:ring-primary/50")} />
        </div>
        ...
        <section className="space-y-8">
          <div className="flex items-center justify-between px-2 opacity-30 italic">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-[10px] font-black uppercase tracking-[0.5em]">{isStaff ? "Platform_Cluster_Audit" : "Available_Assets"}</h2>
            </div>
            <span className="text-[9px] font-black uppercase tracking-widest tabular-nums">{filteredServices?.length || 0} NODES ACTIVE</span>
          </div>

          {apiLoading ? <SkeletonList count={3} /> : error || (!targetMerchantId && !apiLoading) ? (
            <div className="rounded-[2.5rem] border-2 border-dashed border-rose-500/20 bg-rose-500/5 p-16 text-center space-y-6 shadow-inner relative overflow-hidden">
              ...
              <p className="text-xs font-black uppercase tracking-[0.4em] text-rose-500">{!targetMerchantId ? "Identity_Context_Missing" : "Protocol_Error: Cluster_Sync_Failed"}</p>
            </div>
            ...
            <div className="rounded-[3rem] border border-dashed border-border/20 bg-card/20 p-24 text-center shadow-inner relative overflow-hidden">
              ...
              <h3 className="text-xl font-black uppercase italic tracking-tighter opacity-40">Vault <span className="text-primary/60">Empty</span></h3>
            </div>
          )}
        </section>
      </div>
      ...
      <footer className="flex flex-col items-center gap-4 opacity-20 py-12 mt-auto">
         ...
         <p className="text-[8px] font-black uppercase tracking-[0.5em] italic text-center leading-none">Zipha Network Discovery // Protocol: Mesh_V2</p>
      </footer>
    </div>
  );
}

...
import { useInstitutionalAuth } from "@li/hooks/use-institutional-auth";
...
import { Globe, Terminal, ShieldCheck, Package, Waves, Activity } from "lucide-react";
...
function ServicesTerminalContent() {
  ...
  if (!isReady || isLocked) return <LoadingScreen message="SYNCING_DISCOVERY_MESH..." />;
   ...
   */
  const serviceGridCols = (screenSize === 'xxl' || screenSize === 'xl') ? "grid-cols-2 gap-10" : 
                          (isDesktop || (isTablet && !isPortrait)) ? "grid-cols-2 gap-6" : "grid-cols-1 gap-6";
        ...
        <section className="space-y-8 relative">
          <div className="flex items-center justify-between px-2 opacity-30 italic">
            <div className="flex items-center gap-3">
              <Terminal className={cn("size-4", isStaff ? "text-amber-500" : "text-primary")} />
          ...
          {loading ? (
            ...
            <div className={cn("grid transition-all duration-1000 ease-[cubic-bezier(0.2,0.8,0.2,1)]", serviceGridCols)}>
...
function EmptyMeshState() {
  return (
    <div className="rounded-[3rem] border border-dashed border-white/5 bg-card/10 p-24 text-center shadow-inner relative overflow-hidden group">
      ...
      <p className="text-[10px] font-black uppercase tracking-[0.4em] opacity-20 mt-4 italic">No active assets detected in this cluster.</p>
    </div>
  );
}
  ...
  return (
    <div className="rounded-[3rem] border-2 border-dashed border-rose-500/20 bg-rose-500/5 p-16 text-center space-y-8 shadow-inner relative overflow-hidden animate-in zoom-in-95 duration-700">
      ...
      <div className="space-y-2">
        <p className="text-sm font-black uppercase italic tracking-[0.4em] text-rose-500">{message}</p>
        <p className="text-[9px] font-black uppercase tracking-widest text-rose-500/40 italic">Check terminal identity signature or cluster availability.</p>
      </div>
    </div>
  );
}

...
/**
 * 🛰️ PROFILE IDENTITY NODE (Institutional v12.25.0)
...
export default function UserProfilePage() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
       if (!isReady) {
         console.warn("🛰️ [Profile_Node] SDK Handshake timeout. Force-mounting interface.");
         setIsStuck(true);
       }
    }, 4000);
  ...
  if (!auth.isAuthenticated && (!isReady && !isStuck || !tunnelReady || auth.isLoading)) {
    return <LoadingScreen message="DECRYPTING IDENTITY MANIFEST..." subtext="SECURE TUNNEL ACTIVE" />;
  }
  ...
  return (
    <div className="flex flex-col min-h-[100dvh] animate-in fade-in duration-700 max-w-3xl mx-auto text-foreground selection:bg-primary/30">
      ...
      <main className="px-5 py-8 space-y-10 pb-36">
        ...
        <section className="space-y-4">
           ...
           <div className="rounded-[2rem] border border-border/40 bg-card/40 divide-y divide-border/10 shadow-2xl backdrop-blur-md overflow-hidden">
              <Link href="/settings" onClick={() => triggerHaptic("light")} className="flex items-center justify-between p-6 hover:bg-muted/5 active:bg-muted/10 transition-all group">
                 <div className="flex items-center gap-4 min-w-0">
                    <div className={cn("h-11 w-11 shrink-0 rounded-xl flex items-center justify-center border shadow-inner transition-all", isStaff ? "bg-amber-500/5 border-amber-500/20" : "bg-primary/5 border-primary/20")}>
                       <Settings className={cn("h-5 w-5", isStaff ? "text-amber-500" : "text-primary")} />
                    </div>
                    <span className="text-[11px] font-black uppercase italic tracking-tight text-foreground/80 group-hover:text-primary">Hardware Sync</span>
                 </div>
                 <ChevronRight className="h-5 w-5 opacity-20 group-hover:translate-x-1 transition-all" />
              </Link>
              
              <Link href="/support" onClick={() => triggerHaptic("light")} className="flex items-center justify-between p-6 hover:bg-muted/5 active:bg-muted/10 transition-all group">
                 <div className="flex items-center gap-4 min-w-0">
                    <div className="h-11 w-11 shrink-0 rounded-xl bg-rose-500/5 flex items-center justify-center text-rose-500 border border-rose-500/20 shadow-inner group-hover:scale-110">
                       <HelpCircle className="h-5 w-5" />
                    </div>
                    <span className="text-[11px] font-black uppercase italic tracking-tight text-foreground/80 group-hover:text-rose-500">Emergency Protocol</span>
                 </div>
                 <ChevronRight className="h-5 w-5 opacity-20 group-hover:translate-x-1 transition-all" />
              </Link>

              {isStaff && (
                <Link href="/dashboard" onClick={() => triggerHaptic("medium")} className="flex items-center justify-between p-6 bg-amber-500/5 hover:bg-amber-500/10 transition-all group">
                   <div className="flex items-center gap-4 min-w-0">
                      <div className="h-11 w-11 shrink-0 rounded-xl bg-amber-500 flex items-center justify-center text-black border border-amber-500/20 shadow-inner group-hover:scale-110">
        ...
        <section className="space-y-5">
          <div className="flex items-center justify-between px-2">
            ...
            <Badge variant="outline" className={cn("text-[8px] font-black uppercase tracking-[0.2em] px-2 py-0.5 rounded-lg border-none", isStaff ? "text-amber-500 bg-amber-500/10" : "bg-muted/10 text-muted-foreground")}>
          ...
          {subsLoading ? <SkeletonList count={3} /> : allSubscriptions.length > 0 ? (
            ...
            <div className="rounded-[2rem] border border-dashed border-border/10 bg-card/5 p-16 text-center shadow-inner relative overflow-hidden">
               <User className="h-10 w-10 mx-auto text-muted-foreground opacity-5 animate-pulse" />
               <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground/30 italic">No verified signal nodes synchronized.</p>
            </div>
          )}
        </section>
      </main>
      ...
      <footer className="mt-auto flex flex-col items-center gap-4 opacity-20 py-10">
         ...
         <p className="text-[8px] font-black uppercase tracking-[0.5em] italic text-center leading-none">Zipha Identity Cluster synchronized // Sync: {isStaff ? "MASTER" : "NODE"}</p>
      </footer>
    </div>
  );
}
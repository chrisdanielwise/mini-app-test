...
/**
 * 🛰️ TERMINAL SETTINGS (Institutional v12.28.0)
...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
       if (!isReady) {
         console.warn("🛰️ [Settings_Node] SDK Handshake delay. Force-mounting UI via Auth state.");
         setIsStuck(true);
       }
    }, 4000);
      ...
      <main className="px-5 py-8 space-y-10 pb-36">
          ...
          <div className={cn("rounded-[2rem] border backdrop-blur-3xl p-6 md:p-8 shadow-2xl relative overflow-hidden group transition-all duration-500", isStaff ? "border-amber-500/20 bg-amber-500/5" : "border-border/40 bg-card/40")}>
            ...
            <div className="flex items-center gap-5 relative z-10">
              ...
              <div className="flex flex-col min-w-0">
                <span className="text-xl md:text-2xl font-black uppercase italic tracking-tight truncate leading-none">{userDisplay?.fullName || "Identity Unknown"}</span>
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  <Badge className={cn("text-[8px] font-black uppercase tracking-[0.2em] px-2 py-0.5 rounded-lg border-none", isStaff ? "bg-amber-500/10 text-amber-500" : "bg-primary/10 text-primary")}>{auth.user?.role?.toUpperCase() || "STANDARD_USER"}</Badge>
        ...
        <section className="space-y-4">
          ...
          <div className="flex flex-col gap-3">
            <SettingItem icon={User} label="Profile Metadata" sublabel="Identity labels & cluster bio" isStaff={isStaff} onTrigger={() => triggerHaptic("light")} />
            <SettingItem icon={Shield} label="Security Nodes" sublabel="Cryptographic session management" isStaff={isStaff} onTrigger={() => triggerHaptic("light")} />
            <SettingItem icon={Bell} label="Alert Frequency" sublabel="Signal notification latency" isStaff={isStaff} onTrigger={() => triggerHaptic("light")} />
            <SettingItem icon={Globe} label="Data Language" sublabel="Regional localized standards" isStaff={isStaff} onTrigger={() => triggerHaptic("light")} />
          </div>
        </section>

        <div className="pt-4 space-y-8">
          <Button variant="ghost" className="w-full h-14 rounded-2xl bg-rose-500/5 hover:bg-rose-500/10 text-rose-500 font-black uppercase italic tracking-widest text-[10px] border border-rose-500/10 shadow-lg" onClick={() => { triggerHaptic("warning"); window.location.reload(); }}>

    ...
    <div className="min-h-screen bg-background space-y-12 p-6 pb-32 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
        ...
        <div className="lg:col-span-2 space-y-12">
          ...
          {/* Section 1: Bot & Automation */}
          <section className="space-y-6">
            <div className="flex items-center gap-2 text-primary">
              <Bot className="h-4 w-4" />
              <h2 className="text-[10px] font-black uppercase tracking-[0.2em]">Signal Automation</h2>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                <Label className="text-[9px] font-black uppercase opacity-60 ml-1">Bot Username</Label>
                <Input className="rounded-xl border-muted bg-muted/20 h-12 font-bold" placeholder="@MySignalBot" />
              </div>
              <div className="space-y-2">
                <Label className="text-[9px] font-black uppercase opacity-60 ml-1">Bot Token (API)</Label>
                <div className="relative">
                  <Input type="password" defaultValue="••••••••••••••••" className="rounded-xl border-muted bg-muted/20 h-12 font-mono pr-12" />
                  <Key className="absolute right-4 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground opacity-30" />
                </div>
              </div>
            </div>

            {/* ✅ NEW: Global Webhook Toggle */}
            <div className="flex items-center justify-between p-6 rounded-[2.5rem] bg-primary/5 border border-primary/10">
              <div className="max-w-[75%]">
                <p className="text-xs font-black uppercase italic flex items-center gap-2">
                  Webhook Processing <Zap className="h-3 w-3 text-primary fill-primary" />
                </p>
                <p className="text-[8px] font-bold text-muted-foreground uppercase leading-relaxed">
                  Enable external signal injection (TradingView/MT5). When off, only manual signals are permitted.
                </p>
              </div>
          ...
          <section className="space-y-6 pt-6 border-t border-border">
            <div className="flex items-center gap-2 text-emerald-500">
              ...
              <h2 className="text-[10px] font-black uppercase tracking-[0.2em]">Interface Engine</h2>
            </div>
            
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="flex items-center justify-between p-6 rounded-[2.5rem] bg-muted/10 border border-border">
                <div className="max-w-[70%]">
                  <p className="text-xs font-black uppercase italic">Ultra-Wide</p>
                  <p className="text-[8px] font-bold text-muted-foreground uppercase">Expand dashboard to 1920px width.</p>
                </div>
                <Switch checked={isFullSize} onCheckedChange={toggleFullSize} className="data-[state=checked]:bg-primary" />
              </div>

              <div className="flex items-center justify-between p-6 rounded-[2.5rem] bg-muted/10 border border-border">
                <div className="max-w-[70%]">
                  <p className="text-xs font-black uppercase italic">Nav Mode</p>
                  <select 
                    value={navMode}
        ...
        <div className="space-y-6">
          <div className="rounded-[2.5rem] bg-card border border-border p-8 shadow-xl">
            ...
            <ul className="space-y-4">
              <li className="flex items-center gap-3">
                ...
                <span className="text-[10px] font-bold uppercase">Database Linked</span>
              </li>
              <li className="flex items-center gap-3">
                <Globe className="h-4 w-4 text-emerald-500" />
                <span className="text-[10px] font-bold uppercase">Node: Global Edge</span>
              </li>
            </ul>
          ...
          <div className="pt-20 flex flex-col items-center gap-4">
            <p className="text-[8px] font-black uppercase tracking-[0.4em] text-muted-foreground opacity-30 italic">Close Settings</p>
            <Link href="/dashboard">
              <Button 
                variant="outline" 

...
export function useAuth() {
    ...
    return new Promise((resolve) => {
      ...
      if (tg?.CloudStorage && tg.isVersionAtLeast("6.9")) {
        tg.CloudStorage.setItem(
          ...
          (err, success) => {
            if (success)
              console.log("☁️ [Auth] Identity anchored to Telegram Cloud.");
          }
        );
      }
  ...
  useEffect(() => {
    ...
    // 🛡️ AUTH SAFE-ZONE CHECK
    // We must skip auto-sync on these paths to prevent the hook
    // from trying to redirect the user while they are already trying to log in.
    const publicPaths = ["/dashboard/login", "/api/auth/magic"];
    ...
    if (publicPaths.some((p) => window.location.pathname === p)) {
      setState((prev) => ({
        ...
        isLoading: false,
        isReady: true,
        isAuthenticated: false, // Explicitly false so it doesn't try to sync
      }));
      return;
    }

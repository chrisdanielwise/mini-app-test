 ...
 * Strategy: Dual-Generic Signal Bridge.
 * Fix: Added <P> generic to support typed manual execution payloads.
 */
export function useInstitutionalFetch<T, P = any>(
   ...
   * Fix: Payload is now strictly typed as P to satisfy TS7006.
   */
  const execute = useCallback(async (isInitial = true, payload?: P) => {
    ...
    try {
      ...
      if (typeof endpointOrFn === 'function') {
        // ✅ FIX: Pass the typed payload into the function
  ...
  return useMemo(() => ({
    ...
    error,
    hasMore,
    execute, // ✅ Manual trigger now correctly typed via <P>
    refresh: () => {
      impact("light");
      return execute(true);
    },

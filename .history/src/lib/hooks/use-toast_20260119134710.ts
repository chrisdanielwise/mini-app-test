"use client";

import * as React from "react";
import type { ToastActionElement, ToastProps } from "@/components/ui/toast";
import { hapticFeedback } from "@/lib/telegram/webapp";

/**
 * 🛰️ TOAST ORCHESTRATOR (Institutional Apex v2026.1.20)
 * Philosophy: Atomic Lifecycle & Native Hardware Bridging.
 * Fix: Explicitly defined 'variant' in ToasterToast to resolve TS2339.
 * Fix: Typed 'open' parameter in onOpenChange to resolve TS7006.
 */

const TOAST_LIMIT = 1; 
const TOAST_REMOVE_DELAY = 5000;

// ✅ FIX: Explicitly extending ToastProps to include variant and status controls
type ToasterToast = ToastProps & {
  id: string;
  title?: React.ReactNode;
  description?: React.ReactNode;
  action?: ToastActionElement;
  variant?: "default" | "destructive" | "success" | "warning"; // Match your UI theme
};

// 🛡️ MEMORY MANAGEMENT
let memoryState: { toasts: ToasterToast[] } = { toasts: [] };
const listeners = new Set<(state: { toasts: ToasterToast[] }) => void>();

function dispatch(action: any) {
  memoryState = reducer(memoryState, action);
  listeners.forEach((listener) => listener(memoryState));
}

export const reducer = (state: { toasts: ToasterToast[] }, action: any) => {
  switch (action.type) {
    case "ADD_TOAST":
      return { ...state, toasts: [action.toast, ...state.toasts].slice(0, TOAST_LIMIT) };

    case "UPDATE_TOAST":
      return {
        ...state,
        toasts: state.toasts.map((t) =>
          t.id === action.toast.id ? { ...t, ...action.toast } : t
        ),
      };

    case "DISMISS_TOAST":
      return {
        ...state,
        toasts: state.toasts.map((t) =>
          t.id === action.toastId || action.toastId === undefined ? { ...t, open: false } : t
        ),
      };

    case "REMOVE_TOAST":
      return {
        ...state,
        toasts: action.toastId === undefined 
          ? [] 
          : state.toasts.filter((t) => t.id !== action.toastId),
      };
    default:
      return state;
  }
};

/**
 * 🚀 GLOBAL TACTICAL TRIGGER
 * Optimized for institutional alerts and hardware haptics.
 */
export function toast({ ...props }: Omit<ToasterToast, "id">) {
  const id = Math.random().toString(36).substring(2, 9);

  // 🏁 TACTILE SYNC (v9.5.0): Direct Hardware Bridge
  // Maps the software variant to the physical haptic style
  if (typeof window !== "undefined") {
    if (props.variant === "destructive") hapticFeedback("error");
    else if (props.variant === "success") hapticFeedback("success");
    else hapticFeedback("light");
  }

  const update = (newProps: Partial<ToasterToast>) =>
    dispatch({ type: "UPDATE_TOAST", toast: { ...newProps, id } });

  const dismiss = () => dispatch({ type: "DISMISS_TOAST", toastId: id });

  dispatch({
    type: "ADD_TOAST",
    toast: {
      ...props,
      id,
      open: true,
      // ✅ FIX: Added explicit boolean type to resolve TS7006
      onOpenChange: (open: boolean) => {
        if (!open) dismiss();
      },
    } as ToasterToast,
  });

  // Atomic cleanup queue
  setTimeout(() => {
    dispatch({ type: "REMOVE_TOAST", toastId: id });
  }, TOAST_REMOVE_DELAY + 1000);

  return { id, dismiss, update };
}

/**
 * 🕵️ useToast Hook (v16.16.12)
 * Logic: Set-based synchronization for fluid rendering.
 */
export function useToast() {
  const [state, setState] = React.useState(memoryState);

  React.useLayoutEffect(() => {
    listeners.add(setState);
    return () => {
      listeners.delete(setState);
    };
  }, []);

  return {
    ...state,
    toast,
    dismiss: (toastId?: string) => dispatch({ type: "DISMISS_TOAST", toastId }),
  };
}
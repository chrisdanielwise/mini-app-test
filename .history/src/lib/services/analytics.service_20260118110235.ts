 ...
 * Fix: Enforced explicit UUID casting and matched table names to @@map.
 */
export const getRevenueAnalytics = cache(async (merchantId?: string, range?: DateRange) => {
  ...
  try {
    // 🏗️ 1. BASELINE TOTALS (Prisma API)
    ...
    // 🏗️ 2. CHART TELEMETRY (Raw SQL)
    // ✅ FIX: Use Prisma.raw() for the fragment. We hard-code the merchantId into the string safely
    // because we validated it with isUUID() above.

...
export const AuditService = {
  ...
  async checkSuspiciousActivity(userId: string, currentIp: string) {
    try {
      ...
      // 🛡️ Ensure both IPs are treated as strings
      const [currentGeo, prevGeo] = await Promise.all([
        IPGeoService.resolve(currentIp ?? "0.0.0.0"),
        IPGeoService.resolve(previousLogin.ipAddress ?? "0.0.0.0"), // ✅ Fixed: Fallback for null
      ]);

      // 🚨 DISPATCH ALERT: If country codes do not match
      if (
        currentGeo.countryCode !== prevGeo.countryCode &&
        currentGeo.countryCode !== "LOC"
      ) {
...
*If this wasn't you, trigger a Remote Wipe immediately.*
  ...
  async getUserFromRequest(request: NextRequest) {
    try {
      ...
      // 🛡️ FIX: Check if token exists before passing to jwtVerify
      if (!token) {
        console.warn("🔐 [Auth]: No token found in request.");
        return null;
      }

      // Now 'token' is guaranteed to be a string here

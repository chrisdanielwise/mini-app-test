...
export const getMerchantActivity = cache(async (merchantId?: string, limit = 10) => {
  // 🔍 CONSOLE: Identity Ingress
  console.log(`📊 [Service_Trace]: Fetching Activity for Node_${merchantId?.slice(0, 8) || "NULL"}`);

  if (!merchantId || !isUUID(merchantId)) {
    console.warn("⚠️ [Service_Trace]: Aborting query - Invalid Merchant ID");
    return [];
  }
  ...
  try {
    const activity = await prisma.subscription.findMany({
      where: { 
        merchantId, 
        status: SubscriptionStatus.ACTIVE // ✅ FIX: Use Enum object, not "active"
      },
      orderBy: { createdAt: "desc" },
      take: limit,
      include: {
    ...
    console.log(`✅ [Service_Trace]: Successfully retrieved ${activity.length} logs`);
    return activity;
  } catch (error: any) {
    // 🔍 CONSOLE: Catching the Prisma crash
    console.error("🔥 [Service_Trace]: Prisma Query Crashed!", error.message);
    return [];
  }
});

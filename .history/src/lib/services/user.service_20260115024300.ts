...
/**
 * 🛰️ GET_USER_SUBSCRIPTIONS (v16.16.12)
 * Logic: Fetches all active service nodes for a specific identity.
 * Optimization: Uses indexed relation joins for sub-50ms resolution.
 */
export async function getUserSubscriptions(userId: string) {
  return await prisma.subscription.findMany({
    where: { 
      userId,
      status: "ACTIVE" // Only pull live nodes by default
    },
    include: {
      service: {
        select: {

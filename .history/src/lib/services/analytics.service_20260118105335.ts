...
/** 🧹 ATOMIC_SANITIZE: Standard BigInt hydration safety */
function sanitize<T>(data: T): T {
  return JSON.parse(JSON.stringify(data, (_, v) => 
    typeof v === "bigint" ? v.toString() : v
  ));
 ...
 * Fix: Resolved 'syntax error at or near $3' by using Prisma.raw for structural fragments.
 * Fix: Enforced explicit UUID casting to satisfy Postgres strict type-matching.
 */
export const getRevenueAnalytics = cache(async (merchantId?: string, range?: DateRange) => {
  ...
  try {
    ...
    // 🏗️ 2. CHART TELEMETRY
    // ✅ FIX: Use Prisma.raw() for the conditional AND clause to prevent parameter binding errors ($3).

...
import { PaymentStatus, OrderStatus, SubscriptionStatus, CouponScope } from "@/generated/prisma";
    ...
    return prisma.$transaction(async (tx) => {
      ...
      if (params.couponCode) {
        const coupon = await tx.coupon.findUnique({
          where: { merchantId_code: { merchantId: params.merchantId, code: params.couponCode } }
        });
  ...
  async createSubscriptionIntent(params: {
    ...
    if (params.couponCode) {
      const coupon = await prisma.coupon.findUnique({
        where: { merchantId_code: { merchantId: tier.service.merchantId, code: params.couponCode } }
      });

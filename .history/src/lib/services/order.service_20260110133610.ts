...
export class OrderService {
  ...
  static async getById(orderId: string) {
    ...
    const order = await prisma.order.findUnique({
      ...
      include: {
        ...
        merchant: {
          select: { id: true, companyName: true, contactSupportUrl: true },
        },
        rider: {
  ...
  static async assignRider(orderId: string, riderId: string) {
    if (!isUUID(orderId) || !isUUID(riderId))
    ...
    const order = await prisma.order.update({
      ...
      include: {
        rider: {
          include: {
            user: {
              select: { fullName: true, phoneNumber: true, telegramId: true },
            },
          },
        },
      },
  ...
  static async getReadyForPickup(merchantId?: string) {
    ...
    const orders = await prisma.order.findMany({
      ...
      include: {
        items: {
          include: { product: { select: { name: true, dispatchType: true } } },
        },

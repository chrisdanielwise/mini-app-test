...
export const getMerchantActivity = cache(async (merchantId?: string, limit = 10) => {
  // 🔍 CONSOLE: Entry Check
  console.log(`📊 [Service_Trace]: Fetching Activity for Node_${merchantId?.slice(0, 8)}`);

  if (!merchantId || !isUUID(merchantId)) {
    console.warn("⚠️ [Service_Trace]: Aborted - Invalid Merchant UUID");
    return [];
  }
  ...
  try {
    const activity = await prisma.subscription.findMany({
      where: { 
        merchantId, 
        // ✅ FIX: Use the Enum constant, NOT the string "active"
    ...
    console.log(`✅ [Service_Trace]: Success. Found ${activity.length} active records.`);
    return sanitize(activity);

  } catch (error: any) {
    // 🔍 CONSOLE: Forensic Error Capture
    console.error("🔥 [Prisma_Fatal]: Identity Handshake crashed during query!");
    console.error("📝 [Prisma_Context]:", error.message);
    
    // Return empty array instead of throwing to keep the user in the dashboard
    return [];
  }

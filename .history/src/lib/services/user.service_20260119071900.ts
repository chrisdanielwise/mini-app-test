...
export async function findOrCreateFromTelegram(telegramUser: TelegramUser) {
  ...
  return withRetry(async () => {
    ...
    // ✅ FIX: "en", "ru", etc (Prisma enums are case-sensitive)
    ...
    const user = await prisma.user.upsert({
      ...
      create: {
        ...
        lastName: telegramUser.last_name,
        language: lang,
        role: "user", // ✅ FIX: Lowercase per schema
      },
      include: {
    ...
    // ✅ FIX: In your environment, revalidateTag requires 2 arguments
  ...
  return withRetry(async () => {
    const user = await prisma.user.findUnique({
      ...
      include: {
        ...
        subscriptions: {
          where: { status: "active" }, // ✅ FIX: "active" per schema
...
export async function updateUser(userId: string, data: any) {
  ...
  return withRetry(async () => {
    ...
    // ✅ FIX: 2nd argument provided to satisfy TS2554
    revalidateTag("auth", "default");
 ...
 * Logic: Fetches all active service nodes.
 */

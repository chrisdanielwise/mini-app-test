...
/**
 * 🌊 ATOMIC_SANITIZE (Institutional v2026)
 * Architecture: Pure functional normalization for Next.js 16+ & Turbopack.
 * Logic: Recursively handles BigInt (TG_IDs) and Decimals (Liquidity).
 */
...
// =================================================================
// 🕵️ TELEMETRY EXPORTS (Hardened v16.32.12)
// =================================================================
...
export const getGlobalPlatformStats = cache(async () => {
  try {
    ...
    console.error("🔥 [Global_Telemetry_Fault]:", error);
    return null;
  }
});
...
export const getDashboardStats = cache(async (merchantId: string) => {
  ...
  try {
    ...
    console.error(`🔥 [Dashboard_Sync_Fault]: Node_${merchantId}`, error);
    return null;
  }
});

...
/**
 * 📊 GET DASHBOARD STATS
 * Logic: Uses strict Enum constants to satisfy Prisma Case-Sensitivity.
 * Fix: Replaced all string literals with Type-Safe Enum Objects.
 */
export const getDashboardStats = cache(async (merchantId?: string) => {
  ...
  if (merchantId && !isUUID(merchantId)) {
    throw new Error("SECURITY_ALERT: Invalid_Identity_Format");
  }
  ...
  try {
    const [totalRevenue, recentPayments, activeSubscriptions, pendingTickets] =
      await Promise.all([
        prisma.payment.aggregate({
          // ✅ FIX: Replaced "SUCCESS" string with Enum object
        ...
        prisma.subscription.count({ 
          // ✅ FIX: Replaced "ACTIVE" string with Enum object
          where: { ...where, status: SubscriptionStatus.ACTIVE } 
        }),
        prisma.ticket.count({ 
          // ✅ FIX: Replaced "OPEN" string with Enum object
    ...
    return {
      ...
      recentPayments: recentPayments.map((p) => ({
        ...p, // Handled by sanitize logic elsewhere
    ...
    return null; // Return null so UI can show 'Service Offline' instead of crashing
  }
});
 ...
 * Fix: Replaced "active" (lowercase) with SubscriptionStatus.ACTIVE (Uppercase Enum).
 * Fix: Added explicit try/catch to prevent the 500/Unauthorized loop on query failure.
 */
/**
 * ⚡ GET_MERCHANT_ACTIVITY (Apex Hardened v2026.1.18)
 * Fix: Replaced "active" with SubscriptionStatus.ACTIVE to satisfy strict Enum mapping.
 * Fix: Added safe fallback to prevent dashboard crashes on query faults.
 */
export const getMerchantActivity = cache(async (merchantId?: string, limit = 10) => {
  console.log(`📊 [Service_Trace]: Syncing Node_${merchantId?.slice(0, 8)}`);

  if (!merchantId || !isUUID(merchantId)) {
    console.warn("⚠️ [Service_Trace]: Identity node invalid or undefined.");
    return [];
  }
  ...
  try {
    const activity = await prisma.subscription.findMany({
      where: { 
        merchantId, 
        // ✅ CRITICAL FIX: Use the Enum constant, NOT the string "active"
    ...
    console.log(`✅ [Service_Trace]: Successfully retrieved ${activity.length} records.`);
    return sanitizeData(activity);

  } catch (error: any) {
    // 🔍 Capture exactly what Prisma didn't like
    console.error("🔥 [Prisma_Fatal]: Handshake crashed during query!");
    console.error("📝 [Diagnostic_Report]:", error.message);
    
    // Return empty array instead of throwing to prevent the /unauthorized redirect loop
    return [];
  }
});

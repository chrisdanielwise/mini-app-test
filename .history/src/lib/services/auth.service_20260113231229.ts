...
export const AuthService = {
 ...
 /**
   * 📡 VERIFY SESSION JWT
   * Hardened: Combined cryptographic check and DB stamp validation.
   */
  async verifySession(token: string) {
    try {
      ...
      // ✅ Uses the new internal method for stamp validation
      const isValid = await this.verifySessionStamp(sessionUser.id, sessionUser.securityStamp);
  ...
  /**
   * ⚡ THIN CHECK: STAMP VALIDATION
   * Logic: Performed during Heartbeats to verify identity isn't revoked.
   * Optimization: Only selects the 'securityStamp' column to reduce DB load.
   */
  async verifySessionStamp(userId: string, sessionStamp: string): Promise<boolean> {
    try {
      ...
      // Returns true only if the token's stamp matches the database
      return dbUser.securityStamp === sessionStamp;
    } catch (error) {
      console.error("🔥 [Auth_Stamp_Check_Failed]:", error);
      return false;
    }
  },
   ...
   * Logic: Syncs maxAge with tiered session rules (24h vs 7d).
   */

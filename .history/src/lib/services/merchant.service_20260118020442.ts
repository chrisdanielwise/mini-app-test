...
/**
 * 🛰️ GET_MERCHANT_ACTIVITY (Institutional v16.16.39)
 * Logic: Recent successful payments with explicit ServiceTier ingress.
 * Fix: Re-aligned to 'serviceTier' relation key to resolve ts(2353).
 */
export const getMerchantActivity = cache(async (merchantId: string) => {
  ...
  const activity = await prisma.payment.findMany({
    ...
    include: {
      ...
      // 🛡️ FIXED: Must match the field name in your Prisma Payment model
      serviceTier: { 
        select: { 
          price: true, 
          name: true 
 ...
 * Strategy: High-Density Telemetry Ingress.
 * Fix: Re-aligning relation key to standard 'serviceTier' mapping.
 */
export const getGlobalPlatformActivity = cache(async () => {
  const activity = await prisma.payment.findMany({
    ...
    include: {
      ...
      // 🚩 IF THIS ERRORS: Change 'serviceTier' to exactly what is in your schema.prisma
      serviceTier: { 
        select: { 
          price: true 
        } 

...
/**
 * 🛰️ CREATE_OR_EXTEND_SUBSCRIPTION (Institutional v2026.1.20)
 * Logic: Atomic growth or extension of existing access nodes.
 * Fix: Added 'await' to calculateExpiresAt to satisfy Date type requirements.
 */
export async function createOrExtendSubscription(input: {
  ...
  // 🛡️ 3. Temporal Alignment
  const now = new Date();
  const startDate = existing && existing.status === SubscriptionStatus.ACTIVE && existing.expiresAt > now
    ? existing.expiresAt
    : now;

  // ✅ FIX: TS2322 - Added 'await' because calculateExpiresAt is now an async Server Action
  const expiresAt = await calculateExpiresAt(tier.interval, 1, startDate);

  const sharedData = {
    serviceTierId,
    status: SubscriptionStatus.ACTIVE,
    expiresAt, // Now correctly resolved as a Date object
  ...
  // 🔄 5. Cache Purge (Next.js 16 Profile Standard)
  revalidateTag("auth", "page");

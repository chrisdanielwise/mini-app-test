...
/**
   * 🛡️ VERIFY SESSION (Institutional v14.29.0)
  ...
  static async verifySession(token: string) {
    try {
      ...
      // This prevents "Token Expired" errors during slow Turbopack compiles.
      const { payload } = await jose.jwtVerify(token, JWT_CONFIG.secret, {
        clockTolerance: 30, // 30 seconds of wiggle room
      });

      return payload;
    } catch (error: any) {
      // 🕵️ DEEP LOGGING: See exactly why it failed

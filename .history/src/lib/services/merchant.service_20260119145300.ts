 ...
 * Fix: Replaced strings with Enum Objects to match @map schema logic.
 */
    ...
    const [totalRevenue, recentPayments, activeSubscriptions, pendingTickets] =
      await Promise.all([
        prisma.payment.aggregate({
          // ✅ FIX: Use PaymentStatus.SUCCESS (Maps to "success" in DB)
        ...
        prisma.subscription.count({ 
          // ✅ FIX: Use SubscriptionStatus.ACTIVE (Maps to "active")
          where: { merchantId, status: SubscriptionStatus.ACTIVE } 
        }),
        prisma.ticket.count({ 
          // ✅ FIX: Use TicketStatus.OPEN (Maps to "open")

  ...
  try {
    const [totalRevenue, recentPayments, activeSubscriptions, pendingTickets] =
      await Promise.all([
        prisma.payment.aggregate({
          // ✅ FIX: Using PaymentStatus.SUCCESS (Uppercase Object)
          where: { ...where, status: PaymentStatus.SUCCESS },
          _sum: { amount: true }
        }),
        prisma.payment.findMany({
          // ✅ FIX: Using PaymentStatus.SUCCESS
          where: { ...where, status: PaymentStatus.SUCCESS },
        ...
        prisma.subscription.count({ 
          // ✅ FIX: Using SubscriptionStatus.ACTIVE
          where: { ...where, status: SubscriptionStatus.ACTIVE } 
        }),
        prisma.ticket.count({ 
          // ✅ FIX: Using TicketStatus.OPEN
    ...
    return {
      analytics: {
        ...
        chartData: [] // Ready for high-speed charting nodes
      },
      recentPayments: recentPayments.map((p) => ({
        ...sanitize(p), // Ensures BigInt (TG_IDs) are stringified
    ...
    // 🔍 Forensic Capture: Prevents the dashboard from crashing the user session
    console.error("🔥 [Dashboard_Stats_Crash]: Query Rejection ->", error.message);
    return null; 
  }
});
...
export const getMerchantActivity = cache(async (merchantId?: string, limit = 10) => {
  ...
  try {
    const activity = await prisma.subscription.findMany({
      where: { 
        merchantId, 
        status: SubscriptionStatus.ACTIVE // ✅ FIXED: Use Enum Object
      },
      orderBy: { createdAt: "desc" },
      take: limit,
      include: {

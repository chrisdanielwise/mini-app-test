...
async verifySession(token: string) {
    try {
      ...
      // 🛡️ INSTITUTIONAL VERIFICATION (v14.32.0)
      const { payload } = await jose.jwtVerify(
        token, 
        JWT_CONFIG.secret, 
        {
          clockTolerance: 60, // ⬆️ Increased to 60s for slow dev builds
      ...
      if (error.code === 'ERR_JWT_EXPIRED') {
        console.error("⏳ [Auth_Trace]: Token is mathematically expired.");
      } else if (error.code === 'ERR_JWS_SIGNATURE_VERIFICATION_FAILED') {
        console.error("🔑 [Auth_Trace]: Signature Mismatch. Check JWT_SECRET in .env.");
      } else {
        console.error(`🔐 [Auth_Trace]: ${error.code} | ${error.message}`);
      }
      return null;
    }
  },

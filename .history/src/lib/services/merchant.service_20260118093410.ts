...
export const getDashboardStats = cache(async (merchantId?: string) => {
  console.log(`📊 [Service_Trace]: Syncing stats for Node_${merchantId?.slice(0,8)}`);

  if (!merchantId || !isUUID(merchantId)) return null;

  try {
    const [totalRevenue, activeSubscriptions, pendingTickets] = await Promise.all([
      prisma.payment.aggregate({
        // ✅ FIX: Use PaymentStatus.SUCCESS (mapped to "success" in DB)
        where: { merchantId, status: PaymentStatus.SUCCESS },
        _sum: { amount: true }
      }),
      prisma.subscription.count({ 
        // ✅ FIX: Use SubscriptionStatus.ACTIVE (mapped to "active" in DB)
        where: { merchantId, status: SubscriptionStatus.ACTIVE } 
      }),
      prisma.ticket.count({ 
        // ✅ FIX: Use TicketStatus.OPEN (mapped to "open" in DB)
    ...
    return null; // Prevents the whole page from crashing
  }
});
...
export const getMerchantActivity = cache(async (merchantId?: string, limit = 10) => {
  ...
  try {
    const activity = await prisma.subscription.findMany({
      where: { 
        merchantId, 
        // ✅ FIX: Using SubscriptionStatus.ACTIVE (Resolves your Activity_Sync_Fault)

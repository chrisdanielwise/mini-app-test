...
import type { TelegramUser } from "@/src/lib/auth/telegram";
 ...
 * Wraps database calls to handle Neon Cold Starts (Connection timeouts).
 */
  ...
  while (attempts < maxAttempts) {
    try {
      ...
      if (isTimeout && attempts < maxAttempts) {
        console.warn(
          `[Neon Retry] ${label}: Cold Start detected. Attempt ${attempts}/${maxAttempts}. Waiting 2s...`
        );
...
export class UserService {
   ...
   * FIXED: This was where the 7.1s timeout was occurring in your logs.
   */

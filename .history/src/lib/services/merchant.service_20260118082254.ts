...
/**
 * 📊 GET_DASHBOARD_STATS
 * Logic: Uses strict Enum constants to satisfy Prisma Engine.
 */
  ...
  try {
    const [revenue, activeSubs, tickets] = await Promise.all([
      prisma.payment.aggregate({
        where: { 
          merchantId, 
          status: PaymentStatus.SUCCESS // ✅ FIX: Use Enum object
        },
        _sum: { amount: true },
      }),
      prisma.subscription.count({
        where: { 
          merchantId, 
          status: SubscriptionStatus.ACTIVE // ✅ FIX: Use Enum object
        },
      }),
      prisma.ticket.count({
        where: { 
          merchantId, 
          status: TicketStatus.OPEN // ✅ FIX: Use Enum object
        },
      }),
    ]);
    ...
    console.error("🔥 [Critical_Sync_Fault]:", error);
    return null;
  }
});

 ...
 * Fix: Positional arguments to prevent "Argument action is missing" Prisma errors.
 */
export async function logAuthEvent(userId: string, ip: string, actionType: string) {
  try {
    ...
    // 📝 Database Entry: Explicit Mapping
    const log = await prisma.activityLog.create({
      data: {
        actorId: userId,
        action: actionType as any, // Cast to any to handle Prisma Enum flexibility
        ipAddress: ip,
    ...
    // 🚨 SENTINEL ACTIVATION
    if (actionType === "LOGIN") {
      performSentinelAnalysis(userId, ip).catch((e) =>

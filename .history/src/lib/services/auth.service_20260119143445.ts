...
/**
 * 🔐 CREATE_JWT (Institutional Apex v2026.1.20)
 * Logic: Centralized signer for sessions and debug injectors.
 * Fix: Added telegramId to payload to maintain the Identity Link.
 */
export async function createJWT(payload: { 
  sub: string; 
  telegramId: string; // 🛰️ Required for Bot Routing & TMA Sync
  role: string; 
  ...
  return await new jose.SignJWT({
    ...
    user: {
      id: payload.sub,
      telegramId: payload.telegramId, // ✅ Injected into the user vault
 ...
 * Logic: Hydrates the full identity node before signing.
 */
export async function createSession(user: { id: string }) {
  const dbUser = await prisma.user.findUnique({
    ...
    select: { 
      role: true, 
      securityStamp: true,
      telegramId: true // ✅ Now fetching BigInt ID
    }
  });

  return await createJWT({
    sub: user.id,
    telegramId: dbUser?.telegramId.toString() || "0", // 🛡️ Stringify for JSON safety

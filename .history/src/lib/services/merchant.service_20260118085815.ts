...
export const getDashboardStats = cache(async (merchantId?: string) => {
  // 🔍 FORENSIC CONSOLE: The Query Params
  console.log("📊 [Service_Audit]: Telemetry Request", { 
    merchantId, 
    check: "Sending Uppercase Enums to Prisma..." 
  });
  ...
  try {
    const [revenue, activeSubs] = await Promise.all([
      prisma.payment.aggregate({
        where: { merchantId, status: PaymentStatus.SUCCESS }, // SUCCESS, not "success"
        _sum: { amount: true },
      }),
      prisma.subscription.count({
        where: { merchantId, status: SubscriptionStatus.ACTIVE }, // ACTIVE, not "active"
      }),
    ]);
    
    console.log("✅ [Service_Audit]: Telemetry Successfully Aggregated");
    return { revenue, activeSubs };
  } catch (error: any) {
    // 🔍 THIS WILL SHOW THE EXACT ENUM ERROR
    console.error("❌ [Prisma_Validation_Fault]:", error.message);
    return null;
  }
});

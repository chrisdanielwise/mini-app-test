...
/**
 * 📊 GET_DASHBOARD_STATS (Apex v2026.1.18)
 * Fix: Replaced lowercase literals with PaymentStatus.SUCCESS
 * Fix: Short-circuits if merchantId is missing to prevent global scans.
 */
export const getDashboardStats = cache(async (merchantId?: string) => {
  // 🛡️ IDENTITY_GUARD: Prevent un-indexed global scanning
  if (!merchantId || !isUUID(merchantId)) {
    console.warn("📡 [Stats_Sync]: Identity_Node_Missing_Or_Invalid");
    return null;
  }
  ...
  try {
    const [revenue, activeSubs, tickets] = await Promise.all([
      prisma.payment.aggregate({
        where: { merchantId, status: PaymentStatus.SUCCESS }, // ✅ FIXED ENUM
        _sum: { amount: true },
      }),
      prisma.subscription.count({
        where: { merchantId, status: SubscriptionStatus.ACTIVE }, // ✅ FIXED ENUM
      }),
      prisma.ticket.count({
        where: { merchantId, status: TicketStatus.OPEN }, // ✅ FIXED ENUM
      }),
    ]);
    ...
    console.error("🔥 [Telemetry_Sync_Fault]:", error);
    return null;
  }
});
 ...
 * Logic: Merged legacy logic with hardened status guards.
 */
export const getMerchantActivity = cache(async (merchantId?: string, limit = 10) => {
  ...
  const activity = await prisma.subscription.findMany({
    where: { 
      ...where, 
      status: SubscriptionStatus.ACTIVE // ✅ FIX: Use Enum
    },

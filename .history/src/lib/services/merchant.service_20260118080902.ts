...
/**
 * 📊 GET_DASHBOARD_STATS
 * Fix: Replaced lowercase strings with PaymentStatus.SUCCESS
 * Performance: Optimized for indexed scanning.
 */
    ...
    const [totalRevenue, recentPayments, activeSubscriptions, pendingTickets] =
      await Promise.all([
        prisma.payment.aggregate({
          // ✅ FIX: Use Enum instead of "success"
        ...
        prisma.subscription.count({ 
          // ✅ FIX: Use Enum instead of "active"
          where: { ...where, status: SubscriptionStatus.ACTIVE } 
        }),
        prisma.ticket.count({ 
          // ✅ FIX: Use Enum instead of "open"

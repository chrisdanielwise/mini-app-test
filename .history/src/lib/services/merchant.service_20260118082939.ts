...
export const getDashboardStats = cache(async (merchantId?: string) => {
  ...
  try {
    const [totalRevenue, recentPayments, activeSubscriptions, pendingTickets] =
      await Promise.all([
        prisma.payment.aggregate({
          // ✅ ENUM FIX: Replaced "success" string
        ...
        prisma.subscription.count({ 
          // ✅ ENUM FIX: Replaced "active" string
          where: { ...where, status: SubscriptionStatus.ACTIVE } 
        }),
        prisma.ticket.count({ 
          // ✅ ENUM FIX: Replaced "open" string
    ...
    console.error("🔥 [Dashboard_Data_Inaccessible]:", err);
    return null;
  }
});
...
});rt const getMerchantActivity = cache(async (merchantId?: string, limit = 10) => {
  if (merchantId && !isUUID(merchantId)) return [];

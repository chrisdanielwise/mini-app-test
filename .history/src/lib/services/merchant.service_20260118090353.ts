...
export const getDashboardStats = cache(async (merchantId: string) => {
  // 🔍 CONSOLE 1: Ingress Check
  console.log(`📊 [Service_Audit]: Starting Telemetry Sync for Node_${merchantId.slice(0,8)}`);
  ...
  try {
    ...
    console.log(`✅ [Service_Audit]: Telemetry Compiled in ${Date.now() - startTime}ms`);
    ...
    // 🔍 CONSOLE 2: The exact crash reason
    console.error("❌ [Service_Crash]: Prisma rejected the handshake:", error.message);
    return null;
  }
});
/**
...
export const getMerchantActivity = cache(async (merchantId?: string, limit = 10) => {
  ...
  const activity = await prisma.subscription.findMany({
    where: {
        ...where,
        status: SubscriptionStatus.ACTIVE // ✅ Fixed Case Sensitivity
    },

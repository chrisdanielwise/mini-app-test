...
/**
 * 🛡️ REVOKE_SERVICE_ACTION (v16.16.21)
 * Logic: Atomic Node Decoupling & Tier Liquidation.
 * Protocol: High-Friction Administrative Override.
 */
export async function revokeServiceAction(serviceId: string) {
  ...
  try {
    // 🔍 3. CLEARANCE_CHECK: Ensure operator has authority over this node
    ...
    // RBAC: Merchant can only revoke their own; Staff can override all.
    if (!session.isStaff && existingService.merchantId !== session.merchantId) {
      return { success: false, error: "SECURITY_ALERT: UNAUTHORIZED_OVERRIDE_ATTEMPT" };
    }
    ...
    await prisma.$transaction(async (tx) => {
      ...
      // Liquidate Tiers (Soft-revoke access for future subscriptions)
      await tx.serviceTier.updateMany({
        where: { serviceId },
        data: { isActive: false },
      });
      
      // LOG_EVENT: Internal Audit Trail
      console.log(`📡 [DECOUPLING_COMPLETE]: Node ${serviceId} revoked by ${session.user.id}`);
    });

    // 🔄 5. ATOMIC_CACHE_REVALIDATION
    revalidateTag("catalog_node");
    
    return { success: true };
  } catch (error: any) {
    console.error("🚨 [Revocation_Action_Error]:", error.message);
    return { success: false, error: "DECOUPLING_FAILURE: PERSISTENCE_SYNC_INTERRUPTED" };
  }
}
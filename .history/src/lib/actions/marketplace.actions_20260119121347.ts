 ...
 * Fix: Changed 'attributes' from optional to required to satisfy the Service layer.
 * Fix: Added .default({}) to ensure it is always sent in the payload.
 */
const ProductSchema = z.object({
  ...
  // ✅ FIX: Use nativeEnum to match the expected 'DispatchType' type exactly
  dispatchType: z.nativeEnum(DispatchType),
  variants: z.array(z.object({
    // ✅ FIX: Removed .optional() - attributes must be present to satisfy the service
...
export async function upsertProductAction(formData: FormData) {
  ...
  try {
    // 🏁 3. Service Execution
    // validated.data now perfectly matches the required Service parameter type
    ...
    // ✅ FIX: Next.js 15 requires "default" profile argument

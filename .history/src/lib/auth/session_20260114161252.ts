...
export async function requireAuth() {
  ...
  if (!session) {
    ...
    return redirect("/login?reason=auth_required");
  }
...
/**
 * 🛡️ STAFF GATEKEEPER (v14.39.0)
 * Logic: Validates session but delegates role-routing to the Layout.
 * Fix: Prevents "Guard_Fail" loops for Merchants by returning the session instead of redirecting.
 */
export async function requireStaff() {
  const session = await getSession();
  
  // 1. If no session exists, return null.
  // Logic: This allows the StaffRootLayout's 'isLoginPage' check to run first.
  if (!session) {
    return null; 
  }
  
  // 2. Telemetry: Log the ingress for debugging.
  const userRole = session.user.role?.toLowerCase();
  console.log(`🛡️ [Guard_Ingress]: Node ${session.user.id.slice(0,8)} identified as ${userRole}`);

  // 3. RETURN SESSION
  // Logic: We no longer call redirect() here for 'merchant' roles.
  // This allows the Layout to decide if a Merchant should see the dashboard.
  return session;
}
 ...
 * Integrated: Role-based validation and database cold-start resilience.
 */
export const getMerchantSession = cache(async (retryCount = 0): Promise<any | null> => {
  try {
    ...
    const identityNode = await Promise.race([
      ...
      if (
        retryCount < 2 && // Increased to 2 retries for better coverage
    ...
    // Using .toUpperCase() to ensure case-insensitive matching for roles
    if (!identityNode || identityNode.role.toUpperCase() !== "MERCHANT") {
      console.warn(`[Auth_Guard] Access denied for role: ${identityNode?.role}`);
      return null;
    }

    // 🔒 Specific Guard: Ensure user is actually a member of this merchant node
    ...
    return {
      ...
      staffRole: membership.role, // e.g., OWNER, ADMIN, AGENT
      config: {
...
export async function requireMerchantSession() {
  ...
  // If no session exists, redirect to login with the session_expired reason
  if (!session) {

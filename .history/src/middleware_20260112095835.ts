...
/**
 * 🛰️ GLOBAL GATEKEEPER (Proxy Protocol v8.9.7)
 * Hardened: Absolute bypass for login gate and passive reason-handling.
 * Optimization: Role normalization and origin reconstruction for Cloudflare/Ngrok.
 */
...
export async function middleware(request: NextRequest) {
  ...
  // Ensures redirects stay within the Cloudflare Tunnel or Proxy origin.
   ...
   * we yield control to the client-side Telegram SDK.
   */
  if (isLoginPage || searchParams.has('reason')) {
    ...
    // 🧹 ZOMBIE PURGE: If a token exists but we are at login with a reason,
    // clear it to ensure a clean Telegram Auth Handshake on the next attempt.
    if (staffToken && searchParams.has('reason')) {
      response.cookies.set('auth_token', '', { expires: new Date(0), path: '/' });
    }
    
    // Bypasses Tunnel warning pages for Telegram Mini App mobile views
  ...
  if (isDashboardRoute) {
    ...
    // Set explicit reason to trigger the Passive Loop Terminator above

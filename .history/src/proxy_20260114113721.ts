...
const PUBLIC_PASS_THROUGH = [
  ...
  '/dashboard/login', 
  '/unauthorized',    
  '/auth/callback',         // 🚀 ADD THIS: Allows the handshake engine to run
  '/api/auth/magic',        
  '/api/auth/telegram',    
  '/api/auth/logout',
...
export async function proxy(request: NextRequest) {
  ...
  // 🛡️ 1. ROOT EXEMPTION
  // If the user is on the landing page, do not force them to the dashboard
  // even if they have a valid merchant session.
  if (pathname === "/") {

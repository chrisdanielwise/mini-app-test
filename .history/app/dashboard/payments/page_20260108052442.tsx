...
export default async function TransactionsPage() {
  ...
  const transactions = await MerchantService.getTransactions(session.merchant.id)
    ...
    <div className="space-y-6 p-6 pb-20">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Historical log of all signal service payments.</p>
        </div>
        ...
        <Table>
          <TableHeader className="bg-muted/50">
            <TableRow>
              ...
              <TableHead className="font-bold uppercase text-[10px]">Service</TableHead>
                ...
                <TableRow key={tx.id} className="hover:bg-muted/30">
                  ...
                  <TableCell>
                    <Badge 
                      ...
                      className={tx.status === "SUCCESSFUL" ? "bg-emerald-500/10 text-emerald-500 border-emerald-500/20" : ""}
                    >
                      {tx.status}
                    </Badge>
                  </TableCell>

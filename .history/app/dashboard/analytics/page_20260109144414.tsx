...
/** * ✅ FIXED: Using Namespace Import to resolve 'MerchantService doesn't exist' error.
 * This bundles all named exports from your service file into one object.
 */
import * as MerchantService from "@/lib/services/merchant.service";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { TrendingUp, CreditCard, Activity, Users, Ticket, Zap } from "lucide-react";
...
export default async function AnalyticsPage() {
  ...
  // 🏁 2. Data Fetch: Synchronized with V2 Ledger Protocol
  const stats = await MerchantService.getDashboardStats(session.merchant.id);
  const analytics = stats.analytics;

  // 📈 3. Growth Protocol (Mock data for visualization)
  const chartData = [
  ...
  return (
    <div className="space-y-10 p-4 sm:p-8 pb-32 animate-in fade-in duration-700">
      ...
      <div className="rounded-[3rem] border border-border/50 bg-card/30 backdrop-blur-xl p-8 sm:p-12 shadow-2xl shadow-black/5 relative overflow-hidden">
        {/* Background Accent */}
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-12 gap-4 relative">
          ...
          <div className="flex gap-2">
             <div className="px-4 py-2 rounded-xl bg-muted/50 border border-border text-[9px] font-black uppercase tracking-widest cursor-default">
               Protocol: USD
             </div>
          </div>
        </div>

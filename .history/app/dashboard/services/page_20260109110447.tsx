...
/** * ✅ FIXED: Using named import for Turbopack compatibility.
 * Ensures the real database ID is used for fetching.
 */
...
/**
 * 🛠️ COMMAND CENTER: SERVICES
 * Uses dynamic merchant identification to fetch real deployment data.
 * Optimized for Schema V2.0.0 and high-resiliency auth.
 */
export default async function ServicesPage() {
  // 🔐 1. Authenticate and extract the REAL Merchant UUID from the session
  // This prevents the "demo-merchant" 400 validation error.
  const session = await requireMerchantSession();
  const realMerchantId = session.merchant.id;

  // 🏁 2. Fetch services using the sanitized UUID
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest opacity-70">
            Managing assets for <span className="text-primary">{session.merchant.companyName}</span>
          </p>
        </div>

        {/* ✅ Logic-Safe: Passing the real database ID to the deployment modal */}
                  ...
                  <td colSpan={5} className="px-6 py-24 text-center">
                    <div className="flex flex-col items-center gap-3">
                      ...
                      <p className="text-xs font-black uppercase italic text-muted-foreground">
                        No active services found in the ledger.
                      </p>
                      <p className="text-[9px] font-bold uppercase text-muted-foreground opacity-50">
                        Deploy your first signal service to begin.
                      </p>
                    </div>
                  </td>
                </tr>
                      ...
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                          <Button
                            variant="ghost"
                            size="icon"
                            className="h-10 w-10 rounded-2xl bg-muted/50 hover:bg-muted transition-colors"
                          >
                        ...
                        <DropdownMenuContent
                          align="end"
                          className="w-[200px] rounded-2xl border-border shadow-2xl p-2"
                        >
                          <DropdownMenuItem
                            asChild
                            className="rounded-xl font-bold uppercase text-[10px] py-3 cursor-pointer"
                          >
                          ...
                          >
                            <Link
                              href={`/dashboard/subscribers?serviceId=${service.id}`}
                            >
                              Ledger History
                            </Link>
                          </DropdownMenuItem>

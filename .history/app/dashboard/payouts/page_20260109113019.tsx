  ...
  return (
    <div className="space-y-8 p-6 pb-20 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <RequestPayoutModal merchantId={realMerchantId} availableBalance={balance.available} />
      </div>
          ...
          <Table>
            <TableHeader className="bg-muted/50 border-b border-border">
              <TableRow className="hover:bg-transparent border-none">
                <TableHead className="px-8 py-5 font-black uppercase text-[10px] tracking-widest text-muted-foreground">Reference</TableHead>
                <TableHead className="py-5 font-black uppercase text-[10px] tracking-widest text-muted-foreground">Date</TableHead>
                <TableHead className="py-5 font-black uppercase text-[10px] tracking-widest text-muted-foreground">Amount</TableHead>
                <TableHead className="py-5 font-black uppercase text-[10px] tracking-widest text-muted-foreground">Method</TableHead>
                <TableHead className="px-8 py-5 text-right font-black uppercase text-[10px] tracking-widest text-muted-foreground">Status</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {payouts.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={5} className="h-48 text-center">
                    <div className="flex flex-col items-center gap-2 opacity-30">
                      ...
                      <p className="text-[10px] font-black uppercase italic tracking-widest">No payout history on record</p>
                    </div>
                  </TableCell>
                </TableRow>
              ) : (
                payouts.map((payout: any) => (
                  <TableRow key={payout.id} className="hover:bg-muted/30 transition-all border-b border-border/50 last:border-none">
                    ...
                    <td className="py-6">
                      <p className="text-sm font-black tracking-tight">${Number(payout.amount).toFixed(2)}</p>
                    </td>
                    <td className="py-6">

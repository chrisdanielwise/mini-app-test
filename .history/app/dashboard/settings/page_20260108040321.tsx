 ...
 * Note: Added string conversion for ID safety
 */
async function updateMerchantAction(formData: FormData, merchantId: string) {
  ...
  // Prisma 7 Stability: We use the ID directly from the component loop
      ...
      <div className="grid gap-8">
        ...
        <div className="rounded-2xl border border-border bg-card shadow-sm overflow-hidden">
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="flex items-center justify-between px-2">
              <span className="text-sm font-bold uppercase tracking-widest text-muted-foreground">System Status</span>
              <div className="flex items-center gap-3">
                <span
                  className={`h-3 w-3 rounded-full ${merchant.provisioningStatus === "READY" ? "bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.5)]" : "bg-orange-500"
                    }`}
                />
                <span className="text-sm font-black italic uppercase">
                  {merchant.provisioningStatus === "READY" ? "Live" : merchant.provisioningStatus}
                </span>
              </div>
            </div>
          </div>
        ...
        <div className="rounded-2xl border border-destructive/20 bg-destructive/5 p-8">
          <div className="flex flex-col sm:flex-row items-start justify-between gap-6">
            <div className="space-y-1">
              <h2 className="text-xl font-black text-destructive italic uppercase">Danger Zone</h2>
              <p className="text-sm text-muted-foreground">
                Permanently delete your merchant profile. This deletes all service history, subscribers, and payments.
              </p>
            </div>
            <Button variant="destructive" className="shrink-0 rounded-xl font-bold h-12 px-6">

...
export default function ProfilePage() {
   ...
   * Ensures the GreysuitFx UUID is resolved from the database 
   * as soon as the profile page loads.
   */
  useEffect(() => {
  ...
  /**
   * 🚀 THE BRIDGE REDIRECT:
   * Updated to use window.location.href for a clean redirect 
   * and prioritized ID resolution.
   */
  const handleSwitchToDashboard = async (e: React.MouseEvent) => {
    ...
    // 🔄 Instant Fallback Sync if first attempt failed
    ...
    if (merchantId && telegramId) {
      // Construction of absolute URL is handled on the server (callback)

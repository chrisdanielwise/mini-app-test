...
export default function ProfilePage() {
   ...
   * If the auth context is missing the merchantId, we fetch it using the
   * telegramId as the key. This prevents the "Auth Data Missing" console error.
   */
  useEffect(() => {
    async function resolveMerchantIdentity() {
      // Only run if we have a Telegram ID but no Merchant ID in state
  ...
  // 🛡️ ROLE CHECK: Validates Merchant/Admin access
  const isMerchant =
    auth.user?.role === "MERCHANT" || auth.user?.role === "ADMIN";

  const handleMenuClick = () => {
    const hapticsEnabled =
      localStorage.getItem("user_haptics_enabled") !== "false";
    if (hapticsEnabled) {
  ...
  /**
   * 🚀 THE BRIDGE REDIRECT:
   * Uses a prioritized ID lookup to ensure the user is never stuck.
   */
  const handleSwitchToDashboard = (e: React.MouseEvent) => {
    ...
    // Check local sync first, then auth context, then schema relation
    const merchantId =
      syncedMerchantId ||
      auth.user?.merchantId ||
      auth.user?.merchantProfile?.id;
    const telegramId = user?.id?.toString();

    if (merchantId && telegramId) {
      setIsRedirecting(true);
      // Constructing the callback URL for the stateless JWT handshake
      ...
      console.error("Auth Data Missing", {
        merchantId,
        telegramId,
        syncedId: syncedMerchantId,
        authContext: auth.user,
      });
      alert(
        "Establishing secure link to GreysuitFx... Please tap again in 2 seconds."
      );
    }
  };
      ...
      <header className="border-b border-border bg-card/30 p-6 pt-10">
        <div className="flex items-center gap-5">
          ...
          <div className="flex-1 space-y-1">
            ...
            <Badge
              variant="outline"
              className="border-primary/30 bg-primary/5 text-[10px] font-bold uppercase tracking-wider text-primary"
            >
      ...
      <div className="space-y-8 px-4">
        ...
        {isMerchant && (
          <section className="space-y-4 rounded-3xl border border-primary/20 bg-primary/5 p-4 shadow-sm animate-in zoom-in-95 duration-300">
            <div className="flex items-center justify-between px-1">
              ...
              <Badge className="bg-primary text-[9px] font-black italic tracking-tighter">
                LIVE HUB
              </Badge>
            </div>
        ...
        <section className="pt-4 border-t border-border/50">
          ...
          <div className="mt-6 flex flex-col items-center gap-1 opacity-40">
            <p className="text-[9px] font-black tracking-widest uppercase italic">
              Zipha Forex Provision
            </p>

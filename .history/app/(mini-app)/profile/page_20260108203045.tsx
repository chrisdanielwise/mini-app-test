...
export default function ProfilePage() {
  ...
  /**
   * 🚀 THE BRIDGE FIX:
   * Instead of a static <Link>, we use a function to force the Auth Callback.
   * This syncs the Stateless JWT cookie to prevent the dashboard spin.
   */
  const handleSwitchToDashboard = () => {
    handleMenuClick();
    if (auth.user?.merchantId && user?.id) {
      window.location.href = `/api/auth/callback?merchantId=${auth.user.merchantId}&telegramId=${user.id}`;
    }
  };
          ...
          <section className="space-y-4 rounded-3xl border border-primary/20 bg-primary/5 p-4 shadow-sm">
            ...
            <div className="grid grid-cols-2 gap-3">
              <button
                ...
                className="flex flex-col gap-2 rounded-2xl bg-card p-4 border border-border active:scale-95 transition-transform text-left"
              >
            ...
            <Button
              variant="outline"
              onClick={handleSwitchToDashboard}
              className="w-full rounded-xl border-primary/20 bg-card text-[10px] font-black uppercase h-10"
            >
        ...
        <section>
          ...
          {subsLoading ? (
            ...
            <div className="rounded-2xl border-2 border-dashed border-border bg-muted/20 py-10 text-center">
              <p className="text-xs font-bold text-muted-foreground uppercase opacity-60">No active signal services</p>
            </div>
          )}
        </section>

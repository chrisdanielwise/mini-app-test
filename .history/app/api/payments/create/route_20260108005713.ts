...
import { successResponse, validationError, notFoundResponse, serverError, errorResponse } from "@/lib/utils/api-response"
...
export async function POST(request: NextRequest) {
  return withAuth(request, async (req: AuthenticatedRequest, context: AuthContext) => {
    try {
      ...
      if (!isUUID(merchantId) || !isUUID(serviceId) || !isUUID(serviceTierId)) {
        return errorResponse("One or more provided IDs have an invalid format (UUID expected).", 400)
      }
      ...
      if (!tier || tier.service.merchantId !== merchantId || tier.serviceId !== serviceId) {
        return notFoundResponse("Service tier not found for this merchant/service")
      }

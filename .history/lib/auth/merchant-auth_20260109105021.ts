...
export async function getMerchantSession(retryCount = 0): Promise<any | null> {
  try {
    ...
    const merchant = await Promise.race([
      ...
      if (
        retryCount < 1 && 
        (err.message.includes("TIMEOUT") || err.message.includes("connection") || err.code === 'P2028')
      ) {
        console.warn(`⏳ Auth retry ${retryCount + 1}/1: Database is lagging...`);
    ...
    if (!merchant || !merchant.adminUser) {
      console.warn(`[Auth] No merchant profile linked to token ID: ${merchantId}`);
      return null;
    }

...
import { isUUID } from "@/lib/utils/validators" // Assuming the validator utility we created
...
export class AnalyticsService {
  ...
  static async getRevenueAnalytics(merchantId: string, range: DateRange) {
    ...
    payments.forEach((payment) => {
      ...
      // Use Decimal.toNumber() for Prisma 7 compatibility
      const amount = typeof payment.amount === 'object' ? (payment.amount as any).toNumber() : Number(payment.amount)
    ...
    return {
      total: totalRevenue.toFixed(2), // Return as string for JSON precision
      currency: "USD",
  ...
  static async getTopServices(merchantId: string, limit = 5) {
    ...
    return services.map((service) => ({
      ...
      totalRevenue: service.payments.reduce((sum, p) => {
        const val = typeof p.amount === 'object' ? (p.amount as any).toNumber() : Number(p.amount)
        return sum + val
      }, 0).toFixed(2),
    }))
  }
   ...
   * Updated for Prisma 7 upsert syntax
   */
  static async recordDailyMetrics(merchantId: string) {
    ...
    const [newSubscribers, churned, revenue, activeEnd] = await Promise.all([
      ...
      prisma.payment.aggregate({
        where: { merchantId, status: "SUCCESS", createdAt: { gte: today } },
        _sum: { amount: true },
      }),
      prisma.subscription.count({
        where: { merchantId, status: "ACTIVE" },

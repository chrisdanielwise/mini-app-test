 ...
 * Handles Neon cold starts by retrying connection-related failures with exponential backoff.
 */
  ...
  while (attempts < maxAttempts) {
    try {
      ...
      if (isConnectionError && attempts < maxAttempts) {
        console.warn(`[Neon Retry] ${label}: Cold Start/Timeout detected. Attempt ${attempts}/${maxAttempts}. Retrying in ${delay}ms...`);
...
export class AnalyticsService {
  /**
   * 📊 REVENUE ANALYTICS
   * Optimized with aggregate functions to minimize data transfer and compute load.
   */
  static async getRevenueAnalytics(merchantId: string, range: DateRange) {
    ...
    return withRetry(async () => {
      // 1. Get aggregate totals for the period in a single DB round-trip
   ...
   * Tracks active vs. churned subscribers over the selected range.
   */
   ...
   * Returns a breakdown of services by active users and historical revenue.
   */
   ...
   * Ensures the chart receives a data point for EVERY day in the interval, even if the value is zero.
   */

...
export function LoadingSpinner({ className, size = "md" }: LoadingSpinnerProps) {
  const sizeClasses = {
    // sm: Standard small spinner
    sm: "h-4 w-4 border-2",
    // md: Dashboard card level spinner
    md: "h-6 w-6 border-2",
    // lg: Full page/Screen level spinner
...
export function LoadingScreen({ message = "Initializing Secure Link..." }: { message?: string }) {
  return (
    <div className="flex h-[80vh] w-full flex-col items-center justify-center gap-6 bg-background">
      ...
      <div className="flex flex-col items-center gap-1 text-center">
        ...
        <p className="text-[10px] font-bold uppercase text-muted-foreground/60">
          Neon Database Cold Start: 10-15s
        </p>
      </div>
    </div>
  )

 ...
 * Uses the updated getMerchantActivity with Schema V2.0.0 support.
 */
export async function AsyncActivityFeed({
  ...
  const data = await getMerchantActivity.getDashboardStats(merchantId);
        ...
        {payments.map((payment) => (
          ...
          >
            <div className="flex items-center gap-4">
              <div
                className={`p-3 rounded-2xl ${getStatusColor(payment.status)}`}
              >
              ...
              <div className="flex flex-col">
                ...
                <span className="text-[10px] font-bold text-muted-foreground uppercase">
                  {payment.service?.name || "Digital Service"} •{" "}
                  {format(new Date(payment.createdAt), "HH:mm, MMM dd")}
                </span>
              </div>
            </div>

...
// ✅ FIXED: Using the correct hyphenated path to match your file structure
 ...
 * Atomically registers a digital asset and its full pricing architecture.
 */
export function CreateServiceModal({ merchantId }: CreateServiceModalProps) {
  const [open, setOpen] = useState(false);

  // 💎 INITIAL STATE: Matches Schema V2 Enums (CUSTOM instead of STANDARD)
  ...
  // 🔄 HYDRATION: Reset state on successful deployment
  useEffect(() => {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button className="rounded-2xl h-14 px-8 font-black uppercase italic tracking-widest shadow-xl shadow-primary/20 hover:scale-105 transition-all bg-primary text-primary-foreground active:scale-95">
      ...
      <DialogContent className="max-w-3xl rounded-[3rem] border-border bg-card p-0 overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300">
        <form action={formAction} className="flex flex-col max-h-[90vh]">
          {/* 🔑 IDENTITY HANDSHAKE: Hidden field for server-side ownership verification */}
          ...
          <div className="p-10 space-y-10 overflow-y-auto scrollbar-hide">
            <DialogHeader>
              ...
              <DialogDescription className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground opacity-70">
                Establish a new digital asset node and its initial pricing tiers.
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="space-y-6">
              <div className="flex items-center gap-2 text-primary">
                ...
                <h3 className="text-[11px] font-black uppercase tracking-[0.2em]">Asset Identity</h3>
              </div>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  <Label className="text-[10px] font-black uppercase ml-1">Service Label</Label>
                  <Input
                    name="name"
                    placeholder="VIP GOLD SIGNALS"
                    required
                    className="h-14 rounded-2xl border-muted bg-muted/10 font-black uppercase italic text-xs tracking-tight"
                  />
                </div>
                <div className="space-y-2">
                  <Label className="text-[10px] font-black uppercase ml-1">Category Node</Label>
                  <Input
                    name="categoryTag"
                    placeholder="FOREX"
              ...
              <div className="space-y-2">
                <Label className="text-[10px] font-black uppercase ml-1">Asset Description</Label>
                <Textarea
                  name="description"
                  placeholder="Summarize signal frequency and accuracy..."
                  className="rounded-2xl min-h-[100px] bg-muted/10 border-muted font-bold text-xs"
                />
              </div>

              <div className="space-y-2">
                <Label className="text-[10px] font-black uppercase ml-1 text-primary">Target Telegram Channel ID</Label>
                <Input
                  name="vipChannelId"
                ...
                <div className="flex items-center gap-1.5 px-1 opacity-50">
                  ...
                  <p className="text-[8px] font-bold uppercase">Include -100 prefix for secure private nodes.</p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-6 pt-10 border-t border-border/50">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2 text-emerald-500">
                  ...
                  <h3 className="text-[11px] font-black uppercase tracking-[0.2em]">Pricing Architecture</h3>
                </div>
                <Button 
                  type="button" 
                  onClick={addTierRow}
                ...
                >
                  <Plus className="h-3 w-3 mr-1" /> Add Plan Node
                </Button>
              </div>
                ...
                {tiers.map((tier) => (
                  ...
                  >
                    <div className="col-span-12 sm:col-span-5 space-y-2">
                      <Label className="text-[8px] font-black uppercase opacity-50">Tier Name</Label>
                      <Input
                        name="tierNames[]"
                    ...
                    <div className="col-span-6 sm:col-span-3 space-y-2">
                      <Label className="text-[8px] font-black uppercase opacity-50">Price (USD)</Label>
                      <Input
                        name="tierPrices[]"
                        type="number"
                        step="0.01"
                    ...
                    <div className="col-span-4 sm:col-span-3 space-y-2">
                      <Label className="text-[8px] font-black uppercase opacity-50">Cycle & Type</Label>
                      <select
                        name="tierIntervals[]"
                      ...
                      >
                        {/* ✅ FIXED: Values match IntervalUnit enum exactly */}
                      ...
                      {/* ✅ FIXED: Hidden input uses CUSTOM to match your SubscriptionType schema */}
          ...
          <div className="bg-muted/30 p-8 flex items-center justify-end gap-4 border-t border-border/50 backdrop-blur-xl">
            <Button
              ...
              className="rounded-xl h-14 px-8 text-[11px] font-black uppercase tracking-widest text-muted-foreground hover:text-foreground h-12"
            >
              Abord Deployment
            </Button>
            <Button
              type="submit"
              disabled={isPending}
              className="flex-1 max-w-[280px] rounded-[1.5rem] bg-primary h-14 text-[12px] font-black uppercase italic tracking-[0.1em] shadow-2xl shadow-primary/20 transition-all hover:translate-y-[-2px] active:scale-95 active:translate-y-[1px]"
            >
              {isPending ? (
                <>

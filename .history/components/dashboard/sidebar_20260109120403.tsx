...
import {
  ...
  BarChart3, MessageSquare, Bot, Tag, Wallet, Sparkles, User
} from "lucide-react"
    ...
    <aside className="hidden lg:flex w-72 flex-col border-r border-border bg-card/50 backdrop-blur-xl h-screen sticky top-0">
      ...
      <div className="p-6 border-t border-border">
        <div className="rounded-[2rem] bg-card border border-border p-5">
            <p className="text-[10px] font-black text-primary uppercase mb-1">Tier: {merchant.planStatus || "Starter"}</p>
            <Link href="/dashboard/billing" className="text-[9px] font-bold underline uppercase">Upgrade Suite</Link>
        </div>
      </div>
    </aside>
  )

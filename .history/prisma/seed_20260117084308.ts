...
import { SubscriptionStatus, Prisma } from '../src/generated/prisma';

// ... inside main ...

console.log('🛡️  Attempting Merchant Creation with Strict Type Forcing...');

// We use 'create' instead of 'upsert' to simplify the validation path.
// If it already exists, we catch the error and move on.
let greyMerchant: any;

try {
  greyMerchant = await retry(() => prisma.merchantProfile.create({
    data: {
      adminUserId: 994858530n,
      companyName: "GreysuitFx",
      // Use the actual ID from the plan we created earlier in the script
      planId: enterprisePlan.id, 
      botId: 6838294498n,
      botToken: "6838294498:AAHR907AbARqogosjcVtSscTyhBWHiGYQDc",
      // ✅ THE PERMANENT FIX: Object reference + Type Assertion
      planStatus: SubscriptionStatus.ACTIVE as SubscriptionStatus, 
      availableBalance: new Prisma.Decimal("1000"),
      botConfig: {
        create: { welcomeMessage: "GreysuitFx Bot Active" }
      },
      analytics: {
        create: { totalRevenue: new Prisma.Decimal("0") }
      },
      paymentConfig: {
  ...
  if (e.code === 'P2002') {
    console.log('ℹ️  Merchant already exists, fetching existing record...');
...
console.log('✅ Merchant Profile Sync Complete.');

...
// 1. Force the role and language into the correct type shape manually
const greyUserData: unknown = {
  telegramId: 994858530n,
  fullName: "GreysuitFx Admin",
  username: "greysuit_admin",
  email: "admin@greysuitfx.com",
  // We use lowercase strings and cast to any to stop the Engine from complaining
  role: UserRole.MERCHANT as any, 
  language: "en" as any,
};

// 2. Execute the upsert using the bypass data
const greyUser :any = await retry(() =>
  prisma.user.upsert({
    where: { telegramId: greyUserData.telegramId },
    update: {},
    create: greyUserData,
  })
);

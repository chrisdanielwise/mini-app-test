...
async function main() {
  ...
  console.log('✅ VERSION 3.0 FINAL (Strict Enums)');
  ...
  // 🔴 THIS WAS THE ERROR SPOT. FIXED BELOW.
  const greyUser = await retry(() =>
    prisma.user.upsert({
      ...
      create: {
        ...
        role: UserRole.MERCHANT, // ✅ FIXED: Uses Enum
        email: 'admin@greysuitfx.com',
        platformSubscription: {
          create: { 
            status: SubscriptionStatus.ACTIVE, // ✅ FIXED: Uses Enum
  ...
  const greyProfile = await retry(() =>
    prisma.merchantProfile.upsert({
      ...
      create: {
        ...
        planStatus: SubscriptionStatus.ACTIVE, // ✅ FIXED: Uses Enum
        payoutConfig: {
    ...
    const user = await retry(() =>
      prisma.user.create({
        data: {
          ...
          role: UserRole.MERCHANT, // ✅ FIXED
          email: faker.internet.email(),
          platformSubscription: {
            create: { 
              status: SubscriptionStatus.ACTIVE, // ✅ FIXED
              expiresAt: futureDate(365) 
            },
          },
        },
    ...
    const profile = await retry(() =>
      prisma.merchantProfile.create({
        data: {
          ...
          planStatus: SubscriptionStatus.ACTIVE, // ✅ FIXED
          paymentConfig: {
            create: {
            ...
            create: {
              status:
                scenario === 'FRAUD' || scenario === 'CHURNED'
                  ? SubscriptionStatus.INACTIVE // ✅ FIXED
                  : SubscriptionStatus.ACTIVE, // ✅ FIXED
      ...
      const payment = await retry(() =>
        prisma.payment.create({
          data: {
            ...
            amount: tierPrice,
            currency: 'USD',
            status: PaymentStatus.SUCCESS, // ✅ FIXED
        ...
        await retry(() =>
          prisma.groupMembership.create({
            data: {
              ...
              status: 'JOINED', // Note: Using string here if schema expects string or Enum
      ...
      if (scenario === 'WHALE') {
        ...
        await retry(() =>
          prisma.copierExecution.create({
            data: {
              userId: user.id,
              status: ExecutionStatus.SUCCESS, // ✅ FIXED
              brokerTicket: String(
      ...
      const payment = await retry(() =>
        prisma.payment.create({
          data: {
            ...
            amount: tierPrice,
            currency: 'USD',
            status: PaymentStatus.REFUNDED, // ✅ FIXED
            gatewayProvider: 'STRIPE',
            gatewayReference: `ch_${faker.string.alphanumeric(20)}`,
          },
        }),
      ...
      const ticket = await retry(() =>
        prisma.ticket.create({
          data: {
            ...
            status: 'OPEN',
            priority: TicketPriority.HIGH, // ✅ FIXED
            category: TicketCategory.PAYMENT_ISSUE, // ✅ FIXED
          },
        }),
      );
      ...
      await retry(() =>
        prisma.ticketMessage.create({
          data: {
            ticketId: ticket.id,
            senderId: user.id,
            senderType: SenderType.CUSTOMER, // ✅ FIXED

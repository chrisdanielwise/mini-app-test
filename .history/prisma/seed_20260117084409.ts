...
console.log('👑 Minting Primary Merchant: GreysuitFx');

let greyMerchant: any;

try {
  // We use create instead of upsert to reduce complexity
  greyMerchant = await retry(() => prisma.merchantProfile.create({
    data: {
      adminUserId: 994858530n,
      companyName: "GreysuitFx",
      // Using the ID from the plan created earlier in the script
      planId: enterprisePlan.id, 
      botId: 6838294498n,
      botToken: "6838294498:AAHR907AbARqogosjcVtSscTyhBWHiGYQDc",
      // ✅ THE PERMANENT FIX: Reference the Enum object directly
  ...
  // P2002 is Prisma's code for "Unique constraint failed" (record already exists)
  if (e.code === 'P2002') {
    console.log('ℹ️  Merchant Profile already exists. Fetching existing record...');
    greyMerchant = await prisma.merchantProfile.findUniqueOrThrow({
      where: { adminUserId: 994858530n }
    });
  } else {
    console.error('❌ Error creating Merchant Profile:', e);
    throw e;
  }
}

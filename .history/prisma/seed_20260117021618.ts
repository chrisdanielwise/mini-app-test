import {
  ...
  MerchantProfile,
  Service
} from '../src/generated/prisma'; // 👈 Check this matches your output path
...
// 🛡️ STRIP SSL PARAMS: Remove "?sslmode=..." to prevent conflicts with manual config
if (connectionString && connectionString.includes('?')) {
  connectionString = connectionString.split('?')[0];
}

// 🛡️ FORCE SSL TRUST: Aiven uses self-signed certs, so we must rejectUnauthorized: false
const pool = new Pool({
  connectionString,
  ssl: {
...
async function main() {
  ...
  const plans: any[] = []; // Using any[] to store created plans for reference
  ...
  const greyUser = await retry(() =>
    prisma.user.upsert({
      ...
      create: {
        ...
        role: UserRole.MERCHANT, // 👈 ENUM USAGE
        email: 'admin@greysuitfx.com',
        platformSubscription: {
          create: { 
            status: SubscriptionStatus.ACTIVE, // 👈 ENUM USAGE
  ...
  const greyProfile = await retry(() =>
    prisma.merchantProfile.upsert({
      ...
      create: {
        ...
        planStatus: SubscriptionStatus.ACTIVE, // 👈 ENUM USAGE
        payoutConfig: {
  ...
  for (let i = 0; i < 195; i++) {
            ...
            create: {
              status:
                scenario === 'FRAUD' || scenario === 'CHURNED'
                  ? SubscriptionStatus.INACTIVE // 👈 ENUM USAGE
                  : SubscriptionStatus.ACTIVE, // 👈 ENUM USAGE
    ...
    if (
      ...
      const payment = await retry(() =>
        prisma.payment.create({
          data: {
            ...
            amount: tierPrice,
            currency: 'USD',
            status: PaymentStatus.SUCCESS, // 👈 ENUM USAGE
      ...
      // 🧮 Calculate Ledger amounts manually
      ...
      await retry(() =>
        prisma.ledger.create({
          data: {
            merchantId: merchantData.profile.id,
            amount: merchantNet,
            type: LedgerType.CREDIT, // 👈 ENUM USAGE
            description: `Sale: ${tier.name}`,
            balanceAfter: merchantNet,
        ...
        await retry(() =>
          prisma.groupMembership.create({
            data: {
              ...
              status: 'JOINED', // NOTE: MembershipStatus enum might be needed here if typed strictly, but string often works for string-based enums if verified. Assuming 'JOINED' matches schema.
      ...
      if (scenario === 'WHALE') {
        ...
        await retry(() =>
          prisma.copierExecution.create({
            data: {
              userId: user.id,
              status: ExecutionStatus.SUCCESS, // 👈 ENUM USAGE
              brokerTicket: String(
      ...
      const payment = await retry(() =>
        prisma.payment.create({
          data: {
            ...
            amount: tierPrice,
            currency: 'USD',
            status: PaymentStatus.REFUNDED, // 👈 ENUM USAGE
            gatewayProvider: 'STRIPE',
            gatewayReference: `ch_${faker.string.alphanumeric(20)}`,
          },
        }),
      ...
      await retry(() =>
        prisma.screenshot.create({
          data: {
            ...
            status: PaymentStatus.PENDING, // 👈 ENUM USAGE
            packageType: 'Generic',
      ...
      await retry(() =>
        prisma.payment.create({
          data: {
            ...
            amount: tierPrice,
            currency: 'USD',
            status: PaymentStatus.FAILED, // 👈 ENUM USAGE
            gatewayProvider: 'STRIPE',
      ...
      const ticket = await retry(() =>
        prisma.ticket.create({
          data: {
            ...
            status: 'OPEN',
            priority: TicketPriority.HIGH, // 👈 ENUM USAGE
            category: TicketCategory.PAYMENT_ISSUE, // 👈 ENUM USAGE
          },
        }),
      );
      ...
      await retry(() =>
        prisma.ticketMessage.create({
          data: {
            ticketId: ticket.id,
            senderId: user.id,
            senderType: SenderType.CUSTOMER, // 👈 ENUM USAGE
  ...
  await retry(() =>
    prisma.verificationDocument.create({
      data: {
        ...
        status: 'APPROVED', // NOTE: Check DocStatus enum if validation fails, usually DocStatus.APPROVED
      },
    }),
  );

...
async function main() {
  ...
  const greyUser = await retry(() => prisma.user.upsert({
    ...
    create: {
      ...
      role: 'MERCHANT', // ✅ FIXED: STRICT ENUM
      email: 'admin@greysuitfx.com',
      language: LanguageCode.EN as any
    }
  }));

  let greyMerchant: any;
  try {
    greyMerchant = await retry(() => prisma.merchantProfile.create({
      data: {
        ...
        planStatus: SubscriptionStatus.ACTIVE, // ✅ FIXED: STRICT ENUM
    ...
    if (e.code === 'P2002') {
      greyMerchant = await prisma.merchantProfile.findUniqueOrThrow({ where: { adminUserId: greyUser.telegramId } });
    } else { throw e; }
  }
    ...
    data: {
      ...
      tiers: {
        create: {
          ...
          interval: IntervalUnit.MONTH, // ✅ FIXED: STRICT ENUM
          intervalCount: 1,
          type: SubscriptionType.CUSTOM, // ✅ FIXED: STRICT ENUM
          isActive: true
        }
      }
    },
  ...
  for (let i = 0; i < 15; i++) {
    const customer = await prisma.user.create({
      data: {
        telegramId: BigInt(faker.number.int({ min: 111111111, max: 888888888 })),
    ...
    const payment = await prisma.payment.create({
      data: {
        ...
        currency: 'USD',
        status: PaymentStatus.SUCCESS, // ✅ FIXED: STRICT ENUM
    ...
    await prisma.ledger.create({
      data: {
        ...
        amount: new Decimal(48.50), // Net after fee
        type: LedgerType.CREDIT, // ✅ FIXED: STRICT ENUM
        description: 'VIP Signals Subscription',
        balanceAfter: new Decimal(1000 + (i * 48.50))
      }
    });
    ...
    await prisma.subscription.create({
      data: {
        ...
        status: SubscriptionStatus.ACTIVE, // ✅ FIXED: STRICT ENUM
  ...
  console.log('✅ SEEDING COMPLETE: All Tables Populated with Strict Enum Logic.');
}

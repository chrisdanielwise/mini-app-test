...
async function main() {
  ...
  console.log('✅ VERSION 11.0 FINAL (Global Double-Cast Fix)');
  ...
  const standardServicesData = [
    {
      ...
      tiers: [
        ...
        { name: '12 Months Access', price: 400.0, interval: "YEAR" as unknown as IntervalUnit, intervalCount: 1, type: "CUSTOM" as unknown as SubscriptionType },
      ],
    },
    {
      name: 'Mentorship',
      ...
      tiers: [
        { name: 'Mentorship Fee', price: 300.0, interval: "MONTH" as unknown as IntervalUnit, intervalCount: 6, type: "CUSTOM" as unknown as SubscriptionType },
      ],
    }
  ];
  ...
  for (let i = 0; i < 4; i++) {
    const plan = plans[randomInt(0, 2)];
    const user = await retry(() =>
      prisma.user.create({
        data: {
          ...
          platformSubscription: {
            create: { status: "ACTIVE" as unknown as SubscriptionStatus, expiresAt: futureDate(365) },
          },
        },
      }),
    );
    ...
    const s = await retry(() =>
      prisma.service.create({
        data: {
          ...
          tiers: {
            create: [
              { name: 'Monthly', price: new Decimal(40.0), interval: "MONTH" as unknown as IntervalUnit, intervalCount: 1, type: "CUSTOM" as unknown as SubscriptionType },
            ],
          },
        },
        include: { tiers: true },
  ...
  for (let i = 0; i < 195; i++) {
    ...
    if (scenario === 'REFUNDED') {
      const p = await retry(() => prisma.payment.create({
        data: { userId: user.id, serviceId: randomService.id, merchantId: merchantData.profile.id, amount: tierPrice, currency: 'USD', status: "REFUNDED" as unknown as PaymentStatus }
      }));
      await retry(() => prisma.refund.create({
        data: { paymentId: p.id, merchantId: merchantData.profile.id, amount: tierPrice, reason: 'Customer Request', status: 'APPROVED' }
      }));
    }

    if (scenario === 'MANUAL_PAYMENT') {
        await retry(() => prisma.screenshot.create({
          data: { userId: user.id, merchantId: merchantData.profile.id, photoId: faker.string.alphanumeric(10), status: "PENDING" as unknown as PaymentStatus, packageType: 'VIP', paymentType: 'USDT' }
        }));
    }

    if (i % 15 === 0) {
      const ticket = await retry(() => prisma.ticket.create({
        data: { merchantId: merchantData.profile.id, userId: user.id, subject: 'Payment Issue', status: 'OPEN', priority: "HIGH" as unknown as TicketPriority, category: "PAYMENT_ISSUE" as unknown as TicketCategory }
      }));
      await retry(() => prisma.ticketMessage.create({
        data: { ticketId: ticket.id, senderId: user.id, senderType: "CUSTOMER" as unknown as SenderType, message: 'I paid but no access.' }
      }));
    }
  }
  ...
  for (let d = 0; d < 30; d++) {
    await retry(() => prisma.merchantDailyMetric.create({
      data: {
        merchantId: greyProfile.id,
        date: pastDate(30 - d),

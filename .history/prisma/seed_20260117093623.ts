...
async function main() {
  ...
  await retry(() => prisma.webhookEvent.create({
    data: { provider: 'STRIPE', externalId: faker.string.alphanumeric(10), eventType: 'invoice.failed', status: 'FAILED', errorLog: 'Sample Error' }
  }));

  await retry(() => prisma.verificationDocument.create({
    data: { merchantId: greyProfile.id, type: 'PASSPORT', fileUrl: faker.image.url(), status: 'APPROVED' as unknown as SubscriptionType.APP }
  }));

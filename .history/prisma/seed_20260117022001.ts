...
async function main() {
  ...
  console.log('✅ VERSION 2.0 LOADED (Strict Enums)');
  ...
  const greyUser = await retry(() =>
    prisma.user.upsert({
      ...
      create: {
        ...
        // 🟢 FIX IS HERE: Using Enum instead of String
        role: UserRole.MERCHANT, 
        email: 'admin@greysuitfx.com',
        platformSubscription: {
          create: { 
            status: SubscriptionStatus.ACTIVE, // 🟢 FIX IS HERE
  ...
  const greyProfile = await retry(() =>
    prisma.merchantProfile.upsert({
      ...
      create: {
        ...
        planStatus: SubscriptionStatus.ACTIVE, // 🟢 FIX IS HERE
        payoutConfig: {
    ...
    const user = await retry(() =>
      prisma.user.create({
        data: {
          ...
          role: UserRole.MERCHANT, // 🟢 FIX IS HERE
    ...
    const user = await retry(() =>
      prisma.user.create({
        data: {
          ...
          role: UserRole.USER, // 🟢 FIX IS HERE
          email: faker.internet.email(),
          platformSubscription: {
            create: {
              status:
                scenario === 'FRAUD' || scenario === 'CHURNED'
                  ? SubscriptionStatus.INACTIVE // 🟢 FIX IS HERE
                  : SubscriptionStatus.ACTIVE, // 🟢 FIX IS HERE
      ...
      const payment = await retry(() =>
        prisma.payment.create({
          data: {
            ...
            amount: tierPrice,
            currency: 'USD',
            status: PaymentStatus.SUCCESS, // 🟢 FIX IS HERE
      ...
      await retry(() =>
        prisma.ledger.create({
          data: {
            merchantId: merchantData.profile.id,
            amount: merchantNet,
            type: LedgerType.CREDIT, // 🟢 FIX IS HERE
        ...
        await retry(() =>
          prisma.affiliateConversion.create({
            data: {
              ...
              status: 'PENDING', // If you have an Enum for this, use it!
            },
          }),
        );
        ...
        await retry(() =>
          prisma.groupMembership.create({
            data: {
              ...
              status: 'JOINED', // NOTE: Replace with MembershipStatus.JOINED if strict
      ...
      if (scenario === 'WHALE') {
        ...
        await retry(() =>
          prisma.copierExecution.create({
            data: {
              userId: user.id,
              status: ExecutionStatus.SUCCESS, // 🟢 FIX IS HERE
              brokerTicket: String(
      ...
      const payment = await retry(() =>
        prisma.payment.create({
          data: {
            ...
            amount: tierPrice,
            currency: 'USD',
            status: PaymentStatus.REFUNDED, // 🟢 FIX IS HERE
            gatewayProvider: 'STRIPE',
            gatewayReference: `ch_${faker.string.alphanumeric(20)}`,
          },
        }),
      ...
      await retry(() =>
        prisma.screenshot.create({
          data: {
            ...
            status: PaymentStatus.PENDING, // 🟢 FIX IS HERE
            packageType: 'Generic',
      ...
      await retry(() =>
        prisma.payment.create({
          data: {
            ...
            amount: tierPrice,
            currency: 'USD',
            status: PaymentStatus.FAILED, // 🟢 FIX IS HERE
            gatewayProvider: 'STRIPE',
      ...
      const ticket = await retry(() =>
        prisma.ticket.create({
          data: {
            ...
            status: 'OPEN',
            priority: TicketPriority.HIGH, // 🟢 FIX IS HERE
            category: TicketCategory.PAYMENT_ISSUE, // 🟢 FIX IS HERE
          },
        }),
      );
      ...
      await retry(() =>
        prisma.ticketMessage.create({
          data: {
            ticketId: ticket.id,
            senderId: user.id,
            senderType: SenderType.CUSTOMER, // 🟢 FIX IS HERE

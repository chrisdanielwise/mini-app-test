  ...
  await retry(() =>
    prisma.settings.upsert({
      ...
      create: {
        ...
        language: "en", // Mapped from LanguageCode.EN
      },
    })
  );
    ...
    const user = await retry(() =>
      prisma.user.create({
        data: {
          ...
          platformSubscription: {
            create: {
              status: (scenario === "FRAUD" || scenario === "CHURNED") 
  ...
  for (let d = 0; d < 30; d++) {
    ...
    await retry(() =>
      prisma.merchantDailyMetric.upsert({
        where: { merchantId_date: { merchantId: greyProfile.id, date: targetDate } },
        update: {
  ...
  await retry(() =>
    prisma.webhookEvent.create({
      data: {
        ...
        payload: { id: "evt_test_123", object: "event", reason: "card_declined" } as Prisma.InputJsonObject,
        status: "FAILED", 

...
import { 
  ...
  Download, ChevronRight, Terminal, UserPlus, Trash2, Building2, Activity 
} from "lucide-react";
...
/**
 * 🛰️ SUBSCRIBERS_CLIENT_PAGE (Institutional Apex v2026.1.20)
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 px-6">
          <div className="space-y-1.5 min-w-0">
            <div className="flex items-center gap-2.5 italic opacity-30">
              <UserPlus className={cn("size-3", isSuperAdmin ? "text-amber-500" : "text-primary")} />
              <span className={cn(
            ...
            <div className="flex items-center gap-2 text-[7px] font-black text-muted-foreground/20 italic uppercase tracking-[0.2em]">
              ...
              <span className="truncate">Validated: {initialSubscriptions.length} Nodes Synchronized // Identity: {role}</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 w-full md:w-auto shrink-0 scale-90 origin-bottom-right">
            ...
            <div className="flex gap-2">
              ...
              <Button className={cn(
                "h-10 px-4 rounded-xl font-black uppercase tracking-[0.2em] text-[8px] shadow-lg",
          ...
          <div className="flex-1 w-full overflow-auto custom-scrollbar">
            <table className="w-full text-left border-collapse min-w-[1000px] table-fixed">
              <thead className="bg-white/[0.04] border-b border-white/5 sticky top-0 z-20 backdrop-blur-md">
                <tr>
                  <th className="w-[30%] px-6 py-2.5 text-[7.5px] font-black uppercase tracking-[0.5em] text-muted-foreground/30 italic">Identity Node</th>
                  {isSuperAdmin && <th className="w-[18%] px-6 py-2.5 text-[7.5px] font-black uppercase tracking-[0.5em] text-amber-500/30 italic">Origin Merchant</th>}
                  <th className="w-[18%] px-6 py-2.5 text-[7.5px] font-black uppercase tracking-[0.5em] text-muted-foreground/30 italic">Service // Tier</th>
                  <th className="w-[14%] px-6 py-2.5 text-[7.5px] font-black uppercase tracking-[0.5em] text-muted-foreground/30 italic">Protocol Status</th>
                  <th className="w-[13%] px-6 py-2.5 text-[7.5px] font-black uppercase tracking-[0.5em] text-muted-foreground/30 italic">Expiry Horizon</th>
                    ...
                    >
                      ...
                      <td className="px-6 py-3.5">
                        <div className="flex flex-col leading-tight">
                          ...
                          <p className="text-[6px] font-bold text-muted-foreground/20 uppercase tracking-widest ml-5 italic">
                      ...
                      <td className="px-6 py-3.5">
                        <div className={cn(
                          ...
                          <div className={cn("size-1 rounded-full mr-1.5 animate-pulse", sub.status === "ACTIVE" ? "bg-emerald-500" : "bg-amber-500")} />
                          {sub.status}
                        </div>
                      </td>

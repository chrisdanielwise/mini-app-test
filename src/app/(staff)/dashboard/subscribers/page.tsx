...
/**
 * 🏛️ SUBSCRIBER LEDGER CLUSTER (Institutional v9.3.1)
 * Fix: Dynamic RBAC Filter to prevent 'merchantId: null' validation errors.
 * Logic: Super Admins see global traffic; Merchants see local traffic.
 */
export default async function SubscribersPage() {
  ...
  // If merchantId is null (Staff), we pass an empty object to fetch all.
  ...
  const subscriptions = await prisma.subscription.findMany({
    ...
    include: {
      user: true,
      service: {
        include: { merchant: true } // Fetch merchant info for Super Admin view
      },
      serviceTier: true,
    },
    orderBy: {
    ...
    <div className="max-w-7xl mx-auto space-y-6 md:space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500 pb-10 px-4 text-foreground">
      ...
      <div className="flex flex-wrap items-end justify-between gap-x-6 gap-y-6 w-full border-b border-border/40 pb-6">
        <div className="flex flex-col gap-3 min-w-fit flex-1">
          <div className="flex items-center gap-2 text-primary/60">
            ...
            <span className="text-[10px] font-bold uppercase tracking-widest italic opacity-80 whitespace-nowrap">
              {isSuperAdmin ? "Global Network Hub" : "Merchant Identity Hub"}
            </span>
          </div>
          ...
          <div className="space-y-1">
            ...
            <p className="text-[9px] md:text-[10px] font-bold text-muted-foreground uppercase tracking-widest opacity-40 italic">
              Validated: <span className="text-foreground tabular-nums">{subscriptions.length}</span> Nodes Synchronized
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-stretch gap-2.5 w-full lg:w-auto shrink-0">
          ...
          <div className="flex gap-2 h-10 md:h-11">
            <Button variant="outline" className="flex-1 lg:flex-none px-4 rounded-xl border-border/40 bg-muted/10 text-[9px] font-bold uppercase tracking-widest">
              <Filter className="mr-2 h-3.5 w-3.5 opacity-40" /> Filter
            </Button>
            <Button className="flex-1 lg:flex-none px-4 rounded-xl font-bold uppercase tracking-widest text-[9px] shadow-lg bg-primary text-primary-foreground">
          ...
          <table className="w-full text-left border-collapse min-w-[900px]">
            <thead>
              <tr className="bg-muted/30 border-b border-border/10">
                <th className="px-6 py-4 text-[9px] font-black uppercase tracking-widest text-muted-foreground/60">Identity Node</th>
                {isSuperAdmin && <th className="px-6 py-4 text-[9px] font-black uppercase tracking-widest text-muted-foreground/60">Origin Merchant</th>}
                <th className="px-6 py-4 text-[9px] font-black uppercase tracking-widest text-muted-foreground/60">Service // Tier</th>
                <th className="px-6 py-4 text-[9px] font-black uppercase tracking-widest text-muted-foreground/60">Protocol Status</th>
                <th className="px-6 py-4 text-[9px] font-black uppercase tracking-widest text-muted-foreground/60">Expiry Horizon</th>
                <th className="px-6 py-4"></th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border/10">
              {subscriptions.length === 0 ? (
                <tr><td colSpan={isSuperAdmin ? 6 : 5} className="px-6 py-24 text-center opacity-20 font-black uppercase italic tracking-widest text-xs">Zero nodes synchronized.</td></tr>
              ) : (
                subscriptions.map((sub) => (
                  <tr key={sub.id} className="hover:bg-primary/[0.02] transition-colors group">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="h-10 w-10 shrink-0 rounded-xl bg-primary/5 flex items-center justify-center text-primary font-black italic border border-primary/20 shadow-inner">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-col gap-1">
                        <div className="flex items-center gap-2">
                          ...
                          <p className="text-[10px] font-black uppercase tracking-wider text-primary/80 italic truncate max-w-[140px]">
                            {sub.service.name}
                          </p>
                        </div>
                    ...
                    <td className="px-6 py-4">
                      <div className={cn(
                        ...
                        sub.status === "ACTIVE" ? "bg-emerald-500/5 text-emerald-500 border-emerald-500/20" : "bg-amber-500/5 text-amber-500 border-amber-500/20"
                      )}>
                        <div className={cn("h-1 w-1 rounded-full mr-1.5 animate-pulse", sub.status === "ACTIVE" ? "bg-emerald-500" : "bg-amber-500")} />
                        {sub.status}
                      </div>
                    </td>

                    <td className="px-6 py-4 text-[10px] font-bold uppercase italic text-foreground/60">
                      <div className="flex items-center gap-2">
                    ...
                    <td className="px-6 py-4 text-right">
                      <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                          <Button variant="ghost" size="icon" className="h-8 w-8 rounded-lg bg-muted/10 border border-border/10 hover:bg-primary hover:text-white">
      ...
      <div className="flex items-center gap-3 px-1 opacity-20 py-4">
        ...
        <p className="text-[8px] font-black uppercase tracking-[0.3em] italic">
          Verified Link Established // Role: {session.user.role} // Status: Optimal
        </p>
      </div>
    </div>
  );

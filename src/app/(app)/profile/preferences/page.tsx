...
/**
 * 🛰️ USER PREFERENCES TERMINAL (Institutional v9.5.3)
 * Logic: Role-Aware identity auditing and hardware sync.
 * Hardened: Null-safe Haptics and Next.js 16 Hydration Shield.
 */
export default function UserPreferencesPage() {
  ...
  // 🛡️ HYDRATION SHIELD: Restore hardware preferences
  useEffect(() => {
    if (mounted) {
  ...
  // 🛡️ Haptic Protocol Buffer: Verifies WebApp availability
  ...
  // 🛡️ STAFF CLEARANCE DETERMINATION
  const isStaff = useMemo(() => 
    auth.user?.role && ["super_admin", "platform_manager"].includes(auth.user.role),
    [auth.user?.role]
  );

  // 1. SYSTEM INITIALIZATION: Wait for identity handshake and local mount
  if (!isReady || !isMounted || auth.isLoading) {
    ...
    <div className="flex flex-col min-h-[100dvh] animate-in fade-in duration-700 max-w-3xl mx-auto text-foreground selection:bg-primary/30">
      ...
      <main className="px-5 py-8 space-y-8 pb-36">
        ...
        <section className="space-y-3">
          ...
          <div className={cn(
            "group rounded-2xl border bg-card/40 p-5 md:p-6 flex items-center justify-between shadow-lg backdrop-blur-3xl transition-all duration-500",
            isStaff ? "hover:border-amber-500/20" : "hover:border-primary/20"
          )}>
            <div className="flex items-center gap-4 min-w-0">
              <div className={cn(
                "h-12 w-12 shrink-0 rounded-xl flex items-center justify-center border shadow-inner transition-transform group-hover:scale-110",
              ...
              <div className="space-y-1 min-w-0">
                <Label className="text-base font-black uppercase italic tracking-tight leading-none">
                  Haptic Feedback
                </Label>
                <p className="text-[9px] font-bold text-muted-foreground/40 uppercase tracking-widest leading-none">
                  Vibration on node interaction
                </p>
              </div>
            </div>
            <Switch
        ...
        <section className="space-y-3">
          ...
          <div className="rounded-[2rem] border border-border/40 bg-card/40 overflow-hidden shadow-2xl backdrop-blur-md">
            ...
            <div className="p-6 border-b border-border/10 flex items-center justify-between gap-4">
              <div className="flex items-center gap-4 min-w-0">
                ...
                <div className="space-y-1 min-w-0">
                  ...
                  <p className="text-base font-black italic tracking-tight truncate">
                    @{user?.username || auth.user?.username || "anonymous_node"}
                  </p>
                </div>
              </div>
              <Badge
                className={cn(
                  "shrink-0 rounded-lg text-[8px] font-black tracking-[0.2em] px-2 py-0.5 border-none",
                  isStaff ? "bg-amber-500/10 text-amber-500" : "bg-emerald-500/10 text-emerald-500"
                )}
              >
            ...
            {/* Protocol Role */}
            <div className="p-6 flex items-center justify-between group cursor-default hover:bg-muted/5 transition-colors">
              <div className="flex items-center gap-4 min-w-0">
              ...
              <UserCog className="h-4 w-4 opacity-20 group-hover:rotate-12 group-hover:opacity-60 transition-all" />
            </div>
          </div>
        </section>
        ...
        <div className="pt-6 space-y-6">
          <div className="flex items-center gap-3 px-2 opacity-30 italic">
            ...
            <p className="text-[8px] font-black uppercase tracking-[0.4em]">
              Zipha Intelligence Core // Build: 2026.01.v2.26
            </p>
          </div>

          <div className="rounded-2xl bg-muted/10 border border-border/10 p-5 flex gap-4 shadow-inner relative overflow-hidden">
            ...
            <p className="text-[9px] font-bold text-muted-foreground/60 leading-relaxed uppercase tracking-widest italic relative z-10">
              Hardware settings are node-specific. Global identity revocation is managed via the <span className="text-foreground font-black">Institutional Hub</span>. Unauthorized tampering triggers local lockout.
            </p>
          </div>
        </div>
      </main>
      ...
      <footer className="mt-auto flex items-center justify-center gap-3 opacity-20 py-10">
         ...
         <p className="text-[8px] font-black uppercase tracking-[0.5em] italic text-center leading-none">
           Audit Handshake Established // Sync: {isReady ? "Optimal" : "Pending"}
         </p>
      </footer>
    </div>
  );

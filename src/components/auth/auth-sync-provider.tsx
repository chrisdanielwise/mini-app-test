...
/**
 * 🛰️ AUTH_SYNC_PROVIDER (v14.95.0)
 * Logic: Global Broadcast Listener for Identity Anchoring & Revocation.
...
export function AuthSyncProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    authChannel.onmessage = (event) => {
      ...
      // We check for both the short key and the full protocol string for redundancy.
      const isRevocation = [
        "GLOBAL_REVOCATION", 
        "GLOBAL_REVOCATION_PROTOCOL", 
        "TERMINATE_SESSION"
      ].includes(action);

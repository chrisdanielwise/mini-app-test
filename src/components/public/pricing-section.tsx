...
import { Check, Layers, Activity, Sparkles, Waves } from "lucide-react";
 ...
 * Fix: High-density p-6 cards and h-11 buttons prevent layout blowout.
 */
export function PricingSection() {
  const { flavor } = useLayout();
  const { isStaff } = useInstitutionalAuth();
  const { screenSize, isMobile, isTablet, isDesktop, isPortrait, viewportWidth, isReady } = useDeviceContext();
  ...
  return (
    <section id="pricing" className={cn("relative overflow-hidden transition-all duration-700", sectionPadding)}>
      ...
      {/* --- TIER GRID: Tactical Slim Matrix --- */}

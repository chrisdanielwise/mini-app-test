    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button 
          ...
          className={cn(
            "rounded-xl md:rounded-2xl h-12 md:h-14 px-6 md:px-8 font-black uppercase italic tracking-widest shadow-2xl transition-all hover:scale-[1.02] active:scale-95 text-[10px] md:text-xs",
            isStaff 
        ...
        <form action={formAction} className="flex flex-col h-full overflow-hidden">
            ...
            <DialogHeader className="space-y-3">
              <div className="flex items-center gap-3 md:gap-4">
                <div className={cn(
                  ...
                  {isStaff ? <Globe className="h-5 w-5 md:h-6 md:w-6" /> : <Wallet className="h-5 w-5 md:h-6 md:w-6" />}
                </div>
                <div className="min-w-0">
                  ...
                  <DialogDescription className="text-[8px] md:text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground mt-1 opacity-60 truncate">
                    {isStaff ? "Institutional Disbursement Protocol" : `Disbursement Context: ${merchantId.slice(0, 12)}`}
                  </DialogDescription>
                </div>
              </div>
            </DialogHeader>
            ...
            <div className="space-y-6 md:space-y-8">
              ...
              <div className="space-y-3 md:space-y-4">
                <div className="flex justify-between items-end px-1">
                  <Label className={cn("text-[9px] md:text-[10px] font-black uppercase tracking-[0.2em]", isStaff ? "text-amber-500" : "text-primary")}>
                    Liquidity Amount
                  </Label>
                  <button
                    type="button"
                    ...
                    className={cn(
                      "text-[8px] md:text-[9px] font-black uppercase border-b transition-all italic whitespace-nowrap",
                      isStaff ? "text-amber-500/60 border-amber-500/20 hover:text-amber-500" : "text-primary border-primary/20 hover:text-foreground"
                    )}
                  >
                ...
                <div className="relative group">
                  <span className={cn(
                    "absolute left-6 top-1/2 -translate-y-1/2 font-black text-2xl md:text-3xl opacity-20 group-focus-within:opacity-100 transition-opacity",
                  ...
                  <Input
                    ...
                    required
                    className={cn(
                      "rounded-2xl md:rounded-3xl border-border/40 bg-muted/10 h-16 md:h-20 pl-14 font-black text-2xl md:text-3xl tracking-tighter italic text-foreground transition-all shadow-inner",
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 md:gap-6">
                <div className="space-y-2 md:space-y-3">
                  <Label className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.2em] ml-1">Protocol</Label>
                  <div className="relative group">
                    <ArrowRightLeft className={cn("absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 opacity-30 transition-colors pointer-events-none", isStaff ? "group-focus-within:text-amber-500" : "group-focus-within:text-primary")} />
                    <select
                      name="method"
                      className={cn(
                        "w-full h-12 md:h-14 rounded-xl md:rounded-2xl border border-border/40 bg-muted/10 pl-12 pr-4 text-[9px] md:text-[10px] font-black uppercase tracking-widest cursor-pointer outline-none appearance-none transition-all shadow-inner",
                        isStaff ? "focus:ring-amber-500/20" : "focus:ring-primary/20"
                      )}
                    >
                ...
                <div className="space-y-2 md:space-y-3">
                  <Label className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.2em] ml-1">Target Address</Label>
                  <div className="relative group">
                    <Terminal className={cn("absolute left-4 top-1/2 -translate-y-1/2 h-4 w-4 opacity-30 transition-colors pointer-events-none", isStaff ? "group-focus-within:text-amber-500" : "group-focus-within:text-primary")} />
                    <Input
                      name="destination"
                      placeholder="ENTER_ID"
                      required
                      className={cn(
                        "rounded-xl md:rounded-2xl border-border/40 bg-muted/10 h-12 md:h-14 pl-12 font-black uppercase text-[9px] md:text-[10px] tracking-tight shadow-inner",
                        isStaff ? "focus:ring-amber-500/20" : "focus:ring-primary/20"
                      )}
                    />
                  </div>
              ...
              {/* --- Operational Notice --- */}
              <div className={cn("rounded-xl md:rounded-[1.5rem] border p-4 md:p-5 flex items-start gap-3 md:gap-4 shadow-sm", isStaff ? "bg-amber-500/5 border-amber-500/10" : "bg-primary/5 border-primary/10")}>
                <Info className={cn("h-4 w-4 shrink-0 mt-0.5", isStaff ? "text-amber-500" : "text-primary")} />
          ...
          <div className="bg-muted/30 p-5 md:p-8 flex flex-col sm:flex-row items-center justify-end gap-3 md:gap-4 border-t border-border/40 backdrop-blur-md">
            ...
            <Button
              ...
              className={cn(
                "w-full sm:flex-1 h-14 md:h-16 rounded-xl md:rounded-[1.5rem] font-black uppercase italic tracking-[0.15em] shadow-2xl transition-all active:scale-95 disabled:opacity-20",
                isStaff 
                  ? "bg-amber-500 text-black shadow-amber-500/20" 
                  : "bg-primary text-primary-foreground shadow-primary/20"
              )}

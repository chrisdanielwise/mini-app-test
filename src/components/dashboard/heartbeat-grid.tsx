"use client";

import * as React from "react";
import { cn } from "@/lib/utils";
import { Activity, ShieldCheck, Zap, AlertCircle, Cpu, Terminal } from "lucide-react";

// 🏛️ Institutional Contexts & Hooks
import { useLayout } from "@/context/layout-provider";
import { useHaptics } from "@/lib/hooks/use-haptics";
import { useDeviceContext } from "@/components/providers/device-provider";

interface ServiceNode {
  id: string;
  name: string;
  status: "HEALTHY" | "LATENT" | "DEGRADED" | "OFFLINE";
  uptime: string;
}

/**
 * 🛰️ HEARTBEAT_GRID (Institutional Apex v2026.1.20)
 * Strategy: Vertical Compression & Tactical Slim Geometry.
 * Fix: High-density row density and iconography prevents viewport blowout.
 */
export function HeartbeatGrid({ services }: { services: ServiceNode[] }) {
  const { flavor } = useLayout();
  const { impact } = useHaptics();
  const { isReady, screenSize, isMobile } = useDeviceContext();
  
  const isStaffTheme = flavor === "AMBER";

  if (!isReady) return (
    <div className="h-64 w-full bg-card/10 animate-pulse rounded-2xl border border-white/5" />
  );

  const gridCols = {
    xs: "grid-cols-2",
    sm: "grid-cols-3",
    md: "grid-cols-4",
    lg: "grid-cols-6",
    xl: "grid-cols-8",
    xxl: "grid-cols-10"
  }[screenSize] || "grid-cols-5";

  return (
    <div className={cn(
      "relative group flex flex-col w-full transition-all duration-700 shadow-2xl",
      "rounded-3xl border backdrop-blur-xl overflow-hidden",
      isStaffTheme ? "bg-amber-500/[0.01] border-amber-500/10" : "bg-card/40 border-white/5",
      "p-5 md:p-6" // Reduced p-10 -> p-6
    )}>
      
      {/* 🌫️ VAPOUR DEPTH radiants */}
      <div className={cn(
        "absolute -left-24 -top-24 size-48 blur-[100px] opacity-10 pointer-events-none transition-all duration-1000",
        isStaffTheme ? "bg-amber-500" : "bg-primary"
      )} />

      {/* --- TELEMETRY HEADER: Compressed HUD --- */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4 relative z-10 leading-none">
        <div className="space-y-1.5">
          <div className="flex items-center gap-2.5 italic opacity-30">
            <Activity className={cn("size-2.5 animate-pulse", isStaffTheme ? "text-amber-500" : "text-primary")} />
            <div className="flex items-center gap-2">
              <span className="text-[7.5px] font-black uppercase tracking-[0.3em]">Cluster_Sync</span>
              <span className="text-[6.5px] font-mono uppercase opacity-40">v16.31</span>
            </div>
          </div>
          <h2 className="text-xl md:text-2xl font-black uppercase italic tracking-tighter text-foreground">
            Node <span className={isStaffTheme ? "text-amber-500" : "text-primary"}>Heartbeat</span>
          </h2>
        </div>
        
        <div className="flex items-center gap-4 px-3 py-1.5 rounded-lg bg-black/40 border border-white/5 backdrop-blur-md">
          <LegendNode color="bg-emerald-500" label="Healthy" />
          <LegendNode color="bg-amber-500" label="Latent" />
          <LegendNode color="bg-rose-500" label="Degraded" />
        </div>
      </div>

      {/* --- KINETIC TACTICAL GRID: High Density --- */}
      <div className={cn("grid gap-2 relative z-10", gridCols)}>
        {services.map((node, index) => (
          <div
            key={node.id}
            onMouseEnter={() => impact("light")}
            style={{ animationDelay: `${index * 20}ms` }}
            className={cn(
              "group relative flex flex-col items-center justify-center aspect-square rounded-2xl border transition-all duration-300",
              "hover:bg-white/[0.02] hover:border-white/10",
              node.status === "HEALTHY" && "border-emerald-500/5 bg-emerald-500/[0.01]",
              node.status === "LATENT" && "border-amber-500/5 bg-amber-500/[0.01]",
              node.status === "DEGRADED" && "border-rose-500/5 bg-rose-500/[0.01]"
            )}
          >
            {/* 💓 STATUS PULSE: Compressed size-8 */}
            <div className="relative size-8 mb-2 flex items-center justify-center">
              <div className={cn(
                "absolute inset-0 rounded-full animate-ping opacity-5",
                node.status === "HEALTHY" && "bg-emerald-500",
                node.status === "LATENT" && "bg-amber-500",
                node.status === "DEGRADED" && "bg-rose-500"
              )} />
              <div className={cn(
                "relative size-6.5 rounded-lg flex items-center justify-center border shadow-inner transition-transform group-hover:scale-110",
                node.status === "HEALTHY" && "bg-emerald-500/10 text-emerald-500 border-emerald-500/20",
                node.status === "LATENT" && "bg-amber-500/10 text-amber-500 border-amber-500/20",
                node.status === "DEGRADED" && "bg-rose-500/10 text-rose-500 border-rose-500/20"
              )}>
                {node.status === "HEALTHY" && <ShieldCheck className="size-3.5" />}
                {node.status === "LATENT" && <Zap className="size-3.5" />}
                {node.status === "DEGRADED" && <AlertCircle className="size-3.5" />}
              </div>
            </div>

            <div className="space-y-0.5 text-center px-1.5 w-full">
               <p className="text-[7.5px] font-black uppercase tracking-widest text-foreground/70 truncate">
                {node.name}
              </p>
              <div className="flex items-center justify-center gap-1 opacity-20">
                <Cpu className="size-2 text-primary" />
                <p className="text-[6px] font-mono tracking-tighter uppercase leading-none">
                  {node.uptime}
                </p>
              </div>
            </div>
          </div>
        ))}
      </div>

      {/* --- FOOTER SIGNAL --- */}
      <div className="mt-6 flex items-center justify-between opacity-10 italic border-t border-white/5 pt-4 relative z-10">
        <div className="flex items-center gap-2">
          <Terminal className="size-2.5" />
          <span className="text-[7px] font-black uppercase tracking-[0.3em] text-foreground">
            {isStaffTheme ? "Cluster_Oversight_Active" : "Mesh_Telemetry_Ok"}
          </span>
        </div>
        <span className="text-[6.5px] font-mono tracking-widest">[TX_STABLE]</span>
      </div>
    </div>
  );
}

/** 🛠️ ATOMIC: LEGEND_NODE */
function LegendNode({ color, label }: { color: string, label: string }) {
  return (
    <div className="flex items-center gap-1.5">
      <div className={cn("size-1 rounded-full animate-pulse", color)} />
      <span className="text-[6.5px] font-black uppercase tracking-[0.1em] opacity-30 italic">{label}</span>
    </div>
  );
}
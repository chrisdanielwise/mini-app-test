...
import { useLayout } from "@/context/layout-provider"; // Ensure this matches your context filename
 ...
 * Feature: Hot-swaps navigation nodes based on verified Role Clearance.
 */
export function BottomNav() {
  ...
  // 🛡️ IDENTITY SHIELD: Wait for the handshake before rendering role-based buttons
  ...
  const navItems = [
    ...
    // We use startsWith to keep it active even in sub-pages.
    ...(isStaff ? [{ label: "Dash", href: "/dashboard", icon: LayoutDashboard }] : []),
    { label: "Ledger", href: "/history", icon: History },
    { label: "Profile", href: "/settings", icon: User }, // Pointing to the refined settings page
  ];
  ...
  return (
    <nav className="relative flex items-center justify-around rounded-2xl border border-border/40 bg-card/70 p-2 backdrop-blur-3xl shadow-2xl overflow-hidden">
      {navItems.map((item) => {
        // Active Logic: Matches the base route or any sub-route
      ...
      {/* ⚡ CONTEXT SWITCHER: Staff Oversight Mode */}
      {isStaff && (
        <button
